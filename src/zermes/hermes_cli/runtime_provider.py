...
from zermes.agent.credential_pool import CredentialPool, PooledCredential, get_custom_provider_pool_key, load_pool
...
def _copilot_runtime_api_mode(model_cfg: Dict[str, Any], api_key: str) -> str:
    ...
    try:
        from zermes.hermes_cli.models import copilot_model_api_mode
    ...
    *,
    ...
    if provider == "openai-codex":
        ...
        if effective_model and api_mode != "anthropic_messages":
            try:
                from zermes.hermes_cli.models import azure_foundry_model_api_mode
        ...
        if provider in ("opencode-zen", "opencode-go"):
            ...
            # Refs #16878.
            from zermes.hermes_cli.models import opencode_model_api_mode
...
def _get_named_custom_provider(requested_provider: str) -> Optional[Dict[str, Any]]:
    ...
    if isinstance(providers, dict):
        for ep_name, entry in zermes.providers.items():
...
def _resolve_explicit_runtime(
    *,
    ...
    if provider == "anthropic":
        ...
        api_key = explicit_api_key
        if not api_key:
            from zermes.agent.anthropic_adapter import resolve_anthropic_token
