...
def _check_dispatcher_presence() -> tuple[bool, str]:
    ...
    """
    try:
        from zermes.gateway.status import get_running_pid  # type: ignore
    except Exception:
...
def build_parser(parent_subparsers: argparse._SubParsersAction) -> argparse.ArgumentParser:
    ...
    kanban_parser = parent_subparsers.add_parser(
        ...
        description=(
            ...
            "See https://hermes-zermes.agent.nousresearch.com/docs/user-guide/features/kanban "
    ...
    p_daemon = sub.add_parser(
        "daemon",
        help="DEPRECATED — dispatcher now runs in the zermes.gateway. Use `hermes gateway start`.",
    )
    ...
    # Undocumented escape hatch for users who truly cannot run the zermes.gateway.
...
def _cmd_daemon(args: argparse.Namespace) -> int:
    """Deprecated — the dispatcher now runs inside the zermes.gateway.
    ...
    if not getattr(args, "force", False):
        print(
            ...
            "inside the zermes.gateway. To use kanban:\n"
            "\n"
...
def _cmd_specify(args: argparse.Namespace) -> int:
    ...
    from zermes.hermes_cli import kanban_specify as spec
