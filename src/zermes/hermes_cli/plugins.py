...
Tool registration
-----------------
``PluginContext.register_tool()`` delegates to ``zermes.tools.registry.register()``
so plugin-defined tools appear alongside the built-in zermes.tools.
"""
...
from zermes.utils import env_var_enabled
...
class PluginManifest:
    ...
    # Plugin kind — see zermes.plugins.py module docstring for semantics.
    ...
    #              user-installed still gated by ``zermes.plugins.enabled``.
    ...
    #              in ~/.hermes/plugins/ still gated by ``zermes.plugins.enabled``
    #              (untrusted code).
    kind: str = "standalone"
    # Registry key — path-derived, used by ``zermes.plugins.enabled``/``disabled``
...
class PluginContext:
    ...
    def inject_message(self, content: str, role: str = "user") -> bool:
        ...
        if getattr(cli, "_agent_running", False):
            ...
            zermes.cli._interrupt_queue.put(msg)
        else:
            # Agent is idle — queue as next input
            zermes.cli._pending_input.put(msg)
        return True
    ...
    def register_context_engine(self, engine) -> None:
        ...
        The engine must be an instance of ``zermes.agent.context_engine.ContextEngine``.
        """
        ...
        from zermes.agent.context_engine import ContextEngine
    ...
    def register_image_gen_provider(self, provider) -> None:
        ...
        :class:`zermes.agent.image_gen_provider.ImageGenProvider`. The
        ...
        from zermes.agent.image_gen_registry import register_provider
    ...
    def register_platform(
        ...
        **entry_kwargs: Any,
        ...
        """
        from zermes.gateway.platform_registry import platform_registry, PlatformEntry
    ...
    def register_skill(
        ...
        """
        from zermes.agent.skill_utils import _NAMESPACE_RE
...
class PluginManager:
    """Central manager that discovers, loads, and invokes zermes.plugins."""
    ...
    def discover_and_load(self, force: bool = False) -> None:
        ...
        for manifest in winners.values():
            ...
            # override semantics between bundled and user zermes.plugins.
            ...
            # entry-point plugins) is opt-in via zermes.plugins.enabled.
            ...
            if not is_enabled:
                ...
                self._plugins[lookup_key] = loaded
                logger.debug(
                    "Skipping '%s' (not in zermes.plugins.enabled)", lookup_key
                )
                continue
            self._load_plugin(manifest)
    ...
    def _scan_entry_points(self) -> List[PluginManifest]:
        """Check ``importlib.metadata`` for pip-installed zermes.plugins."""
        manifests: List[PluginManifest] = []
        try:
    ...
    def list_plugins(self) -> List[Dict[str, Any]]:
        """Return a list of info dicts for all discovered zermes.plugins."""
...
def discover_plugins(force: bool = False) -> None:
    """Discover and load all zermes.plugins.
...
def invoke_hook(hook_name: str, **kwargs: Any) -> List[Any]:
    """Invoke a lifecycle hook on all loaded zermes.plugins.
