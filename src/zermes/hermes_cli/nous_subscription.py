...
from zermes.hermes_cli.config import get_env_value, load_config
...
def _uses_gateway(section: object) -> bool:
    """Return True when a config section explicitly opts into the zermes.gateway."""
...
def prompt_enable_tool_gateway(config: Dict[str, object]) -> set[str]:
    ...
    try:
        from zermes.hermes_cli.setup import prompt_choice
    except Exception:
        return set()
