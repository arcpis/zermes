...
def _canonical_assignee(assignee: Optional[str]) -> Optional[str]:
    ...
    if assignee is None:
        return None
    from zermes.hermes_cli.profiles import normalize_profile_name
...
def _pid_alive(pid: Optional[int]) -> bool:
    ...
    Windows (via ``zermes.gateway.status._pid_exists``) and ``os.kill(pid, 0)``
...
def has_spawnable_ready(conn: sqlite3.Connection) -> bool:
    ...
    the warning still fires in degraded zermes.environments.
    """
    rows = conn.execute(
    ...
    if not rows:
        return False
    try:
        from zermes.hermes_cli.profiles import profile_exists  # local import: avoids cycle
    except Exception:
...
def list_profiles_on_disk() -> list[str]:
    ...
    ``zermes.hermes_cli.profiles`` (which pulls in a large chunk of the CLI startup
    path).
    """
    try:
        from zermes.hermes_constants import get_default_hermes_root
