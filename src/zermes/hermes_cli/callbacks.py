...
from zermes.hermes_cli.banner import cprint, _DIM, _RST
...
def clarify_callback(cli, question, choices):
    ...
    zermes.cli._clarify_state = {
        "question": question,
        "choices": choices if not is_open_ended else [],
        "selected": 0,
        "response_queue": response_queue,
    }
    zermes.cli._clarify_deadline = _time.monotonic() + timeout
    zermes.cli._clarify_freetext = is_open_ended

    if hasattr(cli, "_app") and zermes.cli._app:
        zermes.cli._app.invalidate()

    while True:
        try:
            result = response_queue.get(timeout=1)
            zermes.cli._clarify_deadline = 0
            return result
        except queue.Empty:
            remaining = zermes.cli._clarify_deadline - _time.monotonic()
            if remaining <= 0:
                break
            if hasattr(cli, "_app") and zermes.cli._app:
                zermes.cli._app.invalidate()

    zermes.cli._clarify_state = None
    zermes.cli._clarify_freetext = False
    zermes.cli._clarify_deadline = 0
    if hasattr(cli, "_app") and zermes.cli._app:
        zermes.cli._app.invalidate()
...
def prompt_for_secret(cli, var_name: str, prompt: str, metadata=None) -> dict:
    ...
    if not getattr(cli, "_app", None):
        if not hasattr(cli, "_secret_state"):
            zermes.cli._secret_state = None
        if not hasattr(cli, "_secret_deadline"):
            zermes.cli._secret_deadline = 0
        try:
    ...
    zermes.cli._secret_state = {
        "var_name": var_name,
        "prompt": prompt,
        "metadata": metadata or {},
        "response_queue": response_queue,
    }
    zermes.cli._secret_deadline = _time.monotonic() + timeout
    # Avoid storing stale draft input as the secret when Enter is pressed.
    if hasattr(cli, "_clear_secret_input_buffer"):
        try:
            zermes.cli._clear_secret_input_buffer()
        except Exception:
            pass
    elif hasattr(cli, "_app") and zermes.cli._app:
        try:
            zermes.cli._app.current_buffer.reset()
        except Exception:
            pass
    ...
    while True:
        try:
            ...
            remaining = zermes.cli._secret_deadline - _time.monotonic()
            if remaining <= 0:
                break
            if hasattr(cli, "_app") and zermes.cli._app:
                zermes.cli._app.invalidate()
...
def approval_callback(cli, command: str, description: str) -> str:
    ...
    Uses zermes.cli._approval_lock to serialize concurrent requests (e.g. from
    parallel delegation subtasks) so each prompt gets its own turn.
    """
    lock = getattr(cli, "_approval_lock", None)
    if lock is None:
        import threading
        zermes.cli._approval_lock = threading.Lock()
        lock = zermes.cli._approval_lock
    ...
    with lock:
        ...
        zermes.cli._approval_deadline = _time.monotonic() + timeout
        ...
        while True:
            try:
                result = response_queue.get(timeout=1)
                zermes.cli._approval_state = None
                zermes.cli._approval_deadline = 0
                if hasattr(cli, "_app") and zermes.cli._app:
                    zermes.cli._app.invalidate()
                return result
            except queue.Empty:
                remaining = zermes.cli._approval_deadline - _time.monotonic()
                if remaining <= 0:
                    break
                if hasattr(cli, "_app") and zermes.cli._app:
                    zermes.cli._app.invalidate()
