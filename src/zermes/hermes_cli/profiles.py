...
from zermes.hermes_constants import APP_COMMAND
...
_PROFILE_DIRS = [
    ...
    # See zermes.hermes_constants.get_subprocess_home() and issue #4426.
    "home",
]
...
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    ...
    "zermes.hermes_state.db",
    "response_store.db", "response_store.db-shm", "response_store.db-wal",
    "zermes.gateway.pid", "gateway_state.json", "processes.json",
...
def _check_gateway_running(profile_dir: Path) -> bool:
    ...
    try:
        ...
        return get_running_pid(profile_dir / "zermes.gateway.pid", cleanup_stale=False) is not None
    except Exception:
        return False
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    try:
        result = subprocess.run(
            [sys.executable, "-c",
             "import json; from zermes.tools.skills_sync import sync_skills; "
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    try:
        ...
        from zermes.hermes_cli.gateway import get_service_name, get_launchd_plist_path
...
def _stop_gateway_process(profile_dir: Path) -> None:
    ...
    pid_file = profile_dir / "zermes.gateway.pid"
