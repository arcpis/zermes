...
def _prompt(question: str, *, password: bool = False, default: str = "") -> str:
    from zermes.hermes_cli.cli_output import prompt as _shared_prompt
...
def cmd_mcp_add(args):
    ...
    if not tools:
        _warning("Server connected but reported no zermes.tools.")
    ...
    _info("Start a new session to use these zermes.tools.")
...
def cmd_mcp_configure(args):
    ...
    print()
    print(color(f"  Connecting to '{name}' to discover zermes.tools...", Colors.CYAN))
    ...
    if not all_tools:
        _warning("Server reports no zermes.tools.")
        return
...
def mcp_command(args):
    ...
    if action == "serve":
        from zermes.mcp_serve import run_mcp_server
