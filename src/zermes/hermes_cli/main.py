...
def _apply_profile_override() -> None:
    ...
    # Mirrors zermes.hermes_cli.profiles._PROFILE_ID_RE so we never call
    ...
    if profile_name is None:
        try:
            from zermes.hermes_constants import get_default_hermes_root
    ...
    if profile_name is not None:
        try:
            from zermes.hermes_cli.profiles import resolve_profile_env
...
# var BEFORE hermes_logging imports zermes.agent.redact (which snapshots the flag at
# module-import time). Without this, config.yaml's toggle is ignored because
# the setup_logging() call below imports zermes.agent.redact, which reads the env var
...
# (chat, setup, gateway, config, etc.) write to zermes.agent.log + errors.log.
try:
    from zermes.hermes_logging import setup_logging as _setup_logging
...
try:
    from zermes.hermes_cli.config import load_config as _load_config_early
    from zermes.hermes_constants import apply_ipv4_preference as _apply_ipv4
...
from zermes.hermes_cli import __version__, __release_date__
from zermes.hermes_constants import AI_GATEWAY_BASE_URL, APP_PACKAGE_NAME, OPENROUTER_BASE_URL
...
def _has_any_provider_configured() -> bool:
    ...
    from zermes.hermes_cli.config import get_env_path, get_hermes_home, load_config
    from zermes.hermes_cli.auth import get_auth_status
    ...
    from zermes.hermes_cli.auth import PROVIDER_REGISTRY
    ...
    if _has_hermes_config:
        try:
            from zermes.agent.anthropic_adapter import (
...
def _normalize_tui_toolsets(toolsets: object) -> list[str]:
    ...
    try:
        from zermes.hermes_cli.oneshot import _normalize_toolsets
...
def _pin_kanban_board_env() -> None:
    ...
    if os.environ.get("HERMES_KANBAN_BOARD"):
        return
    try:
        from zermes.hermes_cli.kanban_db import get_current_board
...
def cmd_chat(args):
    ...
    if not _has_any_provider_configured():
        ...
        from zermes.hermes_cli.setup import (
    ...
    try:
        from zermes.hermes_cli.banner import prefetch_update_check
    ...
    from zermes.cli import main as cli_main
...
def cmd_gateway(args):
    ...
    from zermes.hermes_cli.gateway import gateway_command
...
def cmd_whatsapp(args):
    ...
    from zermes.hermes_cli.config import get_env_value, save_env_value
    ...
    if not current_mode:
        ...
        print("     You message yourself to talk to the zermes.agent.")
...
def cmd_setup(args):
    ...
    from zermes.hermes_cli.setup import run_setup_wizard
...
def select_provider_and_model(args=None):
    ...
    persistence.
    """
    from zermes.hermes_cli.auth import (
        resolve_provider,
        AuthError,
        format_auth_error,
    )
    from zermes.hermes_cli.config import (
    ...
    from zermes.hermes_cli.models import CANONICAL_PROVIDERS, _PROVIDER_LABELS
    ...
    def _named_custom_provider_map(cfg) -> dict[str, dict[str, str]]:
        from zermes.hermes_cli.config import read_raw_config
...
def _clear_stale_openai_base_url():
    ...
    """
    from zermes.hermes_cli.config import get_env_value, save_env_value, load_config
...
def _save_aux_choice(
    ...
    *,
    ...
    """
    from zermes.hermes_cli.config import load_config, save_config
...
def _aux_flow_provider_model(
    ...
    from zermes.hermes_cli.auth import _prompt_model_selection
    from zermes.hermes_cli.models import get_pricing_for_provider
...
def _prompt_provider_choice(choices, *, default=0):
    ...
    """
    try:
        from zermes.hermes_cli.setup import _curses_prompt_choice
...
def _model_flow_openrouter(config, current_model=""):
    ...
    from zermes.hermes_cli.models import model_ids, get_pricing_for_provider
...
def _model_flow_ai_gateway(config, current_model=""):
    ...
    from zermes.hermes_cli.models import ai_gateway_model_ids, get_pricing_for_provider
...
def _model_flow_nous(config, current_model="", args=None):
    ...
    from zermes.hermes_cli.nous_subscription import prompt_enable_tool_gateway
    ...
    from zermes.hermes_cli.models import (
    ...
    if free_tier and not model_ids:
        ...
        if unavailable_models:
            from zermes.hermes_cli.auth import DEFAULT_NOUS_PORTAL_URL
...
def _model_flow_openai_codex(config, current_model=""):
    ...
    from zermes.hermes_cli.codex_models import get_codex_model_ids
    ...
    if not _codex_token:
        try:
            from zermes.hermes_cli.auth import resolve_codex_runtime_credentials
...
def _model_flow_qwen_oauth(_config, current_model=""):
    ...
    from zermes.hermes_cli.models import fetch_api_models
...
def _model_flow_minimax_oauth(config, current_model="", args=None):
    ...
    from zermes.hermes_cli.models import _PROVIDER_MODELS
...
def _model_flow_google_gemini_cli(_config, current_model=""):
    ...
    Flow:
      ...
      2. If creds missing, run PKCE browser OAuth via zermes.agent.google_oauth.
    ...
    if not status.get("logged_in"):
        try:
            from zermes.agent.google_oauth import resolve_project_id_from_env, start_oauth_flow
...
def _model_flow_custom(config):
    ...
    """
    from zermes.hermes_cli.auth import _save_model_choice, deactivate_provider
    from zermes.hermes_cli.config import get_env_value, load_config, save_config
    ...
    from zermes.hermes_cli.models import probe_api_models
...
def _model_flow_azure_foundry(config, current_model=""):
    ...
    * Manual entry when every probe fails (private endpoints, etc.).
    ...
    :func:`zermes.agent.model_metadata.get_model_context_length` chain
    (models.dev, provider metadata, hardcoded family fallbacks).
    """
    from zermes.hermes_cli.auth import _save_model_choice, deactivate_provider  # noqa: F401
    from zermes.hermes_cli.config import (
        get_env_value,
        save_env_value,
        load_config,
    ...
    from zermes.hermes_cli import azure_detect
    import getpass
...
def _remove_custom_provider(config):
    ...
    try:
        ...
        idx = menu.show()
        from zermes.hermes_cli.curses_ui import flush_stdin
    ...
    removed = zermes.providers.pop(idx)
    cfg["custom_providers"] = providers
    save_config(cfg)
    removed_name = (
        removed.get("name", "unnamed") if isinstance(removed, dict) else str(removed)
    )
    print(f'✅ Removed "{removed_name}" from custom zermes.providers.')
...
def _model_flow_copilot(config, current_model=""):
    ...
    from zermes.hermes_cli.config import save_env_value, load_config, save_config
    from zermes.hermes_cli.models import (
        fetch_api_models,
    ...
    if not api_key:
        ...
        if choice == "1":
            try:
                from zermes.hermes_cli.copilot_auth import copilot_device_code_login
            ...
            # Validate token type
            try:
                from zermes.hermes_cli.copilot_auth import validate_copilot_token
...
def _prompt_api_key(pconfig, existing_key: str, provider_id: str = "") -> tuple:
    ...
    from zermes.hermes_cli.auth import LMSTUDIO_NOAUTH_PLACEHOLDER
...
def _model_flow_bedrock(config, current_model=""):
    ...
    try:
        from zermes.agent.bedrock_adapter import (
            has_aws_credentials,
...
def _model_flow_api_key_provider(config, provider_id, current_model=""):
    ...
    if provider_id == "gemini" and existing_key:
        try:
            from zermes.agent.gemini_native_adapter import probe_gemini_tier
        except Exception:
    ...
    if provider_id == "lmstudio":
        ...
        from zermes.hermes_cli.models import fetch_lmstudio_models
        ...
        from zermes.hermes_cli.models import fetch_ollama_cloud_models
        ...
        mdev_models: list = []
        try:
            from zermes.agent.models_dev import list_agentic_models
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            from zermes.hermes_constants import display_hermes_home as _dhh_fn
...
def _model_flow_anthropic(config, current_model=""):
    ...
    from zermes.hermes_cli.auth import get_anthropic_key
...
def cmd_login(args):
    ...
    from zermes.hermes_cli.auth import login_command
...
def cmd_logout(args):
    ...
    from zermes.hermes_cli.auth import logout_command
...
def cmd_auth(args):
    ...
    from zermes.hermes_cli.auth_commands import auth_command
...
def cmd_status(args):
    ...
    from zermes.hermes_cli.status import show_status
...
def cmd_webhook(args):
    ...
    from zermes.hermes_cli.webhook import webhook_command
...
def cmd_slack(args):
    ...
    if sub == "manifest":
        from zermes.hermes_cli.slack_cli import slack_manifest_command
...
def cmd_kanban(args):
    ...
    from zermes.hermes_cli.kanban import kanban_command
...
def cmd_hooks(args):
    ...
    from zermes.hermes_cli.hooks import hooks_command
...
def cmd_doctor(args):
    ...
    from zermes.hermes_cli.doctor import run_doctor
...
def cmd_dump(args):
    ...
    from zermes.hermes_cli.dump import run_dump
...
def cmd_debug(args):
    ...
    from zermes.hermes_cli.debug import run_debug
...
def cmd_config(args):
    ...
    from zermes.hermes_cli.config import config_command
...
def cmd_backup(args):
    ...
    if getattr(args, "quick", False):
        from zermes.hermes_cli.backup import run_quick_backup

        run_quick_backup(args)
    else:
        from zermes.hermes_cli.backup import run_backup
...
def cmd_import(args):
    ...
    from zermes.hermes_cli.backup import run_import
...
def cmd_version(args):
    ...
    try:
        from zermes.hermes_cli.banner import check_for_updates
        from zermes.hermes_cli.config import recommended_update_command
...
def cmd_uninstall(args):
    ...
    from zermes.hermes_cli.uninstall import run_uninstall
...
def _find_stale_dashboard_pids() -> list[int]:
    ...
    patterns = [
        "hermes dashboard",
        "zermes.hermes_cli.main dashboard",
    ...
    try:
        if sys.platform == "win32":
            ...
            # with `zermes.hermes_cli.gateway._scan_gateway_pids` and avoids the
...
def _print_curator_first_run_notice() -> None:
    ...
    print(
        "  Docs:         https://hermes-zermes.agent.nousresearch.com/docs/user-guide/features/curator"
    )
...
def _update_via_zip(args):
    ...
    branch = "main"
    zip_url = f"https://github.com/arcpis/zerme/archive/refs/heads/{branch}.zip"

    print("→ Downloading latest version...")
    try:
        tmp_dir = tempfile.mkdtemp(prefix="hermes-update-")
        zip_path = os.path.join(tmp_dir, f"zermes-{branch}.zip")
        ...
        extracted = os.path.join(tmp_dir, f"zerme-{branch}")
...
OFFICIAL_REPO_URLS = {
    ...
    "git@github.com:arcpis/zerme.git",
    "https://github.com/arcpis/zerme",
    "git@github.com:arcpis/zerme",
}
OFFICIAL_REPO_URL = "https://github.com/arcpis/zerme.git"
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
    ...
    if not has_upstream:
        ...
        print()
        print("ℹ Your fork is not tracking the official Zermes repository.")
        print("  This means you may miss updates from arcpis/zerme.")
        print()
        try:
            response = (
        ...
        if response in ("", "y", "yes"):
            ...
            if _add_upstream_remote(git_cmd, cwd):
                print(
                    "  ✓ Added upstream: https://github.com/arcpis/zerme.git"
                )
                has_upstream = True
            else:
            ...
            print(
                "  Skipped. Run 'git remote add upstream https://github.com/arcpis/zerme.git' to add later."
            )
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    **intentionally left alone** — those are legitimate cancellation
    ...
    try:
        ...
        # zermes.hermes_cli.config.get_hermes_home to simulate setup failure.
        from zermes.hermes_cli.config import get_hermes_home as _get_hermes_home
...
def _ensure_fhs_path_guard() -> None:
    ...
    # /usr/local/bin/hermes, code at /usr/local/lib/zermes).
...
def _run_pre_update_backup(args) -> None:
    ...
    try:
        from zermes.hermes_cli.backup import create_pre_update_backup
    except Exception as exc:
        print(
    ...
    try:
        from zermes.hermes_constants import get_hermes_home, display_hermes_home
...
def cmd_update(args):
    ...
    """
    from zermes.hermes_cli.config import is_managed, managed_error
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    try:
        ...
        try:
            from zermes.hermes_cli.backup import create_quick_snapshot
        ...
        try:
            import importlib
            import zermes.hermes_constants as _hc
        ...
        try:
            from zermes.plugins.memory.honcho.cli import sync_honcho_profiles_quiet
        ...
        try:
            from zermes.hermes_cli.gateway import (
                is_macos,
            ...
            # for up to ``zermes.agent.restart_drain_timeout`` (default 60s) before
                ...
                for scope, scope_cmd in [
                    ...
                    try:
                        ...
                        for line in result.stdout.strip().splitlines():
                            ...
                            unit = parts[
                                0
                            ]  # e.g. hermes-zermes.gateway.service or hermes-gateway-coder.service
            ...
            for pid, proc in profile_processes.items():
                ...
                # finish before the watcher respawns the zermes.gateway.  If the
            ...
            try:
                ...
                if _stuck:
                    ...
                    from zermes.gateway.status import terminate_pid as _terminate_pid
                    for pid in _stuck:
                        try:
        ...
        # current hermes-zermes.gateway.service are enabled, they SIGTERM-fight
        ...
        try:
            ...
            if supports_systemd_services() and has_legacy_hermes_units():
                ...
                print("  hermes-zermes.gateway.service for the bot token and cause SIGTERM")
...
def cmd_profile(args):
    ...
    if action == "list":
        ...
        try:
            ...
            if clone or clone_all:
                try:
                    from zermes.plugins.memory.honcho.cli import clone_honcho_for_profile
        ...
        from zermes.hermes_cli.profiles import profile_exists
        ...
        from zermes.hermes_cli.profiles import rename_profile
        ...
        from zermes.hermes_cli.profiles import export_profile
        ...
        from zermes.hermes_cli.profiles import import_profile
        ...
        import tempfile
        from zermes.hermes_cli.profile_distribution import (
            plan_install,
        ...
        from zermes.hermes_cli.profiles import get_profile_dir, normalize_profile_name
        ...
        from zermes.hermes_cli.profile_distribution import describe_distribution, DistributionError
...
def _render_distribution_plan(plan) -> None:
    ...
    from zermes.hermes_cli.profile_distribution import MANIFEST_FILENAME
    mf = plan.manifest
...
def cmd_dashboard(args):
    ...
    from zermes.hermes_cli.web_server import start_server
...
def cmd_completion(args, parser=None):
    ...
    from zermes.hermes_cli.completion import generate_bash, generate_zsh, generate_fish
...
def cmd_logs(args):
    ...
    from zermes.hermes_cli.logs import tail_log, list_logs
...
def _build_provider_choices() -> list[str]:
    ...
    try:
        from zermes.hermes_cli.models import CANONICAL_PROVIDERS as _cp
...
def main():
    ...
    # =========================================================================
    from zermes.hermes_cli.worker_agents_cmd import add_worker_agents_parser
    ...
    # =========================================================================
    from zermes.hermes_cli.fallback_cmd import cmd_fallback
    ...
    fallback_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "https://hermes-zermes.agent.nousresearch.com/docs/user-guide/features/fallback-providers"
        ),
    )
    ...
    cron_create.add_argument(
        "--workdir",
        help="Absolute path for the job to run from. Injects AGENTS.md / CLAUDE.md / .cursorrules from that directory and uses it as the cwd for terminal/file/code_exec zermes.tools. Omit to preserve old behaviour (no project context files).",
    )
    ...
    # =========================================================================
    from zermes.hermes_cli.kanban import build_parser as _build_kanban_parser
...
""",
    ...
    share_parser.add_argument(
        ...
        help=(
            ...
            "are normally run through zermes.agent.redact.redact_sensitive_text "
    ...
    from zermes.hermes_cli.checkpoints import register_cli as _register_checkpoints_cli
    ...
    def cmd_pairing(args):
        from zermes.hermes_cli.pairing import pairing_command
    ...
    def cmd_skills(args):
        ...
        if getattr(args, "skills_action", None) == "config":
            ...
            from zermes.hermes_cli.skills_config import skills_command as skills_config_command

            skills_config_command(args)
        else:
            from zermes.hermes_cli.skills_hub import skills_command
    ...
    def cmd_plugins(args):
        from zermes.hermes_cli.plugins_cmd import plugins_command
    ...
    # =========================================================================
    # Plugin CLI commands — dynamically registered by memory/general zermes.plugins.
    ...
    if _plugin_cli_discovery_needed():
        try:
            from zermes.plugins.memory import discover_plugin_cli_commands
            from zermes.hermes_cli.plugins import discover_plugins, get_plugin_manager
    ...
    try:
        from zermes.hermes_cli.curator import register_cli as _register_curator_cli
    ...
    memory_parser = subparsers.add_parser(
        ...
        description=(
            "Set up and manage external memory provider zermes.plugins.\n\n"
    ...
    def cmd_memory(args):
        ...
        if sub == "off":
            ...
            from zermes.hermes_cli.memory_setup import memory_command
    ...
    def cmd_tools(args):
        ...
        if action in ("list", "disable", "enable"):
            from zermes.hermes_cli.tools_config import tools_disable_enable_command

            tools_disable_enable_command(args)
        else:
            _require_tty("tools")
            from zermes.hermes_cli.tools_config import tools_command
    ...
    def cmd_mcp(args):
        from zermes.hermes_cli.mcp_config import mcp_command
    ...
    def _confirm_prompt(prompt: str) -> bool:
        """Prompt for y/N confirmation, safe against non-TTY zermes.environments."""
        try:
    ...
    def cmd_sessions(args):
        ...
        if action == "list":
            ...
            from zermes.hermes_cli.relaunch import relaunch
    ...
    def cmd_claw(args):
        from zermes.hermes_cli.claw import claw_command
    ...
    def cmd_acp(args):
        ...
        try:
            from zermes.acp_adapter.entry import main as acp_main
    ...
    logs_parser = subparsers.add_parser(
        ...
        description="View, tail, and filter zermes.agent.log / errors.log / zermes.gateway.log",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    hermes logs                    Show last 50 lines of zermes.agent.log
    hermes logs -f                 Follow zermes.agent.log in real time
    hermes logs errors             Show last 50 lines of errors.log
    hermes logs gateway -n 100     Show last 100 lines of zermes.gateway.log
...
""",
    ...
    from zermes.hermes_cli.config import get_container_exec_info
    ...
    if args.command in _AGENT_COMMANDS or (
        ...
        try:
            ...
            # so inline is safe.  Moved here from zermes.model_tools.py module scope
    ...
    # response only, nothing else. Bypasses zermes.cli.py entirely.
