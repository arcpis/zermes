...
TIPS = [
    ...
    "/stop kills all running background processes spawned by the zermes.agent.",
    ...
    "Ctrl+C interrupts the zermes.agent. Double-press within 2 seconds to force exit.",
    ...
    "hermes logs -f follows zermes.agent.log in real time. --level WARNING --since 1h filters output.",
    ...
    "Set zermes.agent.max_turns: 200 to let the agent take more tool-calling steps per turn.",
    ...
    "Set security.website_blocklist.enabled: true to block specific domains from web zermes.tools.",
    "Set zermes.cron.wrap_response: false to deliver raw agent output without the cron header/footer.",
    ...
    "Custom personalities can be defined in config.yaml under zermes.agent.personalities.",
    ...
    "Per-server tool filtering: zermes.tools.include whitelists and zermes.tools.exclude blacklists specific zermes.tools.",
    ...
    # --- Batch & Data ---
    "zermes.batch_runner.py processes hundreds of prompts in parallel for training data generation.",
    ...
    "hermes login supports OAuth-based auth for Nous and OpenAI Codex zermes.providers.",
    ...
    "zermes.agent.tool_use_enforcement steers models that describe actions instead of calling tools — auto for GPT/Codex.",
    "zermes.agent.restart_drain_timeout (default 60s) lets running agents finish before a gateway restart takes effect.",
    "zermes.agent.api_max_retries (default 3) controls how many times the agent retries a failed API call before surfacing the error — lower it for fast fallback.",
    ...
    "Browser pages over 8000 tokens are auto-summarized by the auxiliary LLM before returning to the zermes.agent.",
    ...
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via hermes zermes.tools.",
    ...
    'tts.zermes.providers.<name>.type: command wires any CLI TTS engine with {input_path} and {output_path} placeholders.',
    ...
    # --- Batch ---
    "zermes.batch_runner.py --resume content-matches completed prompts by text so dataset reorders don't re-run finished work.",
    ...
    'HERMES_GATEWAY_BUSY_ACK_ENABLED=false silences the ⚡/⏳/⏩ ack messages when a user messages a busy zermes.agent.',
