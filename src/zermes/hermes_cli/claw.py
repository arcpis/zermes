...
from zermes.hermes_cli.config import get_hermes_home, get_config_path, load_config, save_config
    ...
    if sys.platform != "win32":
        try:
            result = subprocess.run(
                ["systemctl", "--user", "is-active", "openclaw-zermes.gateway.service"],
                capture_output=True, text=True, timeout=5,
            )
            if result.stdout.strip() == "active":
                found.append("systemd service: openclaw-zermes.gateway.service")
...
def _cmd_migrate(args):
    ...
    # Delegates to zermes.hermes_cli.backup.create_pre_migration_backup(), which
    ...
    if not no_backup:
        try:
            from zermes.hermes_cli.backup import create_pre_migration_backup, _format_size
...
def _cmd_cleanup(args):
    ...
    if running:
        ...
        print_info("Stop OpenClaw first: systemctl --user stop openclaw-zermes.gateway.service")
        print()
        if not auto_yes:
            if not sys.stdin.isatty():
