...
Both the CLI (zermes.cli.py) and gateway (gateway/run.py) /model handlers
...
- ``zermes.agent.models_dev``            -- models.dev catalog, ModelInfo, ProviderInfo
- ``zermes.hermes_cli.providers``        -- canonical provider identity + overlays
- ``zermes.hermes_cli.model_normalize``  -- per-provider name formatting
...
from zermes.hermes_cli.providers import (
...
from zermes.agent.models_dev import (
    ModelCapabilities,
    ModelInfo,
...
def is_nous_hermes_non_agentic(model_name: str) -> bool:
    ...
    Callers in :mod:`zermes.cli.py` and here should go through this single helper
...
def _ensure_direct_aliases() -> None:
    ...
    the module attribute. This keeps `from zermes.hermes_cli.model_switch import
...
def _resolve_alias_fallback(
    ...
    """Try to resolve an alias on the user's authenticated zermes.providers.
...
def resolve_display_context_length(
    ...
    ``zermes.agent.model_metadata.get_model_context_length`` which already knows
...
def switch_model(
    ...
    """Core model-switching pipeline shared between CLI and zermes.gateway.
    ...
    # =================================================================
        ...
        # model — if so, step e must not second-guess and switch zermes.providers.
    ...
    # Mirrors the same logic in zermes.hermes_cli.runtime_provider.resolve_runtime_provider;
...
def list_authenticated_providers(
    ...
    def _record_builtin_endpoint(slug: str) -> None:
        ...
        try:
            from zermes.hermes_cli.auth import PROVIDER_REGISTRY as _reg
        except Exception:
            return
        pcfg = _reg.get(slug)
        if not pcfg:
    ...
    for hermes_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        if not has_creds:
            try:
                from zermes.hermes_cli.auth import _load_auth_store
    ...
    from zermes.hermes_cli.auth import PROVIDER_REGISTRY as _auth_registry
    ...
    for pid, overlay in HERMES_OVERLAYS.items():
        ...
        # reflects the active region (eu.*, ap.*) not the static us.* list.
            ...
            try:
                from zermes.agent.bedrock_adapter import bedrock_model_ids_or_none
    ...
    # with `hermes model`).
    try:
        from zermes.hermes_cli.models import CANONICAL_PROVIDERS as _canon_provs
    except ImportError:
        _canon_provs = []
    ...
    if custom_providers and isinstance(custom_providers, list):
        ...
        for entry in custom_providers:
            ...
            # zermes.run_agent.py, hermes_cli/config.py) already consume that dict.
...
def list_picker_providers(
    ...
    - OpenRouter's model list is replaced with the output of
      :func:`zermes.hermes_cli.models.fetch_openrouter_models`, which filters the
    ...
    """
    from zermes.hermes_cli.models import fetch_openrouter_models
