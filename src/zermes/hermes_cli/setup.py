...
from zermes.hermes_cli.nous_subscription import get_nous_subscription_features
from zermes.tools.tool_backend_helpers import managed_nous_tools_enabled
from zermes.utils import base_url_hostname
...
_DOCS_BASE = "https://hermes-zermes.agent.nousresearch.com/docs"
...
from zermes.hermes_cli.cli_output import (  # noqa: E402
    print_error,
    print_info,
    print_success,
    print_warning,
...
def _curses_prompt_choice(question: str, choices: list, default: int = 0, description: str | None = None) -> int:
    ...
    from zermes.hermes_cli.curses_ui import curses_radiolist
...
def prompt_checklist(title: str, items: list, pre_selected: list = None) -> list:
    ...
    from zermes.hermes_cli.curses_ui import curses_checklist
...
def _print_setup_summary(config: dict, hermes_home):
    ...
    try:
        from zermes.agent.auxiliary_client import get_available_vision_backends
    ...
    if subscription_features.image_gen.managed_by_nous:
        ...
        _img_backend = None
        try:
            from zermes.agent.image_gen_registry import list_providers
            from zermes.hermes_cli.plugins import _ensure_plugins_discovered
    ...
    try:
        from zermes.hermes_cli.auth import get_provider_auth_state
...
def _prompt_vercel_sandbox_settings(config: dict):
    ...
    from zermes.tools.terminal_tool import _SUPPORTED_VERCEL_RUNTIMES
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    from zermes.hermes_cli.main import select_provider_and_model
    try:
    ...
    if not quick and _supports_same_provider_pool_setup(selected_provider):
        try:
            ...
            from zermes.hermes_cli.auth_commands import auth_add_command
...
def _setup_tts_provider(config: dict):
    ...
    if managed_nous_tools_enabled() and subscription_features.nous_auth_present:
        ...
        zermes.providers.append("nous-openai")
    choices.extend(
        [
    ...
    zermes.providers.extend(["edge", "elevenlabs", "openai", "xai", "minimax", "mistral", "gemini", "neutts", "kittentts"])
...
def setup_terminal_backend(config: dict):
    ...
    if selected_backend == "local":
        ...
        from zermes.tools.managed_tool_gateway import is_managed_tool_gateway_ready
        from zermes.tools.tool_backend_helpers import normalize_modal_mode
        ...
        print_info("Persistent cloud development zermes.environments.")
...
def setup_agent_settings(config: dict):
    ...
    try:
        ...
        if max_iter > 0:
            ...
            # HERMES_MAX_ITERATIONS from zermes.agent.max_turns at startup.
...
def _setup_slack():
    ...
    print()
    print_info("   Full guide: https://hermes-zermes.agent.nousresearch.com/docs/user-guide/messaging/slack/")
    print()
...
def _write_slack_manifest_and_instruct():
    ...
    """
    try:
        from zermes.hermes_cli.slack_cli import _build_full_manifest
...
def _setup_bluebubbles():
    """Configure BlueBubbles iMessage zermes.gateway."""
...
def _setup_qqbot():
    ...
    from zermes.hermes_cli.gateway import _setup_qqbot as _gateway_setup_qqbot
...
def _setup_webhooks():
    ...
    print()
    print_info("   Full guide: https://hermes-zermes.agent.nousresearch.com/docs/user-guide/messaging/webhooks/")
    print()
    ...
    print_info("   https://hermes-zermes.agent.nousresearch.com/docs/user-guide/messaging/webhooks/#configuring-routes")
    print()
    print_info("   Open config in your editor:  hermes config edit")
    print_info("   Open config in your editor:  hermes config edit")
...
def setup_gateway(config: dict):
    ...
    from zermes.hermes_cli.gateway import _all_platforms, _platform_status, _configure_platform
...
def _model_section_has_credentials(config: dict) -> bool:
    ...
    Sources of truth:
      * ``PROVIDER_REGISTRY`` in ``zermes.hermes_cli.auth`` — lists every supported
    ...
    """
    try:
        from zermes.hermes_cli.auth import get_active_provider
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from zermes.hermes_cli.config
    ...
    if section_key == "model":
        ...
        from zermes.hermes_cli.gateway import _all_platforms, _platform_status
        ...
        if get_env_value("ELEVENLABS_API_KEY"):
            zermes.tools.append("TTS/ElevenLabs")
        if get_env_value("BROWSERBASE_API_KEY"):
            zermes.tools.append("Browser")
        if get_env_value("FIRECRAWL_API_KEY"):
            zermes.tools.append("Firecrawl")
        if tools:
            return ", ".join(tools)
        return None
...
# Gateway tokens/channels can hijack messaging platforms from the old zermes.agent.
