...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("clear", "Clear screen and start a new session", "Session",
               cli_only=True),
    CommandDef("delete-context", "Delete chat context and DB messages without starting a new session", "Session",
               cli_only=True),
...
# Set of all command names + aliases recognized by the zermes.gateway.
...
def should_bypass_active_session(command_name: str | None) -> bool:
    ...
    safety net in zermes.gateway.run discards any command text that reaches
...
def _iter_plugin_command_entries() -> list[tuple[str, str, str]]:
    ...
    :func:`zermes.hermes_cli.plugins.PluginContext.register_command`. They behave
    like ``CommandDef`` entries for gateway surfacing: they appear in the
    Telegram command menu, in Slack's ``/hermes`` subcommand mapping, and
    (via :func:`zermes.gateway.platforms.discord._register_slash_commands`) in
...
def _collect_gateway_skill_entries(
    ...
    try:
        ...
        from zermes.tools.skills_tool import SKILLS_DIR
...
def slack_app_manifest(request_url: str = "https://hermes-zermes.agent.local/slack/commands") -> dict[str, Any]:
