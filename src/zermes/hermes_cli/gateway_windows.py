...
* We write two files: a shared ``zermes.gateway.cmd`` wrapper script (cwd + env + the
  actual ``python -m zermes.hermes_cli.main gateway run --replace`` invocation) and
...
* Quoting is tricky: schtasks parses ``/TR`` itself and cmd.exe parses the
  generated ``zermes.gateway.cmd``. Those are DIFFERENT parsers. We keep two
...
def get_task_name() -> str:
    ...
    from zermes.hermes_cli.gateway import _profile_suffix
...
def get_task_script_path() -> Path:
    """The generated ``zermes.gateway.cmd`` wrapper that the schtasks entry invokes.
...
def _build_gateway_cmd_script(
    ...
    """Build the ``zermes.gateway.cmd`` wrapper content (CRLF-terminated).

    The script:
      - cd's into the project directory
      - exports HERMES_HOME, PYTHONIOENCODING, VIRTUAL_ENV
      - invokes ``python -m zermes.hermes_cli.main [--profile X] gateway run --replace``
    ...
    prog_args = [python_path, "-m", "zermes.hermes_cli.main"]
    if profile_arg:
...
def _write_task_script() -> Path:
    """Generate and write the zermes.gateway.cmd wrapper. Return its absolute path."""
    _assert_windows()
...
def _build_gateway_argv() -> tuple[list[str], str, dict[str, str]]:
    ...
    Same logical command as what zermes.gateway.cmd runs, but assembled as a
    ...
    argv = [python_exe, "-m", "zermes.hermes_cli.main"]
    if profile_arg:
...
def _spawn_detached(script_path: Path | None = None) -> int:
    ...
    We spawn ``pythonw.exe -m zermes.hermes_cli.main gateway run --replace``
    ...
    # logging module writes to zermes.gateway.log through a FileHandler, so the
...
def _wait_for_gateway_ready(timeout_s: float = 6.0, interval_s: float = 0.4) -> list[int]:
    ...
    """
    from zermes.hermes_cli.gateway import find_gateway_pids
...
def _report_gateway_start(via: str) -> None:
    ...
    if pids:
        ...
        from zermes.hermes_cli.config import get_hermes_home
        print(f"    type {Path(get_hermes_home()).resolve()}\\logs\\zermes.gateway.log")
...
def _print_next_steps() -> None:
    ...
    print(f"  type {hermes_home}\\logs\\zermes.gateway.log       # View logs")
...
def _gateway_pids() -> list[int]:
    """Reuse the cross-platform PID scanner in zermes.gateway.py."""
    from zermes.hermes_cli.gateway import find_gateway_pids
...
def start() -> None:
    """Start the zermes.gateway. Prefers /Run on the scheduled task if present."""
    _assert_windows()
    if is_task_registered():
...
def stop() -> None:
    """Stop the zermes.gateway. Tries /End on the scheduled task, then kills any stragglers."""
    _assert_windows()
    from zermes.hermes_cli.gateway import kill_gateway_processes
