...
def _cmd_rollback(args) -> int:
    ...
    if manifest:
        ...
        if isinstance(cron, dict):
            if zermes.cron.get("backed_up"):
                print(
                    f"  cron jobs:   {zermes.cron.get('jobs_count', 0)} "
                    f"(will be restored for skill-link fields only)"
                )
            else:
                reason = zermes.cron.get("reason", "not captured")
...
# ---------------------------------------------------------------------------
# argparse wiring (called from zermes.hermes_cli.main)
# ---------------------------------------------------------------------------
...
def cli_main(argv=None) -> int:
    """Standalone entry (also usable by zermes.hermes_cli.main fallthrough)."""
