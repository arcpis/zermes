...
Usage:
    python -m zermes.hermes_cli.main web          # Start on http://127.0.0.1:9119
    python -m zermes.hermes_cli.main web --port 8080
"""
...
from zermes.gateway.status import get_running_pid, read_runtime_status
...
try:
    from zermes.hermes_cli.worker_agents_api import router as _worker_agents_router
...
_SCHEMA_OVERRIDES: Dict[str, Dict[str, Any]] = {
    ...
    "logging.level": {
        "type": "select",
        "description": "Log level for zermes.agent.log",
        "options": ["DEBUG", "INFO", "WARNING", "ERROR"],
    },
    "zermes.agent.service_tier": {
        "type": "select",
...
async def get_status():
    ...
    try:
        from zermes.gateway.config import load_gateway_config
...
def _spawn_hermes_action(subcommand: List[str], name: str) -> subprocess.Popen:
    ...
    Uses the running interpreter's ``zermes.hermes_cli.main`` module so the action
    ...
    cmd = [sys.executable, "-m", "zermes.hermes_cli.main", *subcommand]
...
def _claude_code_only_status() -> Dict[str, Any]:
    ...
    """
    try:
        from zermes.agent.anthropic_adapter import read_claude_code_credentials
...
def _resolve_provider_status(provider_id: str, status_fn) -> Dict[str, Any]:
    ...
    try:
        from zermes.hermes_cli import auth as hauth
        if provider_id == "nous":
...
async def disconnect_oauth_provider(provider_id: str, request: Request):
    ...
    if provider_id in ("anthropic", "claude-code"):
        try:
            from zermes.agent.anthropic_adapter import _HERMES_OAUTH_FILE
        ...
        try:
            from zermes.hermes_cli.auth import clear_provider_auth
...
def _save_anthropic_oauth_creds(access_token: str, refresh_token: str, expires_at_ms: int) -> None:
    ...
    try:
        from zermes.agent.credential_pool import (
            PooledCredential,
            load_pool,
            AUTH_TYPE_OAUTH,
            SOURCE_MANUAL,
...
async def _start_device_code_flow(provider_id: str) -> Dict[str, Any]:
    ...
    """
    if provider_id == "nous":
        from zermes.hermes_cli.auth import _request_device_code, PROVIDER_REGISTRY
        import httpx
...
def _nous_poller(session_id: str) -> None:
    ...
    from zermes.hermes_cli.auth import _poll_for_token, refresh_nous_oauth_from_state
    ...
    try:
        ...
        from zermes.hermes_cli.auth import persist_nous_credentials
...
async def get_logs(
    ...
    from zermes.hermes_cli.logs import _read_tail, LOG_FILES
    ...
    try:
        from zermes.hermes_logging import COMPONENT_PREFIXES
    except ImportError:
        COMPONENT_PREFIXES = {}
...
async def list_cron_jobs():
    from zermes.cron.jobs import list_jobs
...
async def update_cron_job(job_id: str, body: CronJobUpdate):
    from zermes.cron.jobs import update_job
...
async def pause_cron_job(job_id: str):
    from zermes.cron.jobs import pause_job
...
async def resume_cron_job(job_id: str):
    from zermes.cron.jobs import resume_job
...
async def trigger_cron_job(job_id: str):
    from zermes.cron.jobs import trigger_job
...
async def delete_cron_job(job_id: str):
    from zermes.cron.jobs import remove_job
...
def _resolve_profile_dir(name: str) -> Path:
    ...
    from zermes.hermes_cli import profiles as profiles_mod
    try:
...
async def toggle_skill(body: SkillToggle):
    from zermes.hermes_cli.skills_config import get_disabled_skills, save_disabled_skills
    config = load_config()
    disabled = get_disabled_skills(config)
    if body.enabled:
...
async def get_toolsets():
    ...
    from zermes.toolsets import resolve_toolset
...
try:
    from zermes.hermes_cli.pty_bridge import PtyBridge, PtyUnavailableError
...
def _resolve_chat_argv(
    ...
    matching what ``zermes.hermes_cli.main._launch_tui`` does for the CLI path.
    Appending ``--resume <id>`` to argv doesn't work because ``ui-tui`` does
    not parse its argv.

    `sidecar_url` (when set) is forwarded as ``HERMES_TUI_SIDECAR_URL`` so
    the spawned ``zermes.tui_gateway.entry`` can mirror dispatcher emits to the
    dashboard's ``/api/pub`` endpoint (see :func:`pub_ws`).
    """
    from zermes.hermes_cli.main import PROJECT_ROOT, _make_tui_argv
...
#
# Drives the same `zermes.tui_gateway.dispatch` surface Ink uses over stdio, so the
...
async def gateway_ws(ws: WebSocket) -> None:
    ...
    from zermes.tui_gateway.ws import handle_ws
...
#
# The PTY-side ``zermes.tui_gateway.entry`` opens /api/pub at startup (driven by
...
_BUILTIN_DASHBOARD_THEMES = [
    {"name": "default",       "label": "Zermes Teal",         "description": "Classic dark teal — the canonical Zermes look"},
    {"name": "default-large", "label": "Zermes Teal (Large)", "description": "Zermes Teal with bigger fonts and roomier spacing"},
...
def _discover_dashboard_plugins() -> list:
    ...
    Checks three plugin sources (same as zermes.hermes_cli.plugins):
    ...
    from zermes.hermes_cli.plugins import get_bundled_plugins_dir
...
async def rescan_dashboard_plugins():
    """Force re-scan of dashboard zermes.plugins."""
...
def _merged_plugins_hub() -> Dict[str, Any]:
    ...
    from zermes.hermes_cli.plugins_cmd import (
...
async def post_agent_plugin_install(request: Request, body: _AgentPluginInstallBody):
    _require_token(request)
    from zermes.hermes_cli.plugins_cmd import dashboard_install_plugin
...
async def post_agent_plugin_enable(request: Request, name: str):
    ...
    from zermes.hermes_cli.plugins_cmd import dashboard_set_agent_plugin_enabled
...
async def post_agent_plugin_update(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from zermes.hermes_cli.plugins_cmd import dashboard_update_user_plugin
...
async def delete_agent_plugin(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from zermes.hermes_cli.plugins_cmd import dashboard_remove_user_plugin
...
def start_server(
    ...
    *,
    ...
    print(f"  Zermes Web UI → http://{host}:{port}")
