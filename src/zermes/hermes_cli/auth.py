...
from zermes.hermes_cli.config import get_hermes_home, get_config_path, read_raw_config
from zermes.hermes_constants import OPENROUTER_BASE_URL
from zermes.utils import atomic_replace, atomic_yaml_write, is_truthy_value
...
SPOTIFY_DOCS_URL = "https://hermes-zermes.agent.nousresearch.com/docs/user-guide/features/spotify"
...
PROVIDER_REGISTRY: Dict[str, ProviderConfig] = {
    ...
    "ai-gateway": ProviderConfig(
        ...
        auth_type="api_key",
        inference_base_url="https://ai-zermes.gateway.vercel.sh/v1",
...
try:
    from zermes.providers import list_providers as _list_providers_for_registry
...
def get_anthropic_key() -> str:
    ...
    """
    from zermes.hermes_cli.config import get_env_value
...
def _resolve_api_key_provider_secret(
    ...
    if provider_id == "copilot":
        ...
        try:
            from zermes.hermes_cli.copilot_auth import resolve_copilot_token, get_copilot_api_token
...
def _load_provider_state(auth_store: Dict[str, Any], provider_id: str) -> Optional[Dict[str, Any]]:
    ...
    if not isinstance(providers, dict):
        return None
    state = zermes.providers.get(provider_id)
...
def _get_config_hint_for_unknown_provider(provider_name: str) -> str:
    ...
    """
    try:
        from zermes.hermes_cli.config import validate_config_structure
...
def resolve_provider(
    ...
    *,
    ...
    try:
        from zermes.providers import list_providers as _lp
        for _pp in _lp():
            for _alias in _pp.aliases:
    ...
    try:
        from zermes.agent.bedrock_adapter import has_aws_credentials
...
#
# Tokens live in ~/.hermes/auth/google_oauth.json (managed by zermes.agent.google_oauth).
# The `base_url` here is the marker "cloudcode-pa://google" that zermes.run_agent.py
...
def resolve_gemini_oauth_runtime_credentials(
    *,
    ...
    """Resolve runtime OAuth creds for google-gemini-zermes.cli."""
    try:
        from zermes.agent.google_oauth import (
            GoogleOAuthError,
            _credentials_path,
        ...
        raise AuthError(
            f"zermes.agent.google_oauth is not importable: {exc}",
...
def get_gemini_oauth_auth_status() -> Dict[str, Any]:
    ...
    try:
        from zermes.agent.google_oauth import _credentials_path, load_credentials
    except ImportError:
        return {"logged_in": False, "error": "zermes.agent.google_oauth unavailable"}
...
def login_spotify_command(args) -> None:
    ...
    print("  Provider state saved under zermes.providers.spotify")
...
def _nous_shared_auth_dir() -> Path:
    ...
    :func:`zermes.hermes_constants.get_default_hermes_root` returns — so
...
def persist_nous_credentials(
    ...
    *,
    ...
    - ``zermes.providers.nous``: singleton state read by
    ...
    entry only, skipping ``zermes.providers.nous``.  When the 24h agent_key TTL
    expired, the recovery path read the empty singleton state and raised
    ``AuthError`` silently (``logger.debug`` at INFO level).

    This helper writes ``zermes.providers.nous`` then calls ``load_pool("nous")`` so
...
def resolve_external_process_provider_credentials(provider_id: str) -> Dict[str, Any]:
    """Resolve runtime details for local subprocess-backed zermes.providers."""
    pconfig = PROVIDER_REGISTRY.get(provider_id)
    if not pconfig or pconfig.auth_type != "external_process":
        raise AuthError(
...
def _update_config_for_provider(
    ...
    # "anthropic/claude-opus-4.6" will fail on direct-API zermes.providers.
    if default_model:
...
def _prompt_model_selection(
    ...
    """
    from zermes.hermes_cli.models import _format_price_per_mtok
...
def _save_model_choice(model_id: str) -> None:
    ...
    """
    from zermes.hermes_cli.config import save_config, load_config
