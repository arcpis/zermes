...
from zermes.gateway.status import terminate_pid
...
from zermes.hermes_cli.colors import Colors, color
...
def _graceful_restart_via_sigusr1(pid: int, drain_timeout: float) -> bool:
    ...
    which drains in-flight agent runs (up to ``zermes.agent.restart_drain_timeout``
    ...
    Args:
        ...
        drain_timeout: Seconds to wait for the process to exit after sending
            ...
            ``zermes.agent.restart_drain_timeout`` to allow the drain loop to
            finish cleanly.
    ...
    # ``_pid_exists`` helper in zermes.gateway.status which does OpenProcess +
...
def _scan_gateway_pids(exclude_pids: set[int], all_profiles: bool = False) -> list[int]:
    ...
    # zermes.gateway.  See #13242.
    exclude_pids = exclude_pids | _get_ancestor_pids()
    pids: list[int] = []
    patterns = [
        "zermes.hermes_cli.main gateway",
        "zermes.hermes_cli.main --profile",
        "zermes.hermes_cli.main -p",
...
def find_gateway_pids(exclude_pids: set | None = None, all_profiles: bool = False) -> list:
    ...
    if not all_profiles:
        try:
            from zermes.gateway.status import get_running_pid
...
def find_profile_gateway_processes(
    ...
    try:
        from zermes.gateway.status import get_running_pid
        from zermes.hermes_cli.profiles import list_profiles
    except Exception:
        return processes
    ...
    for profile in list_profiles():
        try:
            pid = get_running_pid(profile.path / "zermes.gateway.pid", cleanup_stale=False)
        except Exception:
            continue
...
def _gateway_run_args_for_profile(profile: str) -> list[str]:
    args = [get_python_path(), "-m", "zermes.hermes_cli.main"]
    if profile != "default":
...
def launch_detached_profile_gateway_restart(profile: str, old_pid: int) -> bool:
    ...
    watcher = textwrap.dedent(
        ...
        # Platform-appropriate detach for the respawned zermes.gateway.  On POSIX
...
def _read_gateway_runtime_status() -> dict | None:
    try:
        from zermes.gateway.status import read_runtime_status
...
def get_gateway_runtime_snapshot(system: bool = False) -> GatewayRuntimeSnapshot:
    ...
    from zermes.hermes_constants import is_container
...
def _gateway_list() -> None:
    ...
    """
    try:
        from zermes.hermes_cli.profiles import list_profiles, get_active_profile_name
    except Exception:
    ...
    for prof in profiles:
        ...
        if prof.gateway_running:
            try:
                from zermes.gateway.status import get_running_pid
                pid = get_running_pid(prof.path / "zermes.gateway.pid", cleanup_stale=False)
                if pid:
...
def stop_profile_gateway() -> bool:
    ...
    """
    try:
        from zermes.gateway.status import get_running_pid, remove_pid_file
    except ImportError:
        return False
    ...
    try:
        from zermes.gateway.status import write_planned_stop_marker
...
from zermes.hermes_constants import is_container, is_termux, is_wsl
...
# ExecStart content markers that identify a unit as running our zermes.gateway.
...
def _find_legacy_hermes_units() -> list[tuple[str, Path, bool]]:
    ...
    ``hermes-zermes.gateway.service``). When both a legacy unit and the current
    ``hermes-zermes.gateway.service`` are active, they fight over the same bot
...
def get_launchd_plist_path() -> Path:
    ...
    Default ``~/.hermes`` → ``ai.hermes.zermes.gateway.plist`` (backward compatible).
...
User={username}
Group={group_name}
ExecStart={python_path} -m zermes.hermes_cli.main{f" {profile_arg}" if profile_arg else ""} gateway run --replace
...
def systemd_install(force: bool = False, system: bool = False, run_as_user: str | None = None):
    ...
    # before installing the new hermes-zermes.gateway.service. If both remain, they
...
def systemd_stop(system: bool = False):
    ...
    try:
        from zermes.gateway.status import get_running_pid, write_planned_stop_marker
...
def generate_launchd_plist() -> str:
    ...
    prog_args = [
        ...
        "<string>-m</string>",
        "<string>zermes.hermes_cli.main</string>",
    ]
    if profile_arg:
...
<dict>
    ...
    <string>{log_dir}/zermes.gateway.log</string>
    
    <key>StandardErrorPath</key>
    <string>{log_dir}/zermes.gateway.error.log</string>
</dict>
</plist>
"""
...
def launchd_install(force: bool = False):
    ...
    from zermes.hermes_constants import display_hermes_home as _dhh
    print(f"  tail -f {_dhh()}/logs/zermes.gateway.log  # View logs")
...
def _wait_for_gateway_exit(timeout: float = 10.0, force_after: float | None = 5.0) -> bool:
    ...
    Uses the PID from the zermes.gateway.pid file — not launchd labels — so this
...
def launchd_status(deep: bool = False):
    ...
    if deep:
        log_file = get_hermes_home() / "logs" / "zermes.gateway.log"
        if log_file.exists():
            print()
            print("Recent logs:")
...
def run_gateway(verbose: int = 0, quiet: bool = False, replace: bool = False):
    ...
    # catches that and exits cleanly — silently killing the zermes.gateway. On
    ...
    # traceback in zermes.gateway.log / errors.log), we're usually blind to the
    ...
    def _exit_diag(tag: str, **extra: object) -> None:
        if os.environ.get("HERMES_GATEWAY_EXIT_DIAG", "1") != "1":
            return
        try:
            from zermes.hermes_constants import get_hermes_home as _ghh
            log_dir = _ghh() / "logs"
            log_dir.mkdir(parents=True, exist_ok=True)
    ...
    try:
        ...
        raise
    if not success:
        _exit_diag("zermes.gateway.exit_nonzero")
        sys.exit(1)
    _exit_diag("zermes.gateway.exit_clean")
...
def _all_platforms() -> list[dict]:
    ...
    # opt-in via ``zermes.plugins.enabled`` (untrusted code).
    try:
...
def _setup_whatsapp():
    ...
    from zermes.hermes_cli.main import cmd_whatsapp
    import argparse
...
def _setup_dingtalk():
    ...
    if method == 0:
        ...
        try:
            from zermes.hermes_cli.dingtalk_auth import dingtalk_qr_auth
...
def _setup_wecom():
    ...
    if method_idx == 0:
        # ── QR scan flow ──
        try:
            from zermes.gateway.platforms.wecom import qr_scan_for_bot_info
        except Exception as exc:
...
def _is_service_installed() -> bool:
    ...
    if supports_systemd_services():
        ...
        from zermes.hermes_cli import gateway_windows
...
def _setup_weixin():
    ...
    try:
        from zermes.gateway.platforms.weixin import check_weixin_requirements, qr_login
    except Exception as exc:
...
def _setup_feishu():
    ...
    if method_idx == 0:
        # ── QR scan-to-create ──
        try:
            from zermes.gateway.platforms.feishu import qr_register
        except Exception as exc:
    ...
    if not credentials:
        ...
        bot_name = None
        try:
            from zermes.gateway.platforms.feishu import probe_bot
...
def _setup_qqbot():
    ...
    if method_idx == 0:
        ...
        try:
            from zermes.gateway.platforms.qqbot import qr_register
            credentials = qr_register()
        except KeyboardInterrupt:
            print()
...
def _builtin_setup_fn(key: str):
    ...
    Late-bound to avoid a circular import with ``zermes.hermes_cli.setup`` (which
    imports from this module for the remaining bespoke flows).
    """
    from zermes.hermes_cli import setup as _s
    return {
...
def _configure_platform(platform: dict) -> None:
    ...
    must already be in ``zermes.plugins.enabled`` before they appear in this menu.
    """
    entry = platform.get("_registry_entry")
    ...
    if required:
        ...
        print_info(f"  Configure {label} in config.yaml under zermes.gateway.platforms.{platform['key']}")
        ...
        if service_running:
            ...
            if supports_systemd_services() or is_macos() or is_windows():
                ...
                if is_termux():
                    ...
                    print_info("  Run in foreground: hermes gateway run")
                    print_info(f"  Or start it manually in the background (best effort): nohup hermes gateway run >{_dhh()}/logs/zermes.gateway.log 2>&1 &")
                else:
...
def _gateway_command_inner(args):
    ...
    if subcmd == "install":
        ...
        if supports_systemd_services():
            ...
            print("  nohup hermes gateway run > ~/.hermes/logs/zermes.gateway.log 2>&1 &  # background")
            sys.exit(1)
        elif is_container():
        ...
        if supports_systemd_services() and (get_systemd_unit_path(system=False).exists() or get_systemd_unit_path(system=True).exists()):
            ...
            if pids:
                ...
                if is_termux():
                    print("  nohup hermes gateway run > ~/.hermes/logs/zermes.gateway.log 2>&1 &  # Best-effort background start")
                elif is_wsl():
                    print("  tmux new -s hermes 'hermes gateway run'         # persistent via tmux")
                    print("  nohup hermes gateway run > ~/.hermes/logs/zermes.gateway.log 2>&1 &  # background")
                elif is_windows():
