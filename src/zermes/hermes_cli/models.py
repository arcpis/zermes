...
def _codex_curated_models() -> list[str]:
    ...
    """
    from zermes.hermes_cli.codex_models import DEFAULT_CODEX_MODELS, _add_forward_compat_models
...
def _xai_curated_models() -> list[str]:
    ...
    """
    try:
        from zermes.agent.models_dev import _load_disk_cache
...
def check_nous_free_tier() -> bool:
    ...
    try:
        from zermes.hermes_cli.auth import get_provider_auth_state, resolve_nous_runtime_credentials
...
try:
    from zermes.providers import list_providers as _list_providers_for_canonical
...
def _openrouter_model_supports_tools(item: Any) -> bool:
    ...
    can invoke zermes.tools. Models that don't advertise ``tools`` in their
...
def fetch_openrouter_models(
    ...
    *,
    ...
    try:
        from zermes.hermes_cli.model_catalog import get_curated_openrouter_models
...
def get_curated_nous_model_ids() -> list[str]:
    ...
    """
    try:
        from zermes.hermes_cli.model_catalog import get_curated_nous_models
...
def fetch_ai_gateway_models(
    ...
    *,
    ...
    from zermes.hermes_constants import AI_GATEWAY_BASE_URL
...
def list_available_providers() -> list[dict[str, str]]:
    ...
    for pid in provider_order:
        ...
        has_creds = False
        try:
            from zermes.hermes_cli.auth import get_auth_status, has_usable_secret
            if pid == "custom":
...
def parse_model_input(raw: str, current_provider: str) -> tuple[str, str]:
    ...
    if colon > 0:
        ...
        if provider_part and model_part and provider_part in _KNOWN_PROVIDER_NAMES:
            ...
            # zermes.providers.  ``custom:local:qwen`` → ("custom:local", "qwen").
...
def _resolve_static_model_alias(
    ...
    try:
        from zermes.hermes_cli.model_switch import MODEL_ALIASES
    except Exception:
        return None
...
def normalize_provider(provider: Optional[str]) -> str:
    ...
    ``zermes.hermes_cli.auth.resolve_provider()`` to resolve it to a concrete
...
def resolve_fast_mode_overrides(model_id: Optional[str]) -> dict[str, Any] | None:
    ...
    ``_build_api_kwargs`` in zermes.run_agent.py — each API path handles its own
...
def _resolve_copilot_catalog_api_key() -> str:
    ...
    """
    try:
        from zermes.hermes_cli.auth import resolve_api_key_provider_credentials
    ...
    try:
        from zermes.hermes_cli.auth import read_credential_pool
        from zermes.hermes_cli.copilot_auth import (
...
def provider_model_ids(provider: Optional[str], *, force_refresh: bool = False) -> list[str]:
    ...
    if normalized == "nous":
        ...
        try:
            from zermes.hermes_cli.auth import fetch_nous_models, resolve_nous_runtime_credentials
            creds = resolve_nous_runtime_credentials()
            if creds:
...
def _fetch_anthropic_models(timeout: float = 5.0) -> Optional[list[str]]:
    ...
    """
    try:
        from zermes.agent.anthropic_adapter import resolve_anthropic_token, _is_oauth_token
    except ImportError:
        return None
    ...
    if is_oauth:
        ...
        from zermes.agent.anthropic_adapter import _COMMON_BETAS, _OAUTH_ONLY_BETAS, _CONTEXT_1M_BETA
...
def validate_requested_model(
    ...
    *,
    ...
    if normalized == "bedrock":
        try:
            from zermes.agent.bedrock_adapter import discover_bedrock_models, resolve_bedrock_region
