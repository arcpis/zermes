"""``hermes plugins`` CLI subcommand — install, update, remove, and list zermes.plugins.
...
def _plugins_dir() -> Path:
    ...
    zermes.plugins.mkdir(parents=True, exist_ok=True)
    return plugins
...
def _prompt_plugin_env_vars(manifest: dict, console) -> None:
    ...
    from zermes.hermes_cli.config import get_env_value, save_env_value  # noqa: F811
...
def _require_installed_plugin(name: str, plugins_dir: Path, console) -> Path:
    """Return the plugin path if it exists, or exit with an error listing installed zermes.plugins."""
...
def _get_disabled_set() -> set:
    ...
    listed in ``zermes.plugins.enabled``.
    """
    try:
...
def _discover_memory_providers() -> list[tuple[str, str]]:
    """Return [(name, description), ...] for available memory zermes.providers."""
    try:
...
def _discover_context_engines() -> list[tuple[str, str]]:
    ...
    try:
        from zermes.plugins.context_engine import discover_context_engines
...
def _configure_memory_provider() -> bool:
    """Launch a radio picker for memory zermes.providers. Returns True if changed."""
