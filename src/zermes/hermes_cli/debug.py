...
Currently supports:
    hermes debug share    Upload debug report (system info + logs) to a
                          ...
                          ``zermes.agent.redact.redact_sensitive_text`` with
...
def _pending_file() -> Path:
    ...
    who never start the zermes.gateway.
    """
...
⚠️  This will upload the following to a public paste service:
  ...
  • Recent log lines (zermes.agent.log, errors.log, zermes.gateway.log — may contain
    conversation fragments and file paths)
  • Full zermes.agent.log and zermes.gateway.log (up to 512 KB each — likely contains
...
def _primary_log_path(log_name: str) -> Optional[Path]:
    ...
    from zermes.hermes_cli.logs import LOG_FILES
...
def _capture_log_snapshot(
    ...
    *,
    ...
    look newer than the uploaded ``zermes.agent.log`` paste.
...
def collect_debug_report(
    *,
    ...
    buf.write("\n\n")
    buf.write(f"--- zermes.agent.log (last {log_lines} lines) ---\n")
    ...
    buf.write(f"--- zermes.gateway.log (last {errors_lines} lines) ---\n")
...
def run_debug_share(args):
    ...
    if agent_log:
        agent_log = dump_text + "\n\n--- full zermes.agent.log ---\n" + agent_log
    if gateway_log:
        gateway_log = dump_text + "\n\n--- full zermes.gateway.log ---\n" + gateway_log
    ...
    if local_only:
        ...
        if agent_log:
            print(f"\n\n{'=' * 60}")
            print("FULL zermes.agent.log")
            print(f"{'=' * 60}\n")
            print(agent_log)
        if gateway_log:
            print(f"\n\n{'=' * 60}")
            print("FULL zermes.gateway.log")
            print(f"{'=' * 60}\n")
            print(gateway_log)
        return
    ...
    # 2. Full zermes.agent.log (optional)
    if agent_log:
        try:
            urls["zermes.agent.log"] = upload_to_pastebin(agent_log, expiry_days=expiry)
        except Exception as exc:
            failures.append(f"zermes.agent.log: {exc}")

    # 3. Full zermes.gateway.log (optional)
    if gateway_log:
        try:
            urls["zermes.gateway.log"] = upload_to_pastebin(gateway_log, expiry_days=expiry)
        except Exception as exc:
            failures.append(f"zermes.gateway.log: {exc}")
