"""Process-wide voice recording + TTS API for the TUI zermes.gateway.

Wraps ``zermes.tools.voice_mode`` (recording/transcription) and ``zermes.tools.tts_tool``
...
# (``_register_voice_handler`` in zermes.cli.py) logs a warning when that
...
from zermes.tools.voice_mode import (
...
def _debug(msg: str) -> None:
    ...
    Goes to stderr so the TUI gateway wraps it as a zermes.gateway.stderr event,
...
def _play_beep(frequency: int, count: int = 1) -> None:
    """Audible cue matching zermes.cli.py's record/stop beeps.

    880 Hz single-beep on start (zermes.cli.py:_voice_start_recording line 7532),
    660 Hz double-beep on stop (zermes.cli.py:_voice_stop_and_transcribe line 7585).
...
# This Event mirrors zermes.cli.py:_voice_tts_done: cleared while speak_text is
...
def stop_and_transcribe() -> Optional[str]:
    ...
    # — matches zermes.cli.py:_voice_stop_and_transcribe's result.get("transcript").
...
def _continuous_on_silence() -> None:
    ...
    # CLI parity (zermes.cli.py:10619-10621): wait for any in-flight TTS to
...
def speak_text(text: str) -> None:
    ...
    Mirrors zermes.cli.py:_voice_speak_response exactly — same markdown strip
