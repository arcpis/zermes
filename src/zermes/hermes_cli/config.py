...
_EXTRA_ENV_KEYS = frozenset({
    ...
    # Activation is via zermes.plugins.enabled (opt-in through `hermes plugins enable
...
from zermes.hermes_cli.default_soul import DEFAULT_SOUL_MD
...
def format_managed_message(action: str = "modify this Hermes installation") -> str:
    ...
    if managed_system == "NixOS":
        ...
        return (
            ...
            "Edit services.hermes-zermes.agent.settings in your configuration.nix and run:\n"
...
from zermes.hermes_constants import get_hermes_home  # noqa: F811,E402
from zermes.utils import atomic_replace
...
DEFAULT_CONFIG = {
    ...
    "agent": {
        ...
        # once per run and does not interrupt the zermes.agent.  0 = disable warning.
        ...
        #              (see zermes.run_agent._prepare_messages_for_api).
    ...
    "terminal": {
        ...
        # Example: {"SSH_AUTH_SOCK": "/run/user/1000/ssh-zermes.agent.sock"}
        "docker_env": {},
    ...
    "display": {
        ...
        # replies wrapped with zermes.gateway.platforms.base.EphemeralReply — agent
    ...
    # zermes.agent.log captures INFO+ (all agent activity); errors.log captures WARNING+.
    "logging": {
        "level": "INFO",       # Minimum level for zermes.agent.log: DEBUG, INFO, WARNING
    ...
    "model_catalog": {
        "enabled": True,
        "url": "https://hermes-zermes.agent.nousresearch.com/docs/api/model-catalog.json",
...
OPTIONAL_ENV_VARS = {
    ...
    "TOOL_GATEWAY_DOMAIN": {
        "description": "Shared tool-gateway domain suffix for Nous Subscribers only, used to derive vendor hosts, e.g. nousresearch.com -> firecrawl-zermes.gateway.nousresearch.com",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote Hermes API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host zermes.agent. Also configurable via zermes.gateway.proxy_url in config.yaml.",
...
def _set_nested(config, dotted_key: str, value):
    ...
    Supports both dict and list navigation:
      ...
      _set_nested(c, "zermes.providers.1", "x") → c["providers"][1] = "x"
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    """
    try:
        from zermes.agent.skill_utils import discover_all_skill_config_vars, SKILL_CONFIG_PREFIX
    except Exception:
        return []
    ...
    *,
    ...
    for camel, snake in _CAMEL_ALIASES.items():
        if camel in entry and snake not in entry:
            logger.warning(
                "zermes.providers.%s: camelCase key '%s' auto-mapped to '%s' "
    ...
    if unknown:
        logger.warning(
            "zermes.providers.%s: unknown config keys ignored: %s",
    ...
    for url_key in ("base_url", "url", "api"):
        ...
        if isinstance(raw_url, str) and raw_url.strip():
            ...
            if parsed.scheme and parsed.netloc:
                ...
                logger.warning(
                    "zermes.providers.%s: '%s' value '%s' is not a valid URL "
...
def get_custom_provider_context_length(
    ...
    used by:
      ...
      * ``zermes.hermes_cli.model_switch.resolve_display_context_length`` (``/model`` confirmation display)
      * ``zermes.gateway.run._format_session_info`` (``/info`` display)
      * ``zermes.agent.model_metadata.get_model_context_length`` (when custom_providers is threaded through)

    Before this helper existed, the lookup was duplicated in ``zermes.run_agent.py``'s
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    # The loader now requires plugins to appear in ``zermes.plugins.enabled`` before
    # loading. Existing installs had all discovered plugins loading by default
    # (minus anything in ``zermes.plugins.disabled``). To avoid silently breaking
    # those setups on upgrade, populate ``zermes.plugins.enabled`` with the set of
    ...
    if current_ver < 21:
        ...
        if "enabled" not in plugins_cfg:
            ...
            # Scan ``$HERMES_HOME/plugins/`` for currently installed user zermes.plugins.
            ...
            results["config_added"].append(
                f"zermes.plugins.enabled (opt-in allow-list, {len(grandfathered)} grandfathered)"
            )
            if not quiet:
                if grandfathered:
                    print(
                        f"  ✓ Plugins now opt-in: grandfathered "
                        f"{len(grandfathered)} existing plugin(s) into zermes.plugins.enabled"
                    )
                else:
                    print(
    ...
    if missing_skill_config and interactive and not quiet:
        ...
        if answer in ("y", "yes"):
            ...
            config = load_config()
            try:
                from zermes.agent.skill_utils import SKILL_CONFIG_PREFIX
            except Exception:
...
def _normalize_max_turns_config(config: Dict[str, Any]) -> Dict[str, Any]:
    """Normalize legacy root-level max_turns into zermes.agent.max_turns."""
    config = dict(config)
...
def redact_key(key: str) -> str:
    ...
    Thin wrapper over :func:`zermes.agent.redact.mask_secret` — preserves the
    "(not set)" placeholder in dim color for the empty case.
    """
    from zermes.agent.redact import mask_secret
...
def show_config():
    ...
    # Skill config
    try:
        from zermes.agent.skill_utils import discover_all_skill_config_vars, resolve_skill_config_values
