...
def uninstall_gateway_service():
    ...
    try:
        from zermes.hermes_cli.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from zermes.hermes_cli.gateway import get_launchd_plist_path
...
def remove_portable_tooling_windows(hermes_home: Path) -> list[Path]:
    ...
    isolated from any system Git / Node so they cannot break other zermes.tools."""
    removed: list[Path] = []
...
def _uninstall_profile(profile) -> None:
    ...
    #    Use `python -m zermes.hermes_cli.main` so we don't depend on a `hermes`
    #    wrapper that may be half-removed mid-uninstall.
    hermes_invocation = [_sys.executable, "-m", "zermes.hermes_cli.main", "--profile", name]
...
def run_uninstall(args):
    ...
    log_info("Checking for running zermes.gateway...")
