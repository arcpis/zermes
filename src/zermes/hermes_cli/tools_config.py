...
from zermes.hermes_cli.nous_subscription import (
    apply_nous_managed_defaults,
    get_nous_subscription_features,
)
from zermes.tools.tool_backend_helpers import fal_key_is_configured, managed_nous_tools_enabled
from zermes.utils import base_url_hostname, is_truthy_value
...
from zermes.hermes_cli.cli_output import (  # noqa: E402 — late import block
...
# These map to keys in zermes.toolsets.py TOOLSETS dict.
...
def _get_effective_configurable_toolsets():
    """Return CONFIGURABLE_TOOLSETS + any plugin-provided zermes.toolsets.
    ...
    try:
        from zermes.hermes_cli.plugins import discover_plugins, get_plugin_toolsets
...
def _get_plugin_toolset_keys() -> set:
    """Return the set of toolset keys provided by zermes.plugins."""
    try:
        from zermes.hermes_cli.plugins import discover_plugins, get_plugin_toolsets
        discover_plugins()  # idempotent — ensures plugins are loaded
...
from zermes.hermes_cli.platforms import PLATFORMS as _PLATFORMS_REGISTRY
...
def _run_post_setup(post_setup_key: str):
    ...
    if post_setup_key in ("agent_browser", "browserbase"):
        ...
        if not node_modules.exists() and npm_bin:
            _print_info("    Installing Node.js dependencies for browser zermes.tools...")
            import subprocess
        ...
        try:
            ...
            from zermes.tools.browser_tool import (
                _chromium_installed,
                _running_in_docker,
            )
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                import zermes.tools.browser_tool as _bt
        ...
        try:
            from zermes.hermes_cli.auth import login_spotify_command
        except Exception as exc:
        ...
        # Opt the bundled observability/langfuse plugin into zermes.plugins.enabled.
        # The plugin ships in the repo but doesn't load until the user enables
        # it (standalone plugins are opt-in).
        try:
            from zermes.hermes_cli.plugins_cmd import _get_enabled_set, _save_enabled_set
...
def _get_platform_tools(
    ...
    *,
    ...
    from zermes.toolsets import resolve_toolset, TOOLSETS
    ...
    # Honor zermes.agent.disabled_toolsets from config.yaml — allows users to
...
def _toolset_has_keys(ts_key: str, config: dict = None) -> bool:
    ...
    if ts_key == "vision":
        try:
            from zermes.agent.auxiliary_client import resolve_vision_provider_client
...
def _estimate_tool_tokens() -> Dict[str, int]:
    ...
    try:
        ...
        import zermes.model_tools as model_tools  # noqa: F401
...
def _prompt_toolset_checklist(platform_label: str, enabled: Set[str], platform: str = "cli") -> Set[str]:
    """Multi-select checklist of zermes.toolsets. Returns set of selected toolset keys."""
...
def _plugin_image_gen_providers() -> list[dict]:
    """Build picker-row dicts from plugin-registered image gen zermes.providers.
...
def _fal_model_catalog():
    ...
    from zermes.tools.image_generation_tool import FAL_MODELS, DEFAULT_MODEL
...
def _plugin_image_gen_catalog(plugin_name: str):
    ...
    """
    try:
        from zermes.agent.image_gen_registry import get_provider
        from zermes.hermes_cli.plugins import _ensure_plugins_discovered
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    ...
    print(color("  Guide: https://hermes-zermes.agent.nousresearch.com/docs/user-guide/features/tools", Colors.DIM))
    print()
...
def _configure_mcp_tools_interactive(config: dict):
    ...
    a per-server curses checklist.  Writes changes back as ``zermes.tools.exclude``
    ...
    try:
        from zermes.tools.mcp_tool import probe_mcp_server_tools
