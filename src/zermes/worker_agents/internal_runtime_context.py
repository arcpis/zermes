...
def _permission_snapshot(profile: WorkerAgentProfile) -> RuntimePermissionSnapshot:
    return RuntimePermissionSnapshot(
        allowed_tool_names=profile.zermes.tools.allowed_tools,
        workspace_read_roots=profile.workspace.read_roots,
        workspace_write_roots=profile.workspace.write_roots,
        approval_required_tool_names=profile.zermes.tools.approval_required_tools,
    )
...
def _allowed_tool_descriptions(profile: WorkerAgentProfile) -> tuple[str, ...]:
    from zermes.model_tools import get_tool_description

    return tuple(
        f"{tool_name}: {get_tool_description(tool_name)}"
        for tool_name in profile.zermes.tools.allowed_tools
    )
