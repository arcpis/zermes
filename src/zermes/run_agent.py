...
Usage:
    ...
    response = zermes.agent.run_conversation("Tell me about the latest Python updates")
"""
...
#   (b) `patch("zermes.run_agent.OpenAI", ...)` test patterns used by ~28 test files.
#
# NOTE: `fire` is ONLY used in the `__main__` block below (for running
# zermes.run_agent.py directly as a CLI) — it is NOT needed for library usage.
...
from zermes.hermes_cli.timeouts import (
...
from zermes.model_tools import (
    get_tool_definitions,
    get_toolset_for_tool,
    handle_function_call,
    check_toolset_requirements,
)
from zermes.tools.terminal_tool import cleanup_vm, get_active_env, is_persistent_env
...
from zermes.tools.tool_result_storage import maybe_persist_tool_result, enforce_turn_budget
from zermes.tools.interrupt import set_interrupt as _set_interrupt
from zermes.tools.browser_tool import cleanup_browser


# Agent internals extracted to agent/ package for modularity
from zermes.agent.memory_manager import StreamingContextScrubber, build_memory_context_block, sanitize_context
from zermes.agent.think_scrubber import StreamingThinkScrubber
from zermes.agent.retry_utils import jittered_backoff
from zermes.agent.error_classifier import classify_api_error, FailoverReason
...
from zermes.agent.subdirectory_hints import SubdirectoryHintTracker
from zermes.agent.prompt_caching import apply_anthropic_cache_control
from zermes.agent.prompt_builder import build_skills_system_prompt, build_context_files_prompt, build_environment_hints, load_soul_md, TOOL_USE_ENFORCEMENT_GUIDANCE, TOOL_USE_ENFORCEMENT_MODELS, GOOGLE_MODEL_OPERATIONAL_GUIDANCE, OPENAI_MODEL_EXECUTION_GUIDANCE
from zermes.agent.usage_pricing import estimate_usage_cost, normalize_usage
...
from zermes.agent.display import (
...
from zermes.agent.trajectory import (
    convert_scratchpad_to_think, has_incomplete_scratchpad,
    save_trajectory as _save_trajectory_to_file,
)
from zermes.utils import atomic_json_write, base_url_host_matches, base_url_hostname, env_var_enabled, normalize_proxy_url
...
def _install_safe_stdio() -> None:
    """Wrap stdout/stderr so best-effort console output cannot crash the zermes.agent."""
...
class IterationBudget:
    """Thread-safe iteration counter for an zermes.agent.
...
def _extract_parallel_scope_path(tool_name: str, function_args: dict) -> Path | None:
    """Return the normalized file target for path-scoped zermes.tools."""
...
# _summarize_user_message_for_log is imported from zermes.agent.codex_responses_adapter
...
def _routermint_headers() -> dict:
    ...
    from zermes.hermes_cli import __version__ as _HERMES_VERSION
...
class AIAgent:
    ...
    def __init__(
        ...
        # without interrupting the zermes.agent. Unlike interrupt(), steer() does
        ...
        self._cache_ttl = "5m"
        try:
            from zermes.hermes_cli.config import load_config as _load_pc_cfg
        ...
        # Centralized logging — zermes.agent.log (INFO+) and errors.log (WARNING+)
        # both live under ~/.hermes/logs/.  Idempotent, so gateway mode
        # (which creates a new AIAgent per message) won't duplicate handlers.
        from zermes.hermes_logging import setup_logging, setup_verbose_logging
        ...
        if self.verbose_logging:
            ...
            if self.quiet_mode:
                ...
                # File handlers (zermes.agent.log, errors.log) still capture everything.
                for quiet_logger in [
                    'tools',               # all zermes.tools.* (terminal, browser, web, file, etc.)
        ...
        # Codex/Anthropic wrapping for all known zermes.providers.
        ...
        if self.api_mode == "anthropic_messages":
            from zermes.agent.anthropic_adapter import build_anthropic_client, resolve_anthropic_token
            # Bedrock + Claude → use AnthropicBedrock SDK for full feature parity
            # (prompt caching, thinking budgets, adaptive thinking).
            _is_bedrock_anthropic = self.provider == "bedrock"
            if _is_bedrock_anthropic:
                from zermes.agent.anthropic_adapter import build_anthropic_bedrock_client
                ...
                from zermes.agent.anthropic_adapter import _is_oauth_token as _is_oat
            ...
            try:
                from zermes.hermes_cli.config import load_config as _load_br_cfg
            ...
            if api_key and base_url:
                ...
                if base_url_host_matches(effective_base, "openrouter.ai"):
                    from zermes.agent.auxiliary_client import build_or_headers
                    client_kwargs["default_headers"] = build_or_headers()
                elif base_url_host_matches(effective_base, "api.routermint.com"):
                    client_kwargs["default_headers"] = _routermint_headers()
                elif base_url_host_matches(effective_base, "api.githubcopilot.com"):
                    from zermes.hermes_cli.models import copilot_default_headers
                    ...
                    from zermes.agent.auxiliary_client import _codex_cloudflare_headers
                    ...
                    try:
                        from zermes.providers import get_provider_profile as _gpf
                ...
                from zermes.agent.auxiliary_client import resolve_provider_client
        ...
        from zermes.tools.checkpoint_manager import CheckpointManager
        ...
        try:
            from zermes.hermes_cli.config import load_config as _load_agent_config
        ...
        if not skip_memory:
            try:
                ...
                if self._memory_enabled or self._user_profile_enabled:
                    from zermes.tools.memory_tool import MemoryStore
        ...
        if not skip_memory:
            try:
                ...
                if _mem_provider_name:
                    from zermes.agent.memory_manager import MemoryManager as _MemoryManager
                    from zermes.plugins.memory import load_memory_provider as _load_mem
        ...
        if self._memory_manager and self.tools is not None:
            ...
            for _schema in self._memory_manager.get_all_tool_schemas():
                ...
                self.zermes.tools.append(_wrapped)
                if _tname:
        ...
        # overridable via zermes.agent.api_max_retries in config.yaml.  See #11616.
        try:
        ...
        try:
            from zermes.agent.auxiliary_client import _compression_threshold_for_model as _cthresh_fn
        ...
        if _config_context_length is None and _custom_providers:
            try:
                from zermes.hermes_cli.config import get_custom_provider_context_length
        ...
        if _engine_name != "compressor":
            ...
            try:
                from zermes.plugins.context_engine import load_context_engine
            ...
            if _selected_engine is None:
                try:
                    from zermes.hermes_cli.plugins import get_plugin_context_engine
        ...
        from zermes.agent.model_metadata import MINIMUM_CONTEXT_LENGTH
    ...
    def _ensure_lmstudio_runtime_loaded(self, config_context_length: Optional[int] = None) -> None:
        ...
        try:
            from zermes.agent.model_metadata import MINIMUM_CONTEXT_LENGTH
            from zermes.hermes_cli.models import ensure_lmstudio_model_loaded
    ...
    def switch_model(self, new_model, new_provider, api_key='', base_url='', api_mode=''):
        """Switch the model/provider in-place for a live zermes.agent.
        ...
        turn-scoped).
        """
        from zermes.hermes_cli.providers import determine_api_mode
        ...
        if hasattr(self, "context_compressor") and self.context_compressor:
            ...
            try:
                from zermes.hermes_cli.config import load_config, get_compatible_custom_providers
                _sm_cfg = load_config()
    ...
    def _resolved_api_call_timeout(self) -> float:
        ...
        Priority:
          1. ``zermes.providers.<id>.models.<model>.timeout_seconds`` (per-model override)
          2. ``zermes.providers.<id>.request_timeout_seconds`` (provider-wide)
    ...
    def _resolved_api_call_stale_timeout_base(self) -> tuple[float, bool]:
        ...
        Priority:
          1. ``zermes.providers.<id>.models.<model>.stale_timeout_seconds``
          2. ``zermes.providers.<id>.stale_timeout_seconds``
    ...
    def _anthropic_prompt_cache_policy(
        ...
        *,
        ...
        if is_anthropic_wire and is_claude:
            # Third-party Anthropic-compatible zermes.gateway.
            return True, True
    ...
    def _provider_model_requires_responses_api(
        ...
        *,
        ...
        if normalized_provider == "copilot":
            try:
                from zermes.hermes_cli.models import _should_use_copilot_responses_api
    ...
    def interrupt(self, message: str = None) -> None:
        ...
        Example (CLI):
            ...
            if user_typed_something:
                zermes.agent.interrupt(user_input)
    ...
    def _capture_rate_limits(self, http_response: Any) -> None:
        ...
        if not headers:
            return
        try:
            from zermes.agent.rate_limit_tracker import parse_rate_limit_headers
    ...
    def _sync_external_memory_for_turn(
        ...
        *,
        ...
        """Mirror a completed turn into external memory zermes.providers.
    ...
    def _build_system_prompt(self, system_message: str = None) -> str:
        ...
        # zermes.agent.tool_use_enforcement:
        ...
        from zermes.hermes_time import now as _hermes_now
        now = _hermes_now()
    ...
    def _cap_delegate_task_calls(tool_calls: list) -> list:
        ...
        """
        from zermes.tools.delegate_tool import _get_max_concurrent_children
    ...
    def _create_openai_client(self, client_kwargs: dict, *, reason: str, shared: bool) -> Any:
        from zermes.agent.auxiliary_client import _validate_base_url, _validate_proxy_env_urls
        ...
        if self.provider == "google-gemini-cli" or str(client_kwargs.get("base_url", "")).startswith("cloudcode-pa://"):
            from zermes.agent.gemini_cloudcode_adapter import GeminiCloudCodeClient
        ...
        if self.provider == "gemini":
            from zermes.agent.gemini_native_adapter import GeminiNativeClient, is_native_gemini_base_url
        ...
        # access via __getattr__ below. Tests patch via `zermes.run_agent.OpenAI`.
    ...
    def _copilot_headers_for_request(self, *, is_vision: bool) -> dict:
        from zermes.hermes_cli.copilot_auth import copilot_request_headers
    ...
    def _try_refresh_nous_client_credentials(self, *, force: bool = True) -> bool:
        ...
        try:
            from zermes.hermes_cli.auth import resolve_nous_runtime_credentials
    ...
    def _try_refresh_copilot_client_credentials(self) -> bool:
        ...
        try:
            from zermes.hermes_cli.copilot_auth import resolve_copilot_token
    ...
    def _try_refresh_anthropic_client_credentials(self) -> bool:
        ...
        try:
            from zermes.agent.anthropic_adapter import resolve_anthropic_token, build_anthropic_client
    ...
    def _apply_client_headers_for_base_url(self, base_url: str) -> None:
        from zermes.agent.auxiliary_client import _AI_GATEWAY_HEADERS, build_or_headers

        if base_url_host_matches(base_url, "openrouter.ai"):
            self._client_kwargs["default_headers"] = build_or_headers()
        elif base_url_host_matches(base_url, "ai-zermes.gateway.vercel.sh"):
            ...
            _ph_headers = None
            try:
                from zermes.providers import get_provider_profile as _gpf2
    ...
    def _swap_credential(self, entry) -> None:
        ...
        if self.api_mode == "anthropic_messages":
            from zermes.agent.anthropic_adapter import build_anthropic_client, _is_oauth_token
    ...
    def _rebuild_anthropic_client(self) -> None:
        ...
        if getattr(self, "provider", None) == "bedrock":
            ...
            from zermes.agent.anthropic_adapter import build_anthropic_client
            self._anthropic_client = build_anthropic_client(
    ...
    def _interruptible_streaming_api_call(
        ...
        def _call_chat_completions():
            ...
            # HERMES_STREAM_READ_TIMEOUT (default 120s) for cloud zermes.providers.
            if _provider_timeout_cfg is not None:
    ...
    def _try_activate_fallback(self, reason: "FailoverReason | None" = None) -> bool:
        ...
        # raw_codex=True because the main agent needs direct responses.stream()
        # access for Codex zermes.providers.
        try:
            from zermes.agent.auxiliary_client import resolve_provider_client
            ...
            try:
                from zermes.hermes_cli.model_normalize import normalize_model_for_provider
            ...
            if fb_api_mode == "anthropic_messages":
                ...
                from zermes.agent.anthropic_adapter import build_anthropic_client, resolve_anthropic_token, _is_oauth_token
    ...
    def _model_supports_vision(self) -> bool:
        ...
        """
        try:
            from zermes.agent.models_dev import get_model_capabilities
    ...
    def _try_shrink_image_parts_in_messages(self, api_messages: list) -> bool:
        ...
        try:
            from zermes.tools.vision_tools import _resize_image_for_vision
        except Exception as exc:
    ...
    def _build_api_kwargs(self, api_messages: list) -> dict:
        ...
        try:
            from zermes.agent.auxiliary_client import _fixed_temperature_for_model, OMIT_TEMPERATURE
        ...
        if (_is_or or _is_nous) and "claude" in (self.model or "").lower():
            try:
                from zermes.agent.anthropic_adapter import _get_anthropic_max_output
    ...
    def _supports_reasoning_extra_body(self) -> bool:
        ...
        OpenRouter forwards unknown extra_body fields to upstream zermes.providers.
        Some providers/routes reject `reasoning` with 400s, so gate it to
        known reasoning-capable model families and direct Nous Portal.
        """
        if base_url_host_matches(self._base_url_lower, "nousresearch.com"):
            return True
        if base_url_host_matches(self._base_url_lower, "ai-zermes.gateway.vercel.sh"):
            return True
        if (
            base_url_host_matches(self._base_url_lower, "models.github.ai")
            or base_url_host_matches(self._base_url_lower, "api.githubcopilot.com")
            ...
            try:
                from zermes.hermes_cli.models import github_model_reasoning_efforts
    ...
    def _lmstudio_reasoning_options_cached(self) -> list[str]:
        ...
        if cached is not None:
            ...
            if opts or (_time.monotonic() - ts) < 60:
                return opts
        try:
            from zermes.hermes_cli.models import lmstudio_model_reasoning_options
    ...
    def _resolve_lmstudio_summary_reasoning_effort(self) -> Optional[str]:
        ...
        """
        from zermes.agent.lmstudio_reasoning import resolve_lmstudio_effort
    ...
    def _build_assistant_message(self, assistant_message, finish_reason: str) -> dict:
        ...
        if reasoning_text and self.reasoning_callback:
            ...
            # CLI post-response display fallback (zermes.cli.py _reasoning_shown_this_turn).
    ...
    def _sanitize_tool_calls_for_strict_api(api_msg: dict) -> dict:
        """Strip Codex Responses API fields from tool_calls for strict zermes.providers.
    ...
    def _compress_context(self, messages: list, system_message: str, *, approx_tokens: int = None, task_id: str = "default", focus_topic: str = None) -> tuple:
        ...
        try:
            from zermes.tools.file_tools import reset_file_dedup
    ...
    def _dispatch_delegate_task(self, function_args: dict) -> str:
        ...
        """
        from zermes.tools.delegate_tool import delegate_task as _delegate_task
    ...
    def _invoke_tool(self, function_name: str, function_args: dict, effective_task_id: str,
        ...
        zermes.tools. Used by the concurrent execution path; the sequential path retains
        ...
        if not pre_tool_block_checked:
            try:
                from zermes.hermes_cli.plugins import get_pre_tool_call_block_message
        ...
        if function_name == "todo":
            from zermes.tools.todo_tool import todo_tool as _todo_tool
            return _todo_tool(
            ...
            from zermes.tools.session_search_tool import session_search as _session_search
            ...
            from zermes.tools.memory_tool import memory_tool as _memory_tool
            result = _memory_tool(
            ...
            from zermes.tools.clarify_tool import clarify_tool as _clarify_tool
    ...
    def _execute_tool_calls_concurrent(self, assistant_message, messages: list, effective_task_id: str, api_call_count: int = 0) -> None:
        ...
        def _run_tool(index, tool_call, function_name, function_args):
            ...
            try:
                from zermes.tools.environments.base import set_activity_callback
                set_activity_callback(self._touch_activity)
            except Exception:
                pass
            # Propagate approval/sudo callbacks to this worker thread.
            # Mirrors zermes.cli.py run_agent() pattern (GHSA-qg5c-hvr5-hjgr).
    ...
    def _execute_tool_calls_sequential(self, assistant_message, messages: list, effective_task_id: str, api_call_count: int = 0) -> None:
        """Execute tool calls sequentially (original behavior). Used for single calls or interactive zermes.tools."""
    ...
    def _handle_max_iterations(self, messages: list, api_call_count: int) -> str:
        ...
        summary_request = (
            ...
            "without calling any more zermes.tools."
        )
        ...
        try:
            ...
            summary_extra_body = {}
            try:
                from zermes.agent.auxiliary_client import _fixed_temperature_for_model, OMIT_TEMPERATURE as _OMIT_TEMP
            except Exception:
    ...
    def run_conversation(
        ...
        from zermes.hermes_logging import set_session_context
        ...
        from zermes.tools.skill_provenance import set_current_write_origin
        ...
        while (api_call_count < self.max_iterations and self.iteration_budget.remaining > 0) or self._budget_grace_call:
            ...
            # cloud zermes.providers.  Operates on api_messages (the API copy) so
            ...
            while retry_count < max_retries:
                ...
                if self.provider == "nous":
                    try:
                        from zermes.agent.nous_rate_guard import (
                ...
                try:
                    ...
                    if hasattr(response, 'usage') and response.usage:
                        ...
                        if self._session_db and self.session_id:
                            try:
                                ...
                                # visible in zermes.agent.log — silent loss here is
                    ...
                    if self.provider == "nous":
                        try:
                            from zermes.agent.nous_rate_guard import clear_nous_rate_limit
                    ...
                    if (
                        ...
                        try:
                            from zermes.tools.schema_sanitizer import strip_pattern_and_format
                    ...
                    if (
                        ...
                        if self.providers_allowed:
                            self._vprint(
                                f"{self.log_prefix}      Your provider_routing.only restriction is filtering out tool-capable zermes.providers.",
                                force=True,
                            )
                            self._vprint(
                ...
                if assistant_message.tool_calls:
                    ...
                    if not self._has_content_after_think_block(final_response):
                        ...
                        if fallback and getattr(self, '_last_content_tools_all_housekeeping', False):
                            ...
                            # old code injected "Calling the X zermes.tools..." which
        ...
        # Always logged at INFO so zermes.agent.log captures WHY every turn ended.
...
def main(
    ...
    Args:
        query (str): Natural language query for the zermes.agent. Defaults to Python 3.13 example.
    ...
    # Handle tool listing
    if list_tools:
        from zermes.model_tools import get_all_tool_names, get_available_toolsets
        from zermes.toolsets import get_all_toolsets, get_toolset_info
        ...
        print("  python zermes.run_agent.py --enabled_toolsets=research --query='search for Python news'")
        print("  python zermes.run_agent.py --enabled_toolsets=development --query='debug this code'")
        print("  python zermes.run_agent.py --enabled_toolsets=safe --query='analyze without terminal'")
        print("  ")
        print("  # Combine multiple toolsets")
        print("  python zermes.run_agent.py --enabled_toolsets=web,vision --query='analyze website'")
        print("  ")
        print("  # Disable toolsets")
        print("  python zermes.run_agent.py --disabled_toolsets=terminal --query='no command execution'")
        print("  ")
        print("  # Run with trajectory saving enabled")
        print("  python zermes.run_agent.py --save_trajectories --query='your question here'")
        return
