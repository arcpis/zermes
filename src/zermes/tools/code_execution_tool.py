...
def _scrub_child_env(source_env, is_passthrough=None, is_windows=None):
    ...
    if is_passthrough is None:
        try:
            from zermes.tools.env_passthrough import is_env_passthrough as _ep
        except Exception:
...
def _rpc_server_loop(
    ...
    """
    from zermes.model_tools import handle_function_call
...
def execute_code(
    ...
    to a subset of Hermes zermes.tools.
    ...
    from zermes.tools.terminal_tool import _get_env_config
    ...
    try:
        ...
        from zermes.hermes_constants import get_subprocess_home
...
def build_execute_code_schema(enabled_sandbox_tools: set = None,
                              mode: str = None) -> dict:
    """Build the execute_code schema with description listing only enabled zermes.tools.
...
# current configured mode).  zermes.model_tools.py rebuilds per-session anyway.
