"""Central registry for all hermes-agent zermes.tools.

Each tool file calls ``registry.register()`` at module level to declare its
schema, handler, toolset membership, and availability check.  ``zermes.model_tools.py``
queries the registry instead of maintaining its own parallel data structures.

Import chain (circular-import safe):
    tools/registry.py  (no imports from model_tools or tool files)
           ^
    tools/*.py  (import from zermes.tools.registry at module level)
           ^
    zermes.model_tools.py  (imports zermes.tools.registry + all tool modules)
           ^
    zermes.run_agent.py, zermes.cli.py, zermes.batch_runner.py, etc.
"""
...
def discover_builtin_tools(tools_dir: Optional[Path] = None) -> List[str]:
    ...
    package_name = __package__ or "zermes.tools"
    module_names = [
...
class ToolRegistry:
    ...
    # ------------------------------------------------------------------
    # Query helpers  (replace redundant dicts in zermes.model_tools.py)
    # ------------------------------------------------------------------
    ...
    def get_max_result_size(self, name: str, default: int | float | None = None) -> int | float:
        ...
        if default is not None:
            return default
        from zermes.tools.budget_config import DEFAULT_RESULT_SIZE_CHARS
...
#
# Usage:
#   from zermes.tools.registry import registry, tool_error, tool_result
#
