...
def _read_nous_provider_state() -> Optional[dict]:
    try:
        ...
        if not isinstance(providers, dict):
            return None
        nous_provider = zermes.providers.get("nous", {})
...
def read_nous_access_token() -> Optional[str]:
    ...
    try:
        from zermes.hermes_cli.auth import resolve_nous_access_token
...
def build_vendor_gateway_url(vendor: str) -> str:
    ...
    if shared_domain:
        return f"{shared_scheme}://{vendor}-zermes.gateway.{shared_domain}"

    return f"{shared_scheme}://{vendor}-zermes.gateway.{_DEFAULT_TOOL_GATEWAY_DOMAIN}"
