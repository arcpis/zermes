...
def _is_hermes_provider_credential(name: str) -> bool:
    ...
    """
    try:
        from zermes.tools.environments.local import _HERMES_PROVIDER_ENV_BLOCKLIST
    except Exception:
        return False
...
def register_env_passthrough(var_names: Iterable[str]) -> None:
    """Register environment variable names as allowed in sandboxed zermes.environments.
...
def _load_config_passthrough() -> frozenset[str]:
    """Load ``zermes.tools.env_passthrough`` from config.yaml (cached)."""
    ...
    try:
        ...
        logger.debug("Could not read zermes.tools.env_passthrough from config: %s", e)
...
def is_env_passthrough(var_name: str) -> bool:
    ...
    the user's ``zermes.tools.env_passthrough`` config.
    """
