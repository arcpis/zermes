...
For captures / actions with `capture_after=True`:
  ...
  zermes.run_agent.py's tool-message builder inspects `_multimodal` and emits a
  list-shaped `content` for OpenAI-compatible zermes.providers. The Anthropic
...
from zermes.tools.computer_use.backend import (
    ActionResult,
    CaptureResult,
    ComputerUseBackend,
    UIElement,
    ...
    with _backend_lock:
        if _backend is None:
            ...
            if backend_name in ("cua", "cua-driver", ""):
                from zermes.tools.computer_use.cua_backend import CuaDriverBackend
...
def handle_computer_use(args: Dict[str, Any], **kwargs) -> Any:
    """Main entry point — dispatched by zermes.tools.registry.

    Returns either a JSON string (text-only) or a dict marked `_multimodal`
    (image + summary) which zermes.run_agent.py wraps into the tool message.
    """
...
def check_computer_use_requirements() -> bool:
    ...
    if sys.platform != "darwin":
        return False
    from zermes.tools.computer_use.cua_backend import cua_driver_binary_available
