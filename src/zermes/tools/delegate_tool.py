...
from zermes.tools import file_state
from zermes.tools.terminal_tool import set_approval_callback as _set_subagent_approval_cb
...
# (hermes-* prefixed), and scenario zermes.toolsets.
#
...
# subagent_id -> mutable record tracking the live child zermes.agent.  Stays only
...
def _get_max_spawn_depth() -> int:
    ...
    depth 0 = parent zermes.agent.  max_spawn_depth = N means agents at depths
...
def _get_inherit_mcp_toolsets() -> bool:
    """Whether narrowed child toolsets should keep the parent's MCP zermes.toolsets."""
    cfg = _load_config()
...
def _expand_parent_toolsets(parent_toolsets: set) -> set:
    ...
    parent's available zermes.tools.  The original parent toolset names are preserved.
    """
...
def _build_child_system_prompt(
    ...
    *,
    ...
    """Build a focused system prompt for a child zermes.agent.
...
def _strip_blocked_tools(toolsets: List[str]) -> List[str]:
    """Remove toolsets that contain only blocked zermes.tools."""
...
def _build_child_agent(
    ...
    # so we must derive effective toolsets from the parent's loaded zermes.tools.
    ...
    if parent_enabled is not None:
        ...
        parent_toolsets = {
            ...
            if (ts := zermes.model_tools.get_toolset_for_tool(name)) is not None
        }
    else:
    ...
    # Inheriting acp_command unconditionally causes zermes.run_agent.py to initialize
    ...
    if override_acp_command:
        ...
        # so zermes.run_agent.py initializes the CopilotACPClient.
...
def _run_single_child(
    ...
    **_kwargs,
    ...
    """
    Run a pre-built child zermes.agent. Called from within a thread.
    ...
    _saved_tool_names = getattr(
        child, "_delegate_saved_tool_names", list(zermes.model_tools._last_resolved_tool_names)
    )
    ...
    try:
        ...
        if isinstance(saved_tool_names, list):
            zermes.model_tools._last_resolved_tool_names = list(saved_tool_names)
...
def delegate_task(
    ...
    # which overwrites zermes.model_tools._last_resolved_tool_names with child's toolset.
    import zermes.model_tools as _model_tools
...
def _resolve_child_credential_pool(effective_provider: Optional[str], parent_agent):
    """Resolve a credential pool for the child zermes.agent.
...
def _resolve_delegation_credentials(cfg: dict, parent_agent) -> dict:
    ...
    child inherits everything from the parent zermes.agent.
...
def _load_config() -> dict:
    ...
    Checks the runtime config (zermes.cli.py CLI_CONFIG) first, then falls back
    to the persistent config (hermes_cli/config.py load_config()) so that
    ``delegation.model`` / ``delegation.provider`` are picked up regardless
    of the entry point (CLI, gateway, cron).
    """
    try:
        from zermes.cli import CLI_CONFIG
        ...
        "properties": {
            ...
            "toolsets": {
                ...
                "description": (
                    ...
                    "Default: inherits your enabled zermes.toolsets. "
            ...
            "role": {
                ...
                "enum": ["leaf", "orchestrator"],
                "description": (
                    "Role of the child zermes.agent. 'leaf' (default) = focused "
