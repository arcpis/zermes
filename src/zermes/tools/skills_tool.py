...
Usage:
    from zermes.tools.skills_tool import skills_list, skill_view, check_skills_requirements
...
def skill_matches_platform(frontmatter: Dict[str, Any]) -> bool:
    ...
    Delegates to ``zermes.agent.skill_utils.skill_matches_platform`` — kept here
    as a public re-export so existing callers don't need updating.
    """
    from zermes.agent.skill_utils import skill_matches_platform as _impl
...
def _gateway_setup_hint() -> str:
    try:
        from zermes.gateway.platforms.base import GATEWAY_SECRET_CAPTURE_UNSUPPORTED_MESSAGE
...
def _parse_frontmatter(content: str) -> Tuple[Dict[str, Any], str]:
    ...
    Delegates to ``zermes.agent.skill_utils.parse_frontmatter`` — kept here
    as a public re-export so existing callers don't need updating.
    """
...
def _get_category_from_path(skill_path: Path) -> Optional[str]:
    ...
    try:
        from zermes.agent.skill_utils import get_external_skills_dirs
...
def _get_disabled_skill_names() -> Set[str]:
    ...
    Delegates to ``zermes.agent.skill_utils.get_disabled_skill_names`` — kept here
    as a public re-export so existing callers don't need updating.
    """
...
def _get_session_platform() -> str:
    ...
    ``zermes.agent.skill_utils.get_disabled_skill_names`` so that
...
def _find_all_skills(*, skip_disabled: bool = False) -> List[Dict[str, Any]]:
    ...
    """
    from zermes.agent.skill_utils import get_external_skills_dirs, iter_skill_index_files
...
def _serve_plugin_skill(
    ...
    *,
    ...
    from zermes.hermes_cli.plugins import _get_disabled_plugins, get_plugin_manager
    ...
    if preprocess:
        try:
            from zermes.agent.skill_preprocessing import preprocess_skill_content
...
def skill_view(
    ...
    try:
        ...
        if ":" in name:
            from zermes.agent.skill_utils import is_valid_namespace, parse_qualified_name
        ...
        if not skill_md:
            for search_dir in all_dirs:
                from zermes.agent.skill_utils import iter_skill_index_files
        ...
        if file_path and skill_dir:
            from zermes.tools.path_security import validate_within_dir, has_traversal_component
        ...
        if required_cred_files_raw:
            try:
                from zermes.tools.credential_files import register_credential_files
...
def _skill_view_with_bump(args, **kw):
    ...
    try:
        ...
        if isinstance(parsed, dict) and parsed.get("success"):
            ...
            if resolved:
                from zermes.tools.skill_usage import bump_use, bump_view
