...
The active adapter singleton lives in ``zermes.gateway.platforms.yuanbao`` and is
...
def _get_active_adapter():
    """Lazy import to avoid ImportError when zermes.gateway.platforms.yuanbao is unavailable."""
    try:
        from zermes.gateway.platforms.yuanbao import get_active_adapter
...
async def search_sticker(query: str = "", limit: int = 10) -> dict:
    ...
    """
    from zermes.gateway.platforms.yuanbao_sticker import search_stickers
...
from zermes.tools.registry import registry, tool_result  # noqa: E402
