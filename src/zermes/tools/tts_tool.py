...
- Users can declare any number of named providers with ``type: command``
  under ``tts.zermes.providers.<name>`` in ``~/.hermes/config.yaml``. Hermes
...
Usage:
    from zermes.tools.tts_tool import text_to_speech_tool, check_tts_requirements
...
def get_env_value(name, default=None):
    ...
    Tests may monkeypatch and later restore ``zermes.hermes_cli.config.get_env_value``
    before this module is imported. Resolve the helper at call time so TTS does
    not keep a stale imported function for the rest of the test process.
    """
    try:
        from zermes.hermes_cli.config import get_env_value as _get_env_value
    except ImportError:
        return os.getenv(name, default)
    value = _get_env_value(name)
    return default if value is None else value
from zermes.tools.managed_tool_gateway import resolve_managed_tool_gateway
from zermes.tools.tool_backend_helpers import managed_nous_tools_enabled, prefers_gateway, resolve_openai_audio_api_key
from zermes.tools.xai_http import hermes_xai_user_agent
...
def _resolve_max_text_length(
    ...
    Resolution order:
      ...
      2. ``tts.zermes.providers.<provider>.max_text_length`` for user-declared
         command providers
    ...
    # User-declared command provider (under tts.zermes.providers.<name>)
...
def _load_tts_config() -> Dict[str, Any]:
    ...
    try:
        ...
        logger.debug("zermes.hermes_cli.config not available, using default TTS config")
        return {}
...
# ===========================================================================
# Custom command providers (type: command under tts.zermes.providers.<name>)
# ===========================================================================
#
...
# interpreted as a reference to ``tts.zermes.providers.<name>``.
...
def _get_named_provider_config(
    ...
    Looks up ``tts.zermes.providers.<name>`` first (the canonical location), and
    ...
    section = zermes.providers.get(name) if isinstance(providers, dict) else None
...
def _generate_command_tts(
    ...
    if not command_template:
        raise ValueError(
            f"tts.zermes.providers.{provider_name}.command is not configured"
        )
...
def text_to_speech_tool(
    ...
    # User-declared command provider (type: command under tts.zermes.providers.<name>)
    # resolves BEFORE the built-in dispatch. Built-in names short-circuit here
    # so a user's ``tts.zermes.providers.openai.command`` can't override the real
    # OpenAI handler.
...
def _has_openai_audio_backend() -> bool:
    """Return True when OpenAI audio can use direct credentials or the managed zermes.gateway."""
...
# Markdown stripping patterns (same as zermes.cli.py _voice_speak_response)
