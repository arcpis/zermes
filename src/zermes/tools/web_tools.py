...
This module provides generic web tools that work with multiple backend zermes.providers.
...
- Firecrawl: https://docs.firecrawl.dev/introduction (search, extract, crawl; direct or derived firecrawl-zermes.gateway.<domain> for Nous Subscribers)
...
# ``patch("zermes.tools.web_tools.Firecrawl", ...)`` keep working.
if TYPE_CHECKING:
...
from zermes.tools.debug_helpers import DebugSession
from zermes.tools.managed_tool_gateway import (
    build_vendor_gateway_url,
    read_nous_access_token as _read_nous_access_token,
    resolve_managed_tool_gateway,
)
from zermes.tools.tool_backend_helpers import managed_nous_tools_enabled, prefers_gateway
...
def _resolve_web_extract_auxiliary(model: Optional[str] = None) -> tuple[Optional[Any], Optional[str], Dict[str, Any]]:
    ...
    if client is not None and _is_nous_auxiliary_client(client):
        from zermes.agent.auxiliary_client import get_auxiliary_extra_body
...
def web_search_tool(query: str, limit: int = 5) -> str:
    ...
    try:
        ...
        if backend == "searxng":
            from zermes.tools.web_providers.searxng import SearXNGSearchProvider
    ...
    try:
        ...
        if not safe_urls:
            ...
            if backend == "parallel":
                ...
                from zermes.tools.interrupt import is_interrupted as _is_interrupted
...
async def web_crawl_tool(
    ...
    try:
        ...
        if backend == "tavily":
            ...
            from zermes.tools.interrupt import is_interrupted as _is_int
            if _is_int():
                return tool_error("Interrupted", success=False)
