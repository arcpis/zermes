...
from zermes.hermes_constants import get_hermes_home, get_repo_resource_path
...
def _get_bundled_dir() -> Path:
    ...
    then falls back to the repository resources directory.  The legacy
    ``skills/`` fallback keeps older editable checkouts working.
    """
    env_override = os.getenv("HERMES_BUNDLED_SKILLS")
    if env_override:
        return Path(env_override)
    return get_repo_resource_path(
        "skills",
        "bundled",
        legacy_parts=("skills",),
    )
