...
def _cache_mcp_image_block(block) -> str:
    ...
    try:
        from zermes.gateway.platforms.base import cache_image_from_bytes
        ...
        # zermes.gateway.platforms.base not importable in this process (e.g. cron
        # without gateway deps). Fall back to silently dropping — callers
        # get any text blocks that did parse.
        logger.debug("MCP image caching skipped — zermes.gateway.platforms.base unavailable")
        return ""
...
class MCPServerTask:
    ...
    async def _run_stdio(self, config: dict):
        ...
        from zermes.tools.osv_check import check_package_for_malware
    ...
    async def _run_http(self, config: dict):
        ...
        if self._auth_type == "oauth":
            try:
                from zermes.tools.mcp_oauth_manager import get_manager
...
def _get_auth_error_types() -> tuple:
    ...
    Cached after first call. Includes:
      ...
      - ``zermes.tools.mcp_oauth.OAuthNonInteractiveError`` — raised by our callback
    ...
    try:
        ...
        pass
    try:
        from zermes.tools.mcp_oauth import OAuthNonInteractiveError
...
def _handle_auth_error_and_retry(
    ...
    auth-related exception. Workflow:
      ...
      1. Ask :class:`zermes.tools.mcp_oauth_manager.MCPOAuthManager.handle_401` if
...
def _make_read_resource_handler(server_name: str, tool_timeout: float):
    ...
    def _handler(args: dict, **kwargs) -> str:
        from zermes.tools.registry import tool_error
...
def _normalize_mcp_input_schema(schema: dict | None) -> dict:
    ...
    OpenAI-compatible zermes.providers.
    ...
    * MCP/Pydantic optional fields commonly arrive as
    ...
    def _strip_nullable_union(node):
        ...
        Delegates to ``zermes.tools.schema_sanitizer.strip_nullable_unions`` so MCP
        ...
        """
        from zermes.tools.schema_sanitizer import strip_nullable_unions
...
def _register_server_tools(name: str, server: MCPServerTask, config: dict) -> List[str]:
    ...
    Handles include/exclude filtering and utility zermes.tools. Toolset resolution
    for ``mcp-{server}`` and raw server-name aliases is derived from the live
    registry, rather than mutating ``zermes.toolsets.TOOLSETS`` at runtime.
    ...
    #   zermes.tools.include — whitelist: only these tool names are registered
    #   zermes.tools.exclude — blacklist: all tools EXCEPT these are registered
    #   include takes precedence over exclude
    #   Neither set → register all tools (backward-compatible default)
    tools_filter = config.get("tools") or {}
    include_set = _normalize_name_filter(tools_filter.get("include"), f"mcp_servers.{name}.zermes.tools.include")
    exclude_set = _normalize_name_filter(tools_filter.get("exclude"), f"mcp_servers.{name}.zermes.tools.exclude")
    ...
    for mcp_tool in server._tools:
        ...
        # Guard against collisions with built-in (non-MCP) zermes.tools.
    ...
    for entry in _select_utility_schemas(name, server, config):
        ...
        # Same collision guard for utility zermes.tools.
...
def register_mcp_servers(servers: Dict[str, dict]) -> List[str]:
    """Connect to explicit MCP servers and register their zermes.tools.
    ...
    from zermes.tools.interrupt import is_interrupted as _is_interrupted, set_interrupt as _set_interrupt
...
def discover_mcp_tools() -> List[str]:
    """Entry point: load config, connect to MCP servers, register zermes.tools.
...
def probe_mcp_server_tools() -> Dict[str, List[tuple]]:
    """Temporarily connect to configured MCP servers and list their zermes.tools.
    ...
    async def _probe_all():
        ...
        for name, outcome in zip(names, outcomes):
            ...
            for t in outcome._tools:
                ...
                zermes.tools.append((t.name, desc))
            result[name] = tools
