"""Website access policy helpers for URL-capable zermes.tools.
...
def _iter_blocklist_file_rules(path: Path) -> List[str]:
    ...
    rather than raising — a bad file path should not disable all web zermes.tools.
    """
    try:
...
def check_website_access(url: str, config_path: Optional[Path] = None) -> Optional[Dict[str, str]]:
    ...
    (fail-open) so a config typo doesn't break all web zermes.tools.  Pass
