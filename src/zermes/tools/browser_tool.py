...
Usage:
    from zermes.tools.browser_tool import browser_navigate, browser_snapshot, browser_click
...
from zermes.agent.auxiliary_client import call_llm
...
try:
    from zermes.tools.website_policy import check_website_access
except Exception:
    check_website_access = lambda url: None  # noqa: E731 — fail-open if policy module unavailable

try:
    from zermes.tools.url_safety import (
...
from zermes.tools.browser_providers.base import CloudBrowserProvider
from zermes.tools.browser_providers.browserbase import BrowserbaseProvider
from zermes.tools.browser_providers.browser_use import BrowserUseProvider
from zermes.tools.browser_providers.firecrawl import FirecrawlProvider
from zermes.tools.tool_backend_helpers import normalize_browser_cloud_provider

# Camofox local anti-detection browser backend (optional).
# When CAMOFOX_URL is set, all browser operations route through the
# camofox REST API instead of the agent-browser CLI.
try:
    from zermes.tools.browser_camofox import is_camofox_mode as _is_camofox_mode
except ImportError:
...
def _get_dialog_policy_config() -> Tuple[str, float]:
    ...
    """
    # Defer imports so browser_tool can be imported in minimal zermes.environments.
    from zermes.tools.browser_supervisor import (
...
def _ensure_cdp_supervisor(task_id: str) -> None:
    ...
    if not cdp_url:
        return
    try:
        from zermes.tools.browser_supervisor import SUPERVISOR_REGISTRY  # type: ignore[import-not-found]
...
def _url_is_private(url: str) -> bool:
    ...
    Reuses ``zermes.tools.url_safety.is_safe_url`` as the oracle — if the SSRF check
...
BROWSER_TOOL_SCHEMAS = [
    {
        ...
        "description": "Navigate to a URL in the browser. Initializes the session and loads the page. Must be called before other browser zermes.tools. For simple information retrieval, prefer web_search or web_extract (faster, cheaper). For plain-text endpoints — URLs ending in .md, .txt, .json, .yaml, .yml, .csv, .xml, raw.githubusercontent.com, or any documented API endpoint — prefer curl via the terminal tool or web_extract; the browser stack is overkill and much slower for these. Use browser tools when you need to interact with a page (click, fill forms, dynamic content). Returns a compact page snapshot with interactive elements and ref IDs — no need to call browser_snapshot separately after navigating.",
        "parameters": {
            "type": "object",
            "properties": {
                "url": {
...
def _extract_relevant_content(
    ...
    if user_task:
        extraction_prompt = (
            f"You are a content extractor for a browser automation zermes.agent.\n\n"
    ...
    # secrets to the extraction model before zermes.run_agent.py's general
    # redaction layer ever sees the tool result.
    from zermes.agent.redact import redact_sensitive_text
...
def browser_navigate(url: str, task_id: Optional[str] = None) -> str:
    ...
    import urllib.parse
    from zermes.agent.redact import _PREFIX_RE
    ...
    if _is_camofox_mode():
        from zermes.tools.browser_camofox import camofox_navigate
...
def browser_snapshot(
    ...
    """
    if _is_camofox_mode():
        from zermes.tools.browser_camofox import camofox_snapshot
...
def browser_click(ref: str, task_id: Optional[str] = None) -> str:
    ...
    """
    if _is_camofox_mode():
        from zermes.tools.browser_camofox import camofox_click
...
def browser_type(ref: str, text: str, task_id: Optional[str] = None) -> str:
    ...
    """
    if _is_camofox_mode():
        from zermes.tools.browser_camofox import camofox_type
...
def browser_scroll(direction: str, task_id: Optional[str] = None) -> str:
    ...
    if _is_camofox_mode():
        from zermes.tools.browser_camofox import camofox_scroll
...
def browser_back(task_id: Optional[str] = None) -> str:
    ...
    """
    if _is_camofox_mode():
        from zermes.tools.browser_camofox import camofox_back
...
def browser_press(key: str, task_id: Optional[str] = None) -> str:
    ...
    """
    if _is_camofox_mode():
        from zermes.tools.browser_camofox import camofox_press
...
def browser_console(clear: bool = False, expression: Optional[str] = None, task_id: Optional[str] = None) -> str:
    ...
    if _is_camofox_mode():
        from zermes.tools.browser_camofox import camofox_console
...
def _camofox_eval(expression: str, task_id: Optional[str] = None) -> str:
    ...
    from zermes.tools.browser_camofox import _ensure_tab, _post
    try:
...
def browser_get_images(task_id: Optional[str] = None) -> str:
    ...
    """
    if _is_camofox_mode():
        from zermes.tools.browser_camofox import camofox_get_images
...
def browser_vision(question: str, annotate: bool = False, task_id: Optional[str] = None) -> str:
    ...
    """
    if _is_camofox_mode():
        from zermes.tools.browser_camofox import camofox_vision
        return camofox_vision(question, annotate, task_id)

    import base64
    import uuid as uuid_mod
    from zermes.hermes_constants import get_hermes_dir
    ...
    try:
        ...
        try:
            ...
            from zermes.tools.vision_tools import (
...
def _cleanup_single_browser_session(task_id: str) -> None:
    ...
    if _is_camofox_mode():
        try:
            from zermes.tools.browser_camofox import camofox_close, camofox_soft_cleanup
...
if __name__ == "__main__":
    ...
    print("\n💡 Usage:")
    print("  from zermes.tools.browser_tool import browser_navigate, browser_snapshot")
...
# Registry
# ---------------------------------------------------------------------------
from zermes.tools.registry import registry, tool_error
