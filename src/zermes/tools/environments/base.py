...
if _DEBUG_INTERRUPT:
    # AIAgent's quiet_mode path (zermes.run_agent.py) forces the `tools` logger to
    ...
    # zermes.agent.log regardless of quiet-mode.  Scoped to the opt-in case only.
    logger.setLevel(logging.INFO)

# Thread-local activity callback.  The agent sets this before a tool call so
# long-running _wait_for_process loops can report liveness to the zermes.gateway.
...
class BaseEnvironment(ABC):
    ...
    def execute(
        ...
        *,
        ...
        from zermes.tools.terminal_tool import _rewrite_compound_background
    ...
    def _prepare_command(self, command: str) -> tuple[str, str | None]:
        ...
        from zermes.tools.terminal_tool import _transform_sudo_command
