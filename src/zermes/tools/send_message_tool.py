...
def _handle_list():
    ...
    try:
        from zermes.gateway.channel_directory import format_directory_for_display
...
def _handle_send(args):
    ...
    if not pconfig or not pconfig.enabled:
        ...
        # send_message and cron delivery work without a zermes.gateway.yaml entry.
    ...
    if not chat_id:
        ...
        if not home and platform_name == "weixin":
            ...
            if wx_home:
                from zermes.gateway.config import HomeChannel
    ...
    try:
        from zermes.model_tools import _run_async
        result = _run_async(
            _send_to_platform(
                platform,
                pconfig,
        ...
        if isinstance(result, dict) and result.get("success") and mirror_text:
            try:
                from zermes.gateway.mirror import mirror_to_session
...
async def _send_via_adapter(platform, pconfig, chat_id, chunk):
    ...
    """
    try:
        from zermes.gateway.run import _gateway_runner_ref
...
async def _send_to_platform(platform, pconfig, chat_id, message, thread_id=None, media_files=None, force_document=False):
    ...
    from zermes.gateway.platforms.base import BasePlatformAdapter, utf16_len
    from zermes.gateway.platforms.discord import DiscordAdapter
    from zermes.gateway.platforms.slack import SlackAdapter
        ...
        if formatted.strip():
            try:
                ...
                if "parse" in str(md_error).lower() or "markdown" in str(md_error).lower() or "html" in str(md_error).lower():
                    ...
                    if not _has_html:
                        try:
                            from zermes.gateway.platforms.telegram import _strip_mdv2
...
async def _send_discord(token, chat_id, message, thread_id=None, media_files=None):
    ...
    try:
        from zermes.gateway.platforms.base import resolve_proxy_url, proxy_kwargs_for_aiohttp
        ...
        if thread_id:
            ...
            _channel_type = None
            try:
                from zermes.gateway.channel_directory import lookup_channel_type
...
async def _send_weixin(pconfig, chat_id, message, media_files=None):
    ...
    try:
        from zermes.gateway.platforms.weixin import check_weixin_requirements, send_weixin_direct
...
async def _send_feishu(pconfig, chat_id, message, media_files=None, thread_id=None):
    ...
    try:
        from zermes.gateway.platforms.feishu import FeishuAdapter, FEISHU_AVAILABLE
        if not FEISHU_AVAILABLE:
            return {"error": "Feishu dependencies not installed. Run: pip install 'hermes-agent[feishu]'"}
        from zermes.gateway.platforms.feishu import FEISHU_DOMAIN, LARK_DOMAIN
    except ImportError:
        return {"error": "Feishu dependencies not installed. Run: pip install 'hermes-agent[feishu]'"}
...
def _check_send_message():
    ...
    if platform and platform != "local":
        return True
    try:
        from zermes.gateway.status import is_gateway_running
...
async def _send_yuanbao(chat_id, message, media_files=None):
    ...
    """
    try:
        from zermes.gateway.platforms.yuanbao import get_active_adapter, send_yuanbao_direct
    except ImportError:
