...
def get_current_session_key(default: str = "default") -> str:
    ...
    Resolution order:
    1. approval-specific contextvars (set by gateway before zermes.agent.run)
...
def _normalize_command_for_detection(command: str) -> str:
    ...
    Strips ANSI escape sequences (full ECMA-48 via zermes.tools.ansi_strip),
...
def prompt_dangerous_approval(command: str, description: str,
    ...
    # error to the zermes.agent. Any thread that needs interactive approval must
    # install a callback via zermes.tools.terminal_tool.set_approval_callback()
    # before reaching this point (see delegate_tool.py, zermes.run_agent.py
...
def _smart_approve(command: str, description: str) -> str:
    ...
    try:
        ...
        prompt = f"""You are a security reviewer for an AI coding zermes.agent. A terminal command was flagged by pattern matching as potentially dangerous.
...
def check_all_command_guards(command: str, env_type: str,
    ...
    try:
        from zermes.tools.tirith_security import check_command_security
    ...
    if is_gateway or is_ask:
        ...
        if notify_cb is not None:
            ...
            # gateway's inactivity watchdog (zermes.agent.gateway_timeout, default
            ...
            try:
                from zermes.tools.environments.base import touch_activity_if_due
