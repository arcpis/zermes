...
self-registers its schema, handler, and metadata via zermes.tools.registry.register().
This module triggers discovery (by importing all tool modules), then provides
the public API that zermes.run_agent.py, zermes.cli.py, zermes.batch_runner.py, and the RL
environments consume.

Public API (signatures preserved from the original 2,400-line version):
    get_tool_definitions(enabled_toolsets, disabled_toolsets, quiet_mode) -> list
    handle_function_call(function_name, function_args, task_id, user_task) -> str
    TOOL_TO_TOOLSET_MAP: dict          (for zermes.batch_runner.py)
    TOOLSET_REQUIREMENTS: dict         (for zermes.cli.py, doctor.py)
...
from zermes.tools.registry import discover_builtin_tools, registry
...
#   - zermes.cli.py, hermes_cli/*      -> inline on startup (no event loop)
...
def get_tool_definitions(
    ...
    Args:
        enabled_toolsets: Only include tools from these zermes.toolsets.
    ...
    if quiet_mode:
        try:
            from zermes.hermes_cli.config import get_config_path
...
def _compute_tool_definitions(
    ...
    if enabled_toolsets is not None:
        ...
        from zermes.toolsets import get_all_toolsets
    ...
    # all check the tool registry for plugin-provided zermes.toolsets.  No bypass
    ...
    if "execute_code" in available_tool_names:
        from zermes.tools.code_execution_tool import SANDBOX_ALLOWED_TOOLS, build_execute_code_schema, _get_execution_mode
    ...
    for discord_tool_name in _discord_schema_fns:
        if discord_tool_name in available_tool_names:
            try:
                from zermes.tools import discord_tool as _dt
    ...
    try:
        from zermes.tools.schema_sanitizer import sanitize_tool_schemas
...
# Tools whose execution is intercepted by the agent loop (zermes.run_agent.py)
...
def handle_function_call(
    ...
    try:
        ...
        # checked — e.g. zermes.run_agent._invoke_tool passes skip=True to
        ...
        if function_name not in _READ_SEARCH_TOOLS:
            try:
                from zermes.tools.file_tools import notify_other_tool_call
