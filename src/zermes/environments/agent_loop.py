...
identical to hermes-agent's zermes.run_agent.py. Tool execution is dispatched via
handle_function_call() from zermes.model_tools.py.
"""
...
from zermes.tools.terminal_tool import get_active_env
...
class HermesAgentLoop:
    ...
    Same pattern as zermes.run_agent.py:
    ...
    def __init__(
        ...
        """
        from zermes.tools.budget_config import DEFAULT_BUDGET
        self.server = server
    ...
    async def run(self, messages: List[Dict[str, Any]]) -> AgentResult:
        ...
        from zermes.tools.todo_tool import TodoStore, todo_tool as _todo_tool
                        ...
                        if args is not None:
                            try:
                                ...
                                if tool_name == "todo":
                                    ...
                                    tool_result = json.dumps({"error": "Memory is not available in RL zermes.environments."})
                                    tool_elapsed = _time.monotonic() - tool_submit_time
                                elif tool_name == "session_search":
                                    tool_result = json.dumps({"error": "Session search is not available in RL zermes.environments."})
                                    tool_elapsed = _time.monotonic() - tool_submit_time
                                else:
