...
class APIServerAdapter(BasePlatformAdapter):
    ...
    def _create_agent(
        ...
        from zermes.gateway.run import _resolve_runtime_agent_kwargs, _resolve_gateway_model, _load_gateway_config, GatewayRunner
    ...
    async def _handle_chat_completions(self, request: "web.Request") -> "web.Response":
        ...
        if stream:
            ...
            def _on_tool_start(tool_call_id, function_name, function_args):
                ...
                from zermes.agent.display import build_tool_preview, get_tool_emoji
    ...
    async def _write_sse_chat_completion(
        ...
        the agent is interrupted via ``zermes.agent.interrupt()`` so it stops making
    ...
    async def _write_sse_responses(
        ...
        If the client disconnects mid-stream, ``zermes.agent.interrupt()`` is
    ...
    async def _run_agent(
        ...
        callers (e.g. the SSE writer) to call ``zermes.agent.interrupt()`` from
    ...
    async def _handle_runs(self, request: "web.Request") -> "web.Response":
        ...
        async def _run_and_close():
            try:
                ...
                def _run_sync():
                    from zermes.gateway.session_context import clear_session_vars, set_session_vars
    ...
    async def _handle_stop_run(self, request: "web.Request") -> "web.Response":
        """POST /v1/runs/{run_id}/stop — interrupt a running zermes.agent."""
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
        ...
        if task is not None and not task.done():
            ...
            # zermes.agent.interrupt() above to break the loop. Cap the wait so a
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            if is_network_accessible(self._host) and self._api_key:
                try:
                    from zermes.hermes_cli.auth import has_usable_secret
