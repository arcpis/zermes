...
def load_gateway_config() -> GatewayConfig:
    ...
    3. ~/.hermes/zermes.gateway.json (legacy — provides defaults under config.yaml)
    4. Built-in defaults
    """
    _home = get_hermes_home()
    gw_data: dict = {}

    # Legacy fallback: zermes.gateway.json provides the base layer.
    # config.yaml keys always win when both specify the same setting.
    gateway_json_path = _home / "zermes.gateway.json"
    ...
    try:
        ...
        if config_yaml_path.exists():
            ...
            # Each key overwrites whatever zermes.gateway.json may have set.
            ...
            if isinstance(yaml_platforms, dict):
                for plat_name, plat_block in yaml_platforms.items():
                    ...
                    if not isinstance(existing, dict):
                        existing = {}
                    # Deep-merge extra dicts so zermes.gateway.json defaults survive
        ...
        logger.warning(
            "Failed to process config.yaml — falling back to .env / zermes.gateway.json values. "
        ...
        for platform, pconfig in config.platforms.items():
            ...
            if token and token.strip() and not has_usable_secret(token, min_length=4):
                logger.error(
                    ...
                    "Set a real bot token before starting the zermes.gateway. "
