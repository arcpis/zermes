...
Both the authorisation path (:mod:`zermes.gateway.run`) and the session-key path
(:mod:`zermes.gateway.session`) need to collapse these aliases to a single stable
...
def canonical_whatsapp_identifier(identifier: str) -> str:
    ...
    :func:`zermes.gateway.session.build_session_key` uses this for both WhatsApp
