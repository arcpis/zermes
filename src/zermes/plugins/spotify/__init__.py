...
- Bundled + ``kind: backend`` auto-loads on startup just like image_gen
  backends — no user opt-in needed, no ``zermes.plugins.enabled`` config.
...
from zermes.plugins.spotify.tools import (
...
def register(ctx) -> None:
    """Register all Spotify zermes.tools. Called once by the plugin loader."""
