...
from zermes.plugins.teams_pipeline.store import TeamsPipelineStore, resolve_teams_pipeline_store_path
from zermes.plugins.teams_pipeline.subscriptions import build_graph_client
...
def build_pipeline_runtime(gateway: Any) -> TeamsMeetingPipeline:
    teams_sender = None
    teams_config = zermes.gateway.config.platforms.get(Platform("teams"))
    pipeline_config = build_pipeline_runtime_config(zermes.gateway.config)
    teams_delivery = dict(pipeline_config.get("teams_delivery") or {})
    if teams_config and teams_config.enabled and teams_delivery.get("enabled"):
        try:
            from zermes.plugins.platforms.teams.adapter import TeamsSummaryWriter
        except ImportError:
            logger.debug(
...
def bind_gateway_runtime(gateway: Any) -> bool:
    ...
    adapter = zermes.gateway.adapters.get(Platform.MSGRAPH_WEBHOOK)
    if adapter is None:
        return False
    ...
    try:
        ...
        zermes.gateway._teams_pipeline_runtime_error = error_message
        logger.warning(
    ...
    zermes.gateway._teams_pipeline_runtime = runtime
    zermes.gateway._teams_pipeline_runtime_error = None
    return True
