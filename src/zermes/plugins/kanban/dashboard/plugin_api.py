...
``zermes.hermes_cli.kanban_db`` or a direct SQL query. Writes use the same code
...
from zermes.hermes_cli import kanban_db
...
def _check_ws_token(provided: Optional[str]) -> bool:
    ...
    if not provided:
        return False
    try:
        from zermes.hermes_cli import web_server as _ws
    except Exception:
...
def _compute_task_diagnostics(
    ...
    Uses ``zermes.hermes_cli.kanban_diagnostics`` — see that module for the
    rule definitions.
    """
    from zermes.hermes_cli import kanban_diagnostics as kd
...
def _warnings_summary_from_diagnostics(
    ...
    if not diagnostics:
        return None
    from zermes.hermes_cli.kanban_diagnostics import SEVERITY_ORDER
...
def create_task(payload: CreateTaskBody, board: Optional[str] = Query(None)):
    ...
    try:
        ...
        if task and task.status == "ready" and task.assignee:
            try:
                from zermes.hermes_cli.kanban import _check_dispatcher_presence
...
# spawn failures, stuck-blocked). See zermes.hermes_cli.kanban_diagnostics for
# the rule engine.
# ---------------------------------------------------------------------------
...
def specify_task_endpoint(
    ...
    try:
        ...
        from zermes.hermes_cli import kanban_specify  # noqa: WPS433 (intentional)
    ...
    if not home:
        raise HTTPException(
            ...
            detail=f"No home channel configured for platform {platform!r}. "
                   ...
                   f"zermes.gateway.platforms.{platform}.home_channel in config.yaml.",
        )
    board = _resolve_board(board)
    conn = _conn(board=board)
    try:
