...
the hermes-agent zermes.gateway. The gateway speaks a small JSON-over-WebSocket
RPC protocol to the remote node; the node wraps the existing
``zermes.plugins.google_meet.process_manager`` API.
...
from zermes.plugins.google_meet.node.protocol import (
    VALID_REQUEST_TYPES,
    decode,
    encode,
    make_error,
