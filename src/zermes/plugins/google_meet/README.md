...
| `meet_bot.py` | Playwright bot subprocess (standalone, `python -m zermes.plugins.google_meet.meet_bot`) |
| `process_manager.py` | local bot lifecycle + `enqueue_say` |
| `zermes.tools.py` | agent-facing tools + node-routing helper |
| `zermes.cli.py` | `hermes meet setup / auth / join / status / transcript / say / stop / node ...` |
...
| `node/zermes.cli.py` | v3: `hermes meet node {run,list,approve,remove,status,ping}` |
