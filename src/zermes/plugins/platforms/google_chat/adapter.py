...
CARD_CLICKED]. Only MESSAGE dispatches to the zermes.agent. ADDED_TO_SPACE caches the
...
# downstream log-monitor that greps for ``zermes.gateway.platforms.google_chat``.
logger = logging.getLogger("zermes.gateway.platforms.google_chat")
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # attribute to ``zermes.gateway.config.Platform`` — bundled platform plugins
        ...
        if not arg:
            ...
            if not client_secret_present:
                await _reply(
                    ...
                    "```\n"
                    "python -m zermes.plugins.platforms.google_chat.oauth "
...
def interactive_setup() -> None:
    ...
    The setup wizard at ``hermes_cli/zermes.gateway.py`` calls this for plugin
    ...
    """
    from zermes.hermes_cli.cli_output import (
        print_info,
        print_success,
        print_warning,
        prompt,
