...
load when listed in ``zermes.plugins.enabled`` (via ``hermes plugins enable
...
def _usage_and_cost(response: Any, *, provider: str, api_mode: str, model: str, base_url: str) -> tuple[dict[str, int], dict[str, float]]:
    ...
    try:
        ...
        if cost.amount_usd is not None:
            ...
            try:
                from zermes.agent.usage_pricing import get_pricing_entry
...
def _finish_trace(task_key: str, *, output: Any = None) -> None:
    ...
    try:
        ...
        for observation in state.zermes.tools.values():
            _end_observation(observation)
...
def on_post_llm_call(*, task_id: str = "", session_id: str = "", provider: str = "", base_url: str = "",
    ...
    if response is not None:
        ...
        try:
            from zermes.agent.usage_pricing import CanonicalUsage, estimate_usage_cost, get_pricing_entry
            from decimal import Decimal
            _ONE_M = Decimal("1000000")
            _cu = CanonicalUsage(
                input_tokens=_input,
...
def on_post_tool_call(*, tool_name: str = "", args: Any = None, result: Any = None,
    ...
    with _STATE_LOCK:
        ...
        if state is None:
            return
        if tool_key:
            observation = state.zermes.tools.pop(tool_key, None)
        elif state.tools:
            _, observation = state.zermes.tools.popitem()
