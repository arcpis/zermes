...
Usage:
    from zermes.plugins.memory import discover_memory_providers, load_memory_provider
...
def discover_memory_providers() -> List[Tuple[str, str, bool]]:
    """Scan bundled and user-installed directories for available zermes.providers.
...
def _load_provider_from_dir(provider_dir: Path) -> Optional["MemoryProvider"]:
    ...
    module_name = f"zermes.plugins.memory.{name}" if _is_bundled else f"_hermes_user_memory.{name}"
    ...
    if module_name in sys.modules:
        ...
        for parent in ("plugins", "zermes.plugins.memory"):
...
def discover_plugin_cli_commands() -> List[dict]:
    ...
    plugin's ``zermes.cli.py``.  Returns a list of at most one dict with
    keys: ``name``, ``help``, ``description``, ``setup_fn``,
    ``handler_fn``.

    This is a lightweight scan — it only imports ``zermes.cli.py``, not the
    ...
    cli_file = plugin_dir / "zermes.cli.py"
    if not cli_file.exists():
        return results

    _is_bundled = _MEMORY_PLUGINS_DIR in plugin_dir.parents or plugin_dir.parent == _MEMORY_PLUGINS_DIR
    module_name = f"zermes.plugins.memory.{active_provider}.cli" if _is_bundled else f"_hermes_user_memory.{active_provider}.cli"
    try:
