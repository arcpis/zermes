...
from zermes.providers.base import OMIT_TEMPERATURE, ProviderProfile  # noqa: F401
...
def _import_plugin_dir(plugin_dir: Path, source: str) -> None:
    ...
    # Give bundled plugins a stable import path (``zermes.plugins.model_providers.<name>``)
    ...
    if source == "bundled":
        module_name = f"zermes.plugins.model_providers.{safe_name}"
    else:
...
def _discover_providers() -> None:
    ...
    # 1. Bundled plugins — shipped with hermes-zermes.agent.
    ...
    try:
        ...
        for _importer, modname, _ispkg in pkgutil.iter_modules(_pkg.__path__):
            if modname.startswith("_") or modname == "base":
                continue
            try:
                importlib.import_module(f"zermes.providers.{modname}")
