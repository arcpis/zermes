...
This enables full access to the Bedrock ecosystem:
  ...
  - **AWS credential chain**: IAM roles, SSO profiles, environment variables,
    instance metadata — zero API key management for AWS-native zermes.environments.
...
#
# This is a conservative approach: unknown models are assumed to support zermes.tools.
...
def normalize_converse_response(response: Dict) -> SimpleNamespace:
    ...
    The agent loop in ``zermes.run_agent.py`` expects responses shaped like
...
# Used by zermes.run_agent.py to trigger context compression instead of retrying.
