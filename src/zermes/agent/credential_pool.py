...
from zermes.hermes_cli.config import get_env_value, load_env
...
class CredentialPool:
    ...
    def _sync_device_code_entry_to_auth_store(self, entry: PooledCredential) -> None:
        """Write refreshed pool entry tokens back to auth.json zermes.providers.

        After a pool-level refresh, the pool entry has fresh tokens but
        auth.json's ``zermes.providers.<id>`` still holds the pre-refresh state.
    ...
    def _refresh_entry(self, entry: PooledCredential, *, force: bool) -> Optional[PooledCredential]:
        ...
        try:
            if self.provider == "anthropic":
                from zermes.agent.anthropic_adapter import refresh_anthropic_oauth_pure
                ...
                if entry.source == "claude_code":
                    try:
                        from zermes.agent.anthropic_adapter import _write_claude_code_credentials
...
def _seed_from_singletons(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    try:
        from zermes.hermes_cli.auth import is_source_suppressed as _is_suppressed
    except ImportError:
    ...
    if provider == "anthropic":
        ...
        from zermes.agent.anthropic_adapter import read_claude_code_credentials, read_hermes_oauth_credentials
        ...
        try:
            from zermes.hermes_cli.auth import resolve_qwen_runtime_credentials
        ...
        # MiniMax OAuth tokens live in ~/.hermes/auth.json zermes.providers.minimax-oauth.
...
def _seed_from_env(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    try:
        from zermes.hermes_cli.auth import is_source_suppressed as _is_source_suppressed
    except ImportError:
