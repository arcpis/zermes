...
It reads ``zermes.agent.image_input_mode`` from config.yaml (``auto`` | ``native``
...
# ``zermes.run_agent._try_shrink_image_parts_in_messages`` to shrink + retry if
...
def _file_to_data_url(path: Path) -> Optional[str]:
    ...
    (``zermes.run_agent._try_shrink_image_parts_in_messages``) shrinks on the
...
def build_native_content_parts(
    ...
    once — see ``zermes.run_agent._try_shrink_image_parts_in_messages``.
