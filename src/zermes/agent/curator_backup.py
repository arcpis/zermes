...
``zermes.cron.jobs.rewrite_skill_refs()``. Without capturing the pre-run state,
...
def _restore_cron_skill_links(snapshot_dir: Path) -> Dict[str, Any]:
    ...
    ``zermes.cron.jobs`` to pick up the same lock + atomic-write path that tick()
    ...
    try:
        from zermes.cron.jobs import load_jobs, save_jobs, _jobs_file_lock
