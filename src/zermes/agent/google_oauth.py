...
``zermes.agent.gemini_cloudcode_adapter`` to talk to ``cloudcode-pa.googleapis.com``
...
for their own open-source gemini-zermes.cli. They are baked into every copy of the
...
is consistent with opencode-gemini-auth and the official Google gemini-zermes.cli.
...
# Regex patterns for fallback scraping from an installed gemini-zermes.cli.
import re as _re
