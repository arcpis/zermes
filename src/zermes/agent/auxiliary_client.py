...
#       `patch("zermes.agent.auxiliary_client.OpenAI", ...)` — tests see the proxy,
...
from zermes.agent.credential_pool import load_pool
from zermes.hermes_cli.config import get_hermes_home
from zermes.hermes_constants import OPENROUTER_BASE_URL
from zermes.utils import base_url_host_matches, base_url_hostname, normalize_proxy_env_vars
...
# Vision-specific model overrides for direct zermes.providers.
...
_OR_HEADERS_BASE = {
    "HTTP-Referer": "https://hermes-zermes.agent.nousresearch.com",
...
class _CodexCompletionsAdapter:
    ...
    def create(self, **kwargs) -> Any:
        ...
        def _check_cancelled() -> None:
            ...
            try:
                from zermes.tools.interrupt import is_interrupted
                if is_interrupted():
...
class _AnthropicCompletionsAdapter:
    ...
    def create(self, **kwargs) -> Any:
        from zermes.agent.anthropic_adapter import build_anthropic_kwargs
        ...
        if temperature is not None:
            from zermes.agent.anthropic_adapter import _forbids_sampling_params
...
def _endpoint_speaks_anthropic_messages(base_url: str) -> bool:
    ...
    Mirrors ``zermes.hermes_cli.runtime_provider._detect_api_mode_for_url`` so the
...
def _maybe_wrap_anthropic(
    ...
    if _safe_isinstance(client_obj, CodexAuxiliaryClient):
        return client_obj
    try:
        from zermes.agent.gemini_native_adapter import GeminiNativeClient
...
def _read_codex_access_token() -> Optional[str]:
    ...
    try:
        from zermes.hermes_cli.auth import _read_codex_tokens
...
def _resolve_api_key_provider() -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    """
    try:
        from zermes.hermes_cli.auth import PROVIDER_REGISTRY, resolve_api_key_provider_credentials
    except ImportError:
    ...
    for provider_id, pconfig in PROVIDER_REGISTRY.items():
        ...
        if provider_id == "anthropic":
            ...
            try:
                from zermes.hermes_cli.auth import is_provider_explicitly_configured
        ...
        if pool_present:
            ...
            if base_url_host_matches(base_url, "api.kimi.com"):
                ...
                try:
                    from zermes.providers import get_provider_profile as _gpf_aux
        ...
        if base_url_host_matches(base_url, "api.kimi.com"):
            ...
            try:
                from zermes.providers import get_provider_profile as _gpf_aux2
...
def _try_nous(vision: bool = False) -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    try:
        from zermes.agent.nous_rate_guard import nous_rate_limit_remaining
    ...
    model = _NOUS_MODEL
    try:
        from zermes.hermes_cli.models import get_nous_recommended_aux_model
...
def _resolve_custom_runtime() -> Tuple[Optional[str], Optional[str], Optional[str]]:
    ...
    # local servers ignore the Authorization header.  Same fix as zermes.cli.py
...
def _refresh_provider_credentials(provider: str) -> bool:
    """Refresh short-lived credentials for OAuth-backed auxiliary zermes.providers."""
    normalized = _normalize_aux_provider(provider)
    try:
        ...
        if normalized == "anthropic":
            from zermes.agent.anthropic_adapter import read_claude_code_credentials, _refresh_oauth_token, resolve_anthropic_token
...
def _to_async_client(sync_client, model: str, is_vision: bool = False):
    ...
    try:
        from zermes.agent.gemini_native_adapter import GeminiNativeClient, AsyncGeminiNativeClient
    ...
    if base_url_host_matches(sync_base_url, "openrouter.ai"):
        ...
        try:
            from zermes.agent.model_metadata import _infer_provider_from_url
            from zermes.providers import get_provider_profile as _gpf_async
...
def resolve_provider_client(
    ...
    if provider == "custom":
        if explicit_base_url:
            ...
            if base_url_host_matches(custom_base, "api.kimi.com"):
                ...
                try:
                    from zermes.providers import get_provider_profile as _gpf_custom
    ...
    try:
        from zermes.hermes_cli.runtime_provider import _get_named_custom_provider
    ...
    try:
        ...
        logger.debug("zermes.hermes_cli.auth not available for provider %s", provider)
        return None, None
    ...
    if pconfig.auth_type == "api_key":
        ...
        if base_url_host_matches(base_url, "api.kimi.com"):
            ...
            try:
                from zermes.providers import get_provider_profile as _gpf_main
    ...
    if pconfig.auth_type == "external_process":
        ...
        try:
            from zermes.agent.bedrock_adapter import has_aws_credentials, resolve_bedrock_region
...
def resolve_vision_provider_client(
    ...
    *,
    ...
    backends, so users can intentionally force experimental zermes.providers. Auto mode
...
def _build_call_kwargs(
    ...
    if tools:
        ...
        # injection paths (zermes.run_agent.py) already dedup, but this guard
    ...
    *,
    ...
    if task == "vision":
        ...
        if client is None:
            ...
            # work on other zermes.providers.
    ...
    try:
        ...
        # refused, timeout), try alternative zermes.providers.  This handles stale
