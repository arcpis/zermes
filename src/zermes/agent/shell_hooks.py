...
* Python plugins and shell hooks compose naturally: both flow through
  :func:`zermes.hermes_cli.plugins.invoke_hook` and its aggregators.  Python
...
def register_from_config(
    ...
    *,
    ...
    ``cfg`` is the full parsed config dict (``zermes.hermes_cli.config.load_config``
...
def _parse_hooks_block(hooks_cfg: Any) -> List[ShellHookSpec]:
    ...
    because a broken hook must not crash the zermes.agent.
    """
    from zermes.hermes_cli.plugins import VALID_HOOKS
...
def _parse_response(event: str, stdout: str) -> Optional[Dict[str, Any]]:
    ...
    :func:`zermes.hermes_cli.plugins.get_pre_tool_call_block_message`.  This is
...
def run_once(
    ...
    ``kwargs`` is the same dict that :func:`zermes.hermes_cli.plugins.invoke_hook`
