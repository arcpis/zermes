...
Hermes seeds its credential pool from many places:
    ...
    device_code   — auth.json zermes.providers.<provider> (nous, openai-codex, ...)
...
Each source has its own reader inside ``zermes.agent.credential_pool._seed_from_*``
...
def _remove_env_source(provider: str, removed) -> RemovalResult:
    ...
    """
    from zermes.hermes_cli.config import get_env_path, remove_env_value
...
def _remove_nous_device_code(provider: str, removed) -> RemovalResult:
    """Nous OAuth lives in auth.json zermes.providers.nous — clear it and suppress.

    We suppress in addition to clearing because nothing else stops the
    user's next `hermes login` run from writing zermes.providers.nous again
...
def _remove_minimax_oauth(provider: str, removed) -> RemovalResult:
    """MiniMax OAuth lives in auth.json zermes.providers.minimax-oauth — clear it.
...
def _register_all_sources() -> None:
    ...
    register(RemovalStep(
        ...
        description="auth.json zermes.providers.nous",
    ))
    register(RemovalStep(
        ...
        description="auth.json zermes.providers.openai-codex + ~/.codex/auth.json",
    ))
    register(RemovalStep(
    ...
    register(RemovalStep(
        ...
        description="auth.json zermes.providers.minimax-oauth",
    ))
    register(RemovalStep(
