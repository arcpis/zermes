...
``zermes.gateway.approval_expired``).  Missing keys fall back to English; if English
is missing too, the key path itself is returned so a broken catalog never
crashes the zermes.agent.

Usage::

    from zermes.agent.i18n import t
    print(t("approval.choose_long"))                       # current lang
    print(t("zermes.gateway.draining", count=3))                  # {count} formatted
...
def reset_language_cache() -> None:
    ...
    Call after :func:`zermes.hermes_cli.config.save_config` if a running process
...
def t(key: str, lang: str | None = None, **format_kwargs: Any) -> str:
    ...
    **format_kwargs
        ``str.format`` substitution arguments (``t("zermes.gateway.drain", count=3)``
