...
try:
    from zermes.hermes_cli.pt_input_extras import install_shift_enter_alias
...
from zermes.agent.usage_pricing import (
    CanonicalUsage,
    estimate_usage_cost,
    format_duration_compact,
    format_token_count_compact,
)
# NOTE: `from zermes.agent.account_usage import ...` is deliberately NOT at module
# top — it transitively pulls the OpenAI SDK chain (~230 ms cold) and is only
# needed when the user runs `/limits`. Lazy-imported inside the handler below.
from zermes.hermes_cli.banner import _format_context_length, format_banner_version_label
...
from zermes.hermes_cli.browser_connect import (
    DEFAULT_BROWSER_CDP_URL,
    manual_chrome_debug_command,
    try_launch_chrome_debug,
)
from zermes.hermes_cli.env_loader import load_hermes_dotenv
from zermes.utils import base_url_host_matches, is_truthy_value
...
def _strip_reasoning_tags(text: str) -> str:
    ...
    ``zermes.run_agent.py::_strip_think_blocks`` and the stream consumer's
...
def load_cli_config() -> Dict[str, Any]:
    ...
    if config_path.exists():
        try:
            ...
            # zermes.agent.max_turns whenever the nested key is missing.
    ...
    # Compression config is read directly from config.yaml by zermes.run_agent.py and
...
# Initialize centralized logging early — zermes.agent.log + errors.log in ~/.hermes/logs/.
...
try:
    from zermes.hermes_cli.skin_engine import init_skin_from_config
...
try:
    from zermes.agent.auxiliary_client import neuter_async_httpx_del
...
from zermes.hermes_cli.banner import build_welcome_banner
from zermes.hermes_cli.commands import SlashCommandCompleter, SlashCommandAutoSuggest
from zermes.toolsets import get_all_toolsets, get_toolset_info, resolve_toolset, validate_toolset

# Cron job system for scheduled tasks (execution is handled by the gateway)
from zermes.cron import get_job

# Resource cleanup imports for safe shutdown (terminal VMs, browser sessions)
from zermes.tools.terminal_tool import cleanup_all_environments as _cleanup_all_terminals
from zermes.tools.terminal_tool import set_sudo_password_callback, set_approval_callback
from zermes.tools.skills_tool import set_secret_capture_callback
from zermes.hermes_cli.callbacks import prompt_for_secret
from zermes.tools.browser_tool import _emergency_cleanup_all_sessions as _cleanup_all_browsers
...
def _run_state_db_auto_maintenance(session_db) -> None:
    ...
    :func:`zermes.hermes_cli.config.load_config` (the authoritative loader that
    ...
    try:
        ...
        from zermes.hermes_constants import get_hermes_home as _get_hermes_home
...
def _toolsets_enable_tool(toolsets: Optional[List[str]], tool_name: str) -> bool:
    ...
    Composite toolsets such as ``hermes-cli`` include other zermes.toolsets.  Checking
...
def _run_checkpoint_auto_maintenance() -> None:
    ...
    :func:`zermes.hermes_cli.config.load_config`. Honours ``auto_prune`` /
...
class _SkinAwareAnsi:
    ...
    def __str__(self) -> str:
        if self._cached is None:
            try:
                from zermes.hermes_cli.skin_engine import get_active_skin
...
from zermes.hermes_constants import is_termux as _is_termux_environment
...
def _resolve_attachment_path(raw_path: str) -> Path | None:
    ...
    # is silently lost (the warning ends up in zermes.agent.log but the user sees
...
def _build_compact_banner() -> str:
    ...
    w = min(shutil.get_terminal_size().columns - 2, 100)
    if w < 30:
        return f"\n[{title_color}]{tiny_line}[/] [dim {dim_color}]- Arcpis[/]\n"
...
def _get_plugin_cmd_handler_names() -> set:
    ...
    try:
        from zermes.hermes_cli.plugins import get_plugin_commands
...
def save_config_value(key_path: str, value: any) -> bool:
    ...
    Args:
        key_path: Dot-separated path like "zermes.agent.system_prompt"
        value: Value to save
    ...
    try:
        ...
        from zermes.utils import atomic_yaml_write
...
class HermesCLI:
    ...
    def __init__(
        ...
        if self.model == _DEFAULT_CONFIG_MODEL:
            ...
            if "localhost" in _base_url or "127.0.0.1" in _base_url:
                from zermes.hermes_cli.runtime_provider import _auto_detect_local_model
    ...
    def set_voice_record_key_cache(self, raw_key: object) -> None:
        ...
        """
        try:
            from zermes.hermes_cli.voice import format_voice_record_key_for_status
    ...
    def _normalize_model_for_provider(self, resolved_provider: str) -> bool:
        ...
        try:
            from zermes.hermes_cli.model_normalize import (
        ...
        if resolved_provider == "copilot":
            try:
                from zermes.hermes_cli.models import copilot_model_api_mode, normalize_copilot_model_id
        ...
        if resolved_provider in {"opencode-zen", "opencode-go"}:
            try:
                from zermes.hermes_cli.models import normalize_opencode_model_id, opencode_model_api_mode
    ...
    def show_banner(self):
        ...
        if ctx_len and ctx_len <= 8192:
            ...
            self._console_print(
                ...
                f"this is likely too low for agent use with zermes.tools.[/]"
            )
            self._console_print(
        ...
        from zermes.hermes_cli.model_switch import is_nous_hermes_non_agentic
    ...
    def _try_attach_clipboard_image(self) -> bool:
        ...
        """
        from zermes.hermes_cli.clipboard import save_clipboard_image
    ...
    def _handle_rollback_command(self, command: str):
        ...
        """
        from zermes.tools.checkpoint_manager import format_checkpoint_list
    ...
    def _handle_snapshot_command(self, command: str):
        ...
        """
        from zermes.hermes_cli.backup import (
    ...
    def _handle_paste_command(self):
        ...
        from zermes.hermes_cli.clipboard import has_clipboard_image
    ...
    def _show_tool_availability_warnings(self):
        ...
        try:
            from zermes.model_tools import check_tool_availability
    ...
    def show_help(self):
        ...
        from zermes.hermes_cli.commands import COMMANDS_BY_CATEGORY

        try:
            from zermes.hermes_cli.skin_engine import get_active_help_header
    ...
    def show_tools(self):
        ...
        # Display by toolset
        for toolset in sorted(zermes.toolsets.keys()):
            print(f"  [{toolset}]")
    ...
    def _show_recent_sessions(self, *, reason: str = "history", limit: int = 10) -> bool:
        ...
        from zermes.hermes_cli.main import _relative_time
    ...
    def _handle_resume_command(self, cmd_original: str) -> None:
        ...
        # Resolve title or ID
        from zermes.hermes_cli.main import _resolve_session_by_name_or_id
    ...
    def retry_last(self):
        ...
        the last user message, then re-sends that user message to the zermes.agent.
    ...
    def _run_curses_picker(self, title: str, items: list[str], default_index: int = 0) -> int | None:
        ...
        import threading
        from zermes.hermes_cli.curses_ui import curses_single_select
    ...
    def _handle_model_picker_selection(self, persist_global: bool = False) -> None:
        ...
        if stage == "model":
            ...
            if selected < len(model_list):
                from zermes.hermes_cli.model_switch import switch_model
    ...
    def _handle_model_switch(self, cmd_original: str):
        ...
        """
        from zermes.hermes_cli.model_switch import switch_model, parse_model_flags, list_authenticated_providers
        ...
        custom_provs = None
        try:
            from zermes.hermes_cli.config import get_compatible_custom_providers, load_config
            cfg = load_config()
    ...
    def _should_handle_model_command_inline(self, text: str, has_images: bool = False) -> bool:
        ...
        if not text or has_images or not _looks_like_slash_command(text):
            return False
        try:
            from zermes.hermes_cli.commands import resolve_command
    ...
    def _should_handle_steer_command_inline(self, text: str, has_images: bool = False) -> bool:
        ...
        zermes.agent.steer() directly, which is thread-safe (uses _pending_steer_lock).
        """
        if not text or has_images or not _looks_like_slash_command(text):
            return False
        if not getattr(self, "_agent_running", False):
    ...
    def _handle_gquota_command(self, cmd_original: str) -> None:
        ...
        try:
            from zermes.agent.google_oauth import get_valid_access_token, GoogleOAuthError, load_credentials
            from zermes.agent.google_code_assist import retrieve_user_quota, CodeAssistError
    ...
    def _handle_personality_command(self, cmd: str):
        ...
        if len(parts) > 1:
            ...
            if personality_name in ("none", "default", "neutral"):
                ...
                if save_config_value("zermes.agent.system_prompt", ""):
                ...
                self.agent = None  # Force re-init
                if save_config_value("zermes.agent.system_prompt", self.system_prompt):
    ...
    def _handle_cron_command(self, cmd: str):
        ...
        import shlex
        from zermes.tools.cronjob_tools import cronjob as cronjob_tool
    ...
    def _handle_curator_command(self, cmd: str):
        ...
        Delegates to zermes.hermes_cli.curator so the CLI and the `hermes curator`
        ...
        try:
            from zermes.hermes_cli.curator import cli_main
            cli_main(tokens)
        except SystemExit:
    ...
    def _handle_skills_command(self, cmd: str):
        """Handle /skills slash command — delegates to zermes.hermes_cli.skills_hub."""
        from zermes.hermes_cli.skills_hub import handle_skills_slash
        handle_skills_slash(cmd, ChatConsole())

    def _show_gateway_status(self):
        """Show status of the gateway and connected messaging platforms."""
        from zermes.gateway.config import load_gateway_config, Platform
    ...
    def process_command(self, command: str) -> bool:
        ...
        from zermes.hermes_cli.commands import resolve_command as _resolve_cmd
        ...
        if canonical in ("quit", "exit"):
            ...
            from zermes.hermes_cli.config import reload_env
            count = reload_env()
            ...
            try:
                from zermes.hermes_cli.plugins import get_plugin_manager
            ...
            # Check for skill slash commands (/gif-search, /axolotl, etc.)
                ...
                from zermes.hermes_cli.commands import COMMANDS
    ...
    def _handle_browser_command(self, cmd: str):
        ...
        if sub.startswith("connect"):
            ...
            try:
                from zermes.tools.browser_tool import _ensure_cdp_supervisor  # type: ignore[import-not-found]
            ...
            if current:
                ...
                try:
                    from zermes.tools.browser_tool import cleanup_all_browsers, _stop_cdp_supervisor
            ...
            if current:
                ...
                try:
                    from zermes.tools.browser_tool import _get_cloud_provider
                ...
                if provider is not None:
                    ...
                    try:
                        from zermes.tools.browser_tool import _get_browser_engine
    ...
    def _handle_skin_command(self, cmd: str):
        ...
        try:
            from zermes.hermes_cli.skin_engine import list_skins, set_active_skin, get_active_skin_name
        except ImportError:
    ...
    def _handle_footer_command(self, cmd_original: str) -> None:
        ...
        """
        from zermes.hermes_cli.config import load_config
        from zermes.hermes_cli.colors import Colors as _Colors
    ...
    def _handle_reasoning_command(self, cmd: str):
        ...
        if save_config_value("zermes.agent.reasoning_effort", arg):
    ...
    def _handle_fast_command(self, cmd: str):
        ...
        try:
            from zermes.hermes_cli.models import _is_anthropic_fast_model
            agent = getattr(self, "agent", None)
            model = getattr(agent, "model", None) or getattr(self, "model", None)
        ...
        if save_config_value("zermes.agent.service_tier", saved_value):
    ...
    def _manual_compress(self, cmd_original: str = ""):
        ...
        with self._busy_command("Compressing context..."):
            try:
                ...
                # session on the agent (zermes.run_agent.py::_compress_context). Sync the
    ...
    def _handle_debug_command(self):
        ...
        from zermes.hermes_cli.debug import run_debug_share
    ...
    def _show_usage(self):
        ...
        if rl_state and rl_state.has_data:
            from zermes.agent.rate_limit_tracker import format_rate_limit_display
            print()
    ...
    def _check_config_mcp_changes(self) -> None:
        ...
        from zermes.hermes_cli.config import get_config_path as _get_config_path
    ...
    def _on_tool_progress(self, event_type: str, function_name: str = None, preview: str = None, function_args: dict = None, **kwargs):
        ...
        if event_type == "tool.completed":
            ...
            if function_name and self.tool_progress_mode in ("all", "new"):
                ...
                try:
                    from zermes.agent.display import get_cute_tool_message
        ...
        if not function_name or function_name.startswith("_"):
            return
        try:
            from zermes.tools.voice_mode import play_beep
            threading.Thread(
                target=play_beep,
    ...
    def _on_tool_start(self, tool_call_id: str, function_name: str, function_args: dict):
        """Capture local before-state for write-capable zermes.tools."""
        try:
            from zermes.agent.display import capture_local_edit_snapshot
    ...
    def _on_tool_complete(self, tool_call_id: str, function_name: str, function_args: dict, function_result: str):
        ...
        try:
            from zermes.agent.display import render_edit_diff_with_delta
    ...
    def _voice_start_recording(self):
        ...
        if getattr(self, '_should_exit', False):
            return
        from zermes.tools.voice_mode import create_audio_recorder, check_voice_requirements
    ...
    def _voice_stop_and_transcribe(self):
        ...
        try:
            ...
            from zermes.tools.voice_mode import transcribe_recording
    ...
    def _voice_speak_response(self, text: str):
        ...
        self._voice_tts_done.clear()
        try:
            from zermes.tools.tts_tool import text_to_speech_tool
            from zermes.tools.voice_mode import play_audio_file
    ...
    def _enable_voice_mode(self):
        ...
        from zermes.tools.voice_mode import check_voice_requirements, detect_audio_environment
    ...
    def _disable_voice_mode(self):
        ...
        try:
            from zermes.tools.voice_mode import stop_playback
            stop_playback()
        except Exception:
            pass
        self._voice_tts_done.set()
    ...
    def _toggle_voice_tts(self):
        ...
        if self._voice_tts:
            from zermes.tools.tts_tool import check_tts_requirements
    ...
    def _show_voice_status(self):
        ...
        from zermes.tools.voice_mode import check_voice_requirements
    ...
    def chat(self, message, images: list = None) -> Optional[str]:
        ...
        if self.agent is None:
            _cprint(f"{_DIM}Initializing zermes.agent...{_RST}")
        ...
        if isinstance(message, str) and "@" in message:
            try:
                from zermes.agent.context_references import preprocess_context_references
        ...
        if isinstance(message, str):
            from zermes.run_agent import _sanitize_surrogates
    ...
    def _print_exit_summary(self):
        ...
        if msg_count > 0:
            ...
            try:
                from zermes.hermes_cli.skin_engine import get_active_goodbye
    ...
    def _get_tui_prompt_symbols(self) -> tuple[str, str]:
        ...
        """
        try:
            from zermes.hermes_cli.skin_engine import get_active_prompt_symbol
    ...
    def _build_tui_style_dict(self) -> dict[str, str]:
        ...
        try:
            from zermes.hermes_cli.skin_engine import get_prompt_toolkit_style_overrides
    ...
    def run(self):
        ...
        try:
            ...
            if not is_seen(self.config, OPENCLAW_RESIDUE_FLAG) and detect_openclaw_residue():
                ...
                try:
                    from zermes.hermes_cli.config import get_config_path as _get_cfg_path_resid
        ...
        try:
            from zermes.tools.computer_use_tool import set_approval_callback as _set_cu_cb
        ...
        def handle_enter(event):
            ...
            handled as commands, not sent as interrupt text to the zermes.agent.
            """
            ...
            if text or has_images:
                ...
                # zermes.agent.steer() is thread-safe (holds _pending_steer_lock).
        ...
        def handle_ctrl_c(event):
            ...
            if self._agent_running and self.agent:
                ...
                print("\n⚡ Interrupting zermes.agent... (press Ctrl+C again to force exit)")
        ...
        try:
            from zermes.hermes_cli.config import load_config
            from zermes.hermes_cli.voice import (
        ...
        try:
            ...
            if sys.platform == "win32":
                def _sigint_absorb(signum, frame):
                    # Absorb silently. Do NOT call zermes.agent.interrupt() here:
        ...
        try:
            ...
            try:
                from zermes.tools.voice_mode import cleanup_temp_recordings
...
def main(
    ...
    if gateway:
        import asyncio
        from zermes.gateway.run import start_gateway
    ...
    if toolsets:
        if isinstance(toolsets, str):
            toolsets_list = [t.strip() for t in zermes.toolsets.split(",")]
    ...
    #
    # Fix: route SIGTERM/SIGHUP through zermes.agent.interrupt() which sets the
