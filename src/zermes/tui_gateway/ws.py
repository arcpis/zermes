...
Reuses :func:`zermes.tui_gateway.server.dispatch` verbatim so every RPC method, every
...
emits a ``zermes.gateway.ready`` event immediately after connection accept, then
...
async def handle_ws(ws: Any) -> None:
    """Run one WebSocket session. Wire-compatible with ``zermes.tui_gateway.entry``."""
    await ws.accept()
