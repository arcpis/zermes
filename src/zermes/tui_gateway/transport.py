...
can be driven over stdio (``zermes.tui_gateway.entry``) or WebSocket
(``zermes.tui_gateway.ws``) without duplicating code.
...
----------------------
``zermes.tui_gateway.server.write_json`` still works without any transport bound.
...
# will hang waiting for ``zermes.gateway.ready``.  Default stays off so the
