...
from zermes.tui_gateway.server import _CRASH_LOG, dispatch, resolve_skin, write_json
from zermes.tui_gateway.transport import TeeTransport
...
def _install_sidecar_publisher() -> None:
    ...
    from zermes.tui_gateway.event_publisher import WsPublisherTransport
...
# with hasattr so ``python -m zermes.tui_gateway.entry`` (spawned by
...
def main():
    ...
    # a zermes.model_tools.py module-level side effect; moved to explicit
    # startup calls to avoid freezing the gateway's loop on lazy import
    # (#16856).
    #
    # Cold-start guard: importing ``zermes.tools.mcp_tool`` transitively pulls the
    # full MCP SDK (mcp, pydantic, httpx, jsonschema, starlette parsers —
    # ~200ms on macOS), which runs on the TUI's critical path before
    # ``zermes.gateway.ready`` can be emitted.  The overwhelming majority of users
    ...
    if not write_json({
        "jsonrpc": "2.0",
        "method": "event",
        "params": {"type": "zermes.gateway.ready", "payload": {"skin": resolve_skin()}},
    }):
