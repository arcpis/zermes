...
from zermes.utils import is_truthy_value
from zermes.tui_gateway.transport import (
    StdioTransport,
    Transport,
    bind_transport,
    current_transport,
...
# flushes through the stderr->zermes.gateway.stderr event pump. This hook
...
def _panic_hook(exc_type, exc_value, exc_tb):
    ...
    try:
        ...
        pass
    # Stderr goes through to the TUI as a zermes.gateway.stderr Activity line —
...
# to minutes (slash.exec, zermes.cli.exec, shell.exec, session.resume,
...
# so stray print() from libraries/tools becomes harmless zermes.gateway.stderr instead
...
class _SlashWorker:
    ...
    def __init__(self, session_key: str, model: str):
        ...
        argv = [
            sys.executable,
            "-m",
            "zermes.tui_gateway.slash_worker",
            "--session-key",
            session_key,
        ]
        if model:
...
def _finalize_session(session: dict | None, end_reason: str = "tui_close") -> None:
    ...
    # Use session_id (from zermes.agent.session_id) not session_key — after compression,
...
def dispatch(req: dict, transport: Optional[Transport] = None) -> dict | None:
    ...
    *transport* (optional): pins every write produced by this request —
    ...
    the original behaviour for ``zermes.tui_gateway.entry``.
    """
...
def _start_agent_build(sid: str, session: dict) -> None:
    ...
    def _build() -> None:
        ...
        try:
            ...
            # pending_title applied post-first-message (see zermes.cli.exec handler).
            current["agent"] = agent
            ...
            if _sessions.get(sid) is not current:
                ...
                if notify_registered:
                    try:
                        from zermes.tools.approval import unregister_gateway_notify
...
def resolve_skin() -> dict:
    try:
        from zermes.hermes_cli.skin_engine import init_skin_from_config, get_active_skin
...
def _resolve_startup_runtime() -> tuple[str, str | None]:
    ...
    try:
        from zermes.hermes_cli.models import detect_static_provider_for_model
...
def _load_enabled_toolsets() -> list[str] | None:
    ...
    try:
        from zermes.toolsets import validate_toolset
    except Exception:
    ...
    if explicit and validate_toolset is not None:
        ...
        try:
            ...
            from zermes.hermes_cli.tools_config import _parse_enabled_flag
...
def _apply_model_switch(sid: str, session: dict, raw_input: str) -> dict:
    from zermes.hermes_cli.model_switch import parse_model_flags, switch_model
...
def _sync_session_key_after_compress(
    ...
    *,
    ...
    a new continuation session, rotating ``zermes.agent.session_id``.  The TUI
...
def _probe_config_health(cfg: dict) -> str:
    ...
    if isinstance(display_cfg, dict):
        ...
        if (
            ...
            warnings.append(
                "`display.personality` is set but `zermes.agent.personalities` is empty/null; "
...
def _session_info(agent) -> dict:
    ...
    try:
        ...
        pass
    try:
        from zermes.model_tools import get_toolset_for_tool
        ...
        pass
    try:
        from zermes.hermes_cli.banner import get_available_skills

        info["skills"] = get_available_skills()
    except Exception:
        pass
    try:
        from zermes.tools.mcp_tool import get_mcp_status
    ...
    try:
        ...
        pass
    try:
        from zermes.hermes_cli.banner import get_update_result
...
def _tool_ctx(name: str, args: dict) -> str:
    try:
        from zermes.agent.display import build_tool_preview
...
def _wire_callbacks(sid: str):
    from zermes.tools.terminal_tool import set_sudo_password_callback
    ...
    def secret_cb(env_var, prompt, metadata=None):
        ...
        from zermes.hermes_cli.config import save_env_value_secure
...
def _available_personalities(cfg: dict | None = None) -> dict:
    try:
        ...
        try:
            from zermes.hermes_cli.config import load_config as _load_full_cfg
...
def _make_agent(sid: str, key: str, session_id: str | None = None):
    ...
    if startup_skills:
        from zermes.agent.skill_commands import build_preloaded_skills_prompt
...
def _init_session(sid: str, key: str, agent, history: list, cols: int = 80):
    ...
    try:
        from zermes.tools.approval import register_gateway_notify, load_permanent_allowlist
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from zermes.tools.approval import resolve_gateway_approval
...
def _(rid, params: dict) -> dict:
    from zermes.tools.delegate_tool import set_spawn_paused
...
def _(rid, params: dict) -> dict:
    from zermes.tools.delegate_tool import interrupt_subagent
        ...
        try:
            ...
            if images:
                try:
                    ...
                    from zermes.hermes_cli.config import load_config as _tui_load_config
            ...
            if isinstance(result, dict):
                ...
                # If auto-compression fired inside run_conversation(), zermes.agent.session_id
                ...
                # turn to Ink. Mirrors classic CLI behavior at zermes.cli.py where
            ...
            # (zermes.cli.py:_voice_speak_response).  Only the final text — tool
            ...
            if (
                ...
                try:
                    from zermes.hermes_cli.voice import speak_text
                    ...
                    logger.warning("voice TTS skipped: zermes.hermes_cli.voice unavailable")
                except Exception as e:
...
def _(rid, params: dict) -> dict:
    ...
    if err:
        return err
    try:
        from zermes.hermes_cli.clipboard import has_clipboard_image, save_clipboard_image
    except Exception as e:
...
def _(rid, params: dict) -> dict:
    ...
    if key == "model":
        try:
            ...
            if session:
                # Reject during an in-flight turn.  zermes.agent.switch_model()
                # mutates self.model / self.provider / self.base_url /
                # self.client in place; the worker thread running
                # zermes.agent.run_conversation is reading those on every
    ...
    if key == "fast":
        ...
        _write_config_key("zermes.agent.service_tier", nv)
        if agent is not None:
    ...
    if key == "reasoning":
        try:
            ...
            _write_config_key("zermes.agent.reasoning_effort", arg)
    ...
    if key in ("prompt", "personality", "skin"):
        try:
            ...
            if key == "prompt":
                ...
                _write_config_key("zermes.agent.system_prompt", new_prompt)
...
def _(rid, params: dict) -> dict:
    ...
    if key == "provider":
        try:
            from zermes.hermes_cli.models import list_available_providers, normalize_provider
...
def _(rid, params: dict) -> dict:
    try:
        from zermes.hermes_cli.main import _has_any_provider_configured
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        from zermes.tools.mcp_tool import shutdown_mcp_servers, discover_mcp_tools
        ...
        if bool(params.get("always", False)):
            try:
                from zermes.cli import save_config_value as _save_cfg
...
def _(rid, params: dict) -> dict:
    ...
    ``zermes.hermes_cli.config.reload_env``, matching classic CLI's ``/reload``
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        skill_count = 0
        try:
            from zermes.agent.skill_commands import scan_skill_commands
...
@method("zermes.cli.exec")
def _(rid, params: dict) -> dict:
    """Run `python -m zermes.hermes_cli.main` with argv; capture stdout/stderr (non-interactive only)."""
    ...
    try:
        r = subprocess.run(
            [sys.executable, "-m", "zermes.hermes_cli.main", *argv],
            capture_output=True,
            text=True,
        ...
        return _err(rid, 5016, "zermes.cli.exec: timeout")
    except Exception as e:
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from zermes.hermes_cli.commands import SlashCommandCompleter
...
def _(rid, params: dict) -> dict:
    try:
        ...
        from zermes.hermes_cli.auth import PROVIDER_REGISTRY as _auth_reg
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        from zermes.hermes_cli.config import is_managed, save_env_value
        from zermes.hermes_cli.model_switch import list_authenticated_providers
...
def _(rid, params: dict) -> dict:
    ...
    """
    try:
        from zermes.hermes_cli.auth import PROVIDER_REGISTRY, clear_provider_auth
        from zermes.hermes_cli.config import remove_env_value
...
def _mirror_slash_side_effects(sid: str, session: dict, command: str) -> str:
    """Apply side effects that must also hit the gateway's live zermes.agent."""
    ...
    # worker thread running zermes.agent.run_conversation is using.  Parity
...
def _voice_mode_enabled() -> bool:
    ...
    zermes.cli.py initialises ``_voice_mode = False`` at startup and only flips
...
def _(rid, params: dict) -> dict:
    ...
    * ``on`` / ``off`` — flip voice *mode* (the umbrella bit). Turning it
      ...
      (Ctrl+B) after mode is on, matching zermes.cli.py's enable/Ctrl+B split.
    * ``tts`` — toggle speech-output of agent replies. Requires mode on
    ...
    if action in ("on", "off"):
        ...
        if not enabled:
            ...
            try:
                from zermes.hermes_cli.voice import stop_continuous
...
def _(rid, params: dict) -> dict:
    ...
    try:
        if action == "start":
            ...
            from zermes.hermes_cli.voice import start_continuous
...
def _resolve_browser_cdp_url() -> str:
    ...
    ``zermes.tools.browser_tool._get_cdp_override`` would invoke
...
def _failure_messages(url: str, port: int, system: str) -> list[str]:
    from zermes.hermes_cli.browser_connect import manual_chrome_debug_command
...
def _browser_connect(rid, params: dict) -> dict:
    ...
    from zermes.hermes_cli.browser_connect import DEFAULT_BROWSER_CDP_URL
    ...
    try:
        ...
        if parsed.scheme in {"ws", "wss"} and parsed.path.startswith(
            ...
            if not ok and _is_default_local_cdp(parsed):
                from zermes.hermes_cli.browser_connect import try_launch_chrome_debug
...
@method("zermes.tools.show")
def _(rid, params: dict) -> dict:
    try:
        from zermes.model_tools import get_toolset_for_tool, get_tool_definitions
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        from zermes.hermes_cli.tools_config import (
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        if action == "install":
            from zermes.hermes_cli.skills_hub import do_install
        ...
        if action == "browse":
            from zermes.hermes_cli.skills_hub import browse_skills
        ...
        if action == "inspect":
            from zermes.hermes_cli.skills_hub import inspect_skill
...
def _(rid, params: dict) -> dict:
    ...
    if not cmd:
        return _err(rid, 4004, "empty command")
    try:
        from zermes.tools.approval import detect_dangerous_command
