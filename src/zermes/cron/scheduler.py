...
from zermes.hermes_cli.config import load_config, _expand_env_vars
...
class CronPromptInjectionBlocked(Exception):
    ...
    non-interactive (auto-approve) cron zermes.agent.
    """
...
def _resolve_cron_enabled_toolsets(job: dict, cfg: dict) -> list[str] | None:
    ...
    if per_job:
        return per_job
    try:
        from zermes.hermes_cli.tools_config import _get_platform_tools  # lazy: avoid heavy import at cron module load
...
from zermes.cron.jobs import get_due_jobs, mark_job_run, save_job_output, advance_next_run
...
def _resolve_single_delivery_target(job: dict, deliver_value: str) -> Optional[dict]:
    ...
    if ":" in deliver_value:
        ...
        from zermes.tools.send_message_tool import _parse_target_ref
        ...
        try:
            from zermes.gateway.channel_directory import resolve_channel_name
...
# Media extension sets — audio routing is centralized in zermes.gateway.platforms.base
...
def _deliver_result(job: dict, content: str, adapters=None, loop=None) -> Optional[str]:
    ...
    from zermes.tools.send_message_tool import _send_to_platform
    from zermes.gateway.config import load_gateway_config, Platform

    # Optionally wrap the content with a header/footer so the user knows this
    # is a cron delivery.  Wrapping is on by default; set zermes.cron.wrap_response: false
...
def _build_job_prompt(job: dict, prerun_script: Optional[tuple] = None) -> str:
    ...
    if context_from:
        from zermes.cron.jobs import OUTPUT_DIR
    ...
    from zermes.tools.skills_tool import skill_view
    from zermes.tools.skill_usage import bump_use
...
def _scan_assembled_cron_prompt(assembled: str, job: dict) -> str:
    ...
    """
    from zermes.tools.cronjob_tools import _scan_cron_prompt
...
def run_job(job: dict) -> tuple[bool, str, str, Optional[str]]:
    ...
    from zermes.gateway.session_context import set_session_vars, clear_session_vars, _VAR_MAP
    ...
    try:
        ...
        if not isinstance(result, dict):
            raise RuntimeError(
                f"zermes.agent.run_conversation returned {type(result).__name__} instead of dict: {result!r}"
            )
...
"""
        ...
        # main OpenAI/httpx client held by this ephemeral cron zermes.agent. Without
...
def tick(verbose: bool = True, adapters=None, loop=None) -> int:
    ...
    try:
        ...
        # zermes.tools.mcp_tool._run_stdio's finally block are reaped.
        try:
            from zermes.tools.mcp_tool import _kill_orphaned_mcp_children
