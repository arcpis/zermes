...
def _translate_acp_cwd(cwd: str) -> str:
    ...
    """
    from zermes.hermes_constants import is_wsl
...
def _register_task_cwd(task_id: str, cwd: str) -> None:
    """Bind a task/session id to the editor's working directory for zermes.tools.
...
def _clear_task_cwd(task_id: str) -> None:
    ...
    if not task_id:
        return
    try:
        from zermes.tools.terminal_tool import clear_task_env_overrides
...
@dataclass
class SessionState:
    """Tracks per-session state for an ACP-managed Hermes zermes.agent."""
