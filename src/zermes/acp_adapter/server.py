...
from zermes.acp_adapter.auth import detect_provider
from zermes.acp_adapter.events import (
    make_message_cb,
    make_step_cb,
    make_thinking_cb,
    make_tool_progress_cb,
)
from zermes.acp_adapter.permissions import make_approval_callback
from zermes.acp_adapter.session import SessionManager, SessionState, _expand_acp_enabled_toolsets
from zermes.acp_adapter.tools import build_tool_complete, build_tool_start

logger = logging.getLogger(__name__)

try:
    from zermes.hermes_cli import __version__ as HERMES_VERSION
except Exception:
...
class HermesACPAgent(acp.Agent):
    ...
    def _build_model_state(self, state: SessionState) -> SessionModelState | None:
        ...
        try:
            from zermes.hermes_cli.models import curated_models_for_provider, normalize_provider, provider_label
    ...
    def _resolve_model_selection(raw_model: str, current_provider: str) -> tuple[str, str]:
        ...
        try:
            from zermes.hermes_cli.models import detect_provider_for_model, parse_model_input
    ...
    async def prompt(
        ...
        **kwargs: Any,
        ...
        def _run_agent() -> dict:
            ...
            # (e.g. the interactive sudo password cache in zermes.tools.terminal_tool)
            ...
            if approval_cb:
                try:
                    from zermes.tools import terminal_tool as _terminal_tool
                    previous_approval_cb = _terminal_tool._get_approval_callback()
                    _terminal_tool.set_approval_callback(approval_cb)
                except Exception:
                    logger.debug("Could not set ACP approval callback", exc_info=True)
            # Signal to zermes.tools.approval that we have an interactive callback
    ...
    def _cmd_context(self, args: str, state: SessionState) -> str:
        ...
        if getattr(agent, "compression_enabled", True) is False:
            lines.append("Compression is disabled for this zermes.agent.")
        else:
    ...
    def _cmd_compact(self, args: str, state: SessionState) -> str:
        ...
        try:
            ...
            if not getattr(agent, "compression_enabled", True):
                return "Context compression is disabled for this zermes.agent."
            if not hasattr(agent, "_compress_context"):
                return "Context compression not available for this zermes.agent."
