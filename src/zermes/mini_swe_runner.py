...
compatible with zermes.batch_runner.py and zermes.trajectory_compressor.py.
...
Usage:
    ...
    python zermes.mini_swe_runner.py --task "Create a hello world Python script" --env local
    
    # Run with Docker
    python zermes.mini_swe_runner.py --task "List files in /tmp" --env docker --image python:3.11-slim
    
    # Run with Modal (cloud)
    python zermes.mini_swe_runner.py --task "Install numpy and test it" --env modal --image python:3.11-slim
    
    # Batch mode from JSONL file
    python zermes.mini_swe_runner.py --prompts_file prompts.jsonl --output_file trajectories.jsonl --env docker
"""
...
def create_environment(
    ...
    **kwargs
    ...
    if env_type == "local":
        ...
        from zermes.tools.environments.docker import DockerEnvironment
        return DockerEnvironment(image=image, cwd=cwd, timeout=timeout, **kwargs)
    
    elif env_type == "modal":
        from zermes.tools.environments.modal import ModalEnvironment
...
class MiniSWERunner:
    ...
    def _convert_to_hermes_format(
        ...
        This produces the exact format used by zermes.batch_runner.py.
        """
        trajectory = []
...
def main(
    ...
    Examples:
        ...
        python zermes.mini_swe_runner.py --task "Create hello.py that prints Hello World"
        
        # Single task with Docker
        python zermes.mini_swe_runner.py --task "List files" --env docker
        
        # Batch from file
        python zermes.mini_swe_runner.py --prompts_file tasks.jsonl --output_file results.jsonl
    """
    ...
    if task:
        ...
        print("   Example: python zermes.mini_swe_runner.py --task 'Create a hello world script'")
