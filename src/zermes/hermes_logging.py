...
Log files produced:
    zermes.agent.log   — INFO+, all agent/tool/session activity (the main log)
    errors.log  — WARNING+, errors and warnings only (quick triage)
    zermes.gateway.log — INFO+, gateway-only events (created when mode="gateway")

All files use ``RotatingFileHandler`` with ``RedactingFormatter`` so
secrets are never written to disk.

Component separation:
    zermes.gateway.log only receives records from ``zermes.gateway.*`` loggers —
    platform adapters, session management, slash commands, delivery.
    zermes.agent.log remains the catch-all (everything goes there).
...
from zermes.hermes_constants import get_config_path, get_hermes_home
...
class _ComponentFilter(logging.Filter):
    ...
    Used to route gateway-specific records to ``zermes.gateway.log`` while
    keeping ``zermes.agent.log`` as the catch-all.
    """
...
def setup_logging(
    ...
    *force* is ``True``.
    ...
    log_level
        Minimum level for the ``zermes.agent.log`` file handler.  Accepts any
    ...
    mode
        ...
        When ``"gateway"``, an additional ``zermes.gateway.log`` file is created
    ...
    # --- zermes.agent.log (INFO+) — the main activity log -------------------------
    _add_rotating_handler(
        root,
        log_dir / "zermes.agent.log",
        level=level,
        max_bytes=max_bytes,
    ...
    # --- zermes.gateway.log (INFO+, gateway component only) ------------------------
    if mode == "gateway":
        _add_rotating_handler(
            root,
            log_dir / "zermes.gateway.log",
            level=logging.INFO,
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    def __init__(self, *args, **kwargs):
        from zermes.hermes_cli.config import is_managed
