...
def get_repo_root() -> Path:
    """Return the source checkout root when running from a repository tree."""
    return Path(__file__).resolve().parents[2]


def get_repo_resource_path(*parts: str, legacy_parts: tuple[str, ...] | None = None) -> Path:
    """Return a path under the repository ``resources`` directory.

    During source-tree migrations this also supports a legacy fallback so an
    editable checkout remains usable if a caller runs against an older tree.
    """
    root = get_repo_root()
    candidate = root / "resources" / Path(*parts)
    if candidate.exists() or legacy_parts is None:
        return candidate
    legacy = root / Path(*legacy_parts)
    if legacy.exists():
        return legacy
    return candidate
...
def get_optional_skills_dir(default: Path | None = None) -> Path:
    ...
    Packaged installs may ship ``resources/skills/optional`` outside the Python package
    ...
    if default is not None:
        return default
    repo_optional = get_repo_resource_path(
        "skills",
        "optional",
        legacy_parts=("optional-skills",),
    )
    if repo_optional.exists():
        return repo_optional
...
def get_config_path() -> Path:
    ...
    in 7+ files (skill_utils.py, zermes.hermes_logging.py, zermes.hermes_time.py, etc.).
    """
...
AI_GATEWAY_BASE_URL = "https://ai-zermes.gateway.vercel.sh/v1"
