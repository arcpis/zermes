...
- **Managed Worker Agents** — Zermes includes backend contracts and services for long-lived governed `WorkerAgent` identities: profiles, registry lifecycle, organization trees, department assets, runtime adapter boundaries, task-scoped temporary subagents, result routing, organization evolution proposals, and management read models. This is currently a backend/service foundation rather than a full end-user management UI.
...
- **Governed Self-Evolution** — Repository improvement requests can flow through `complete_code_task`, approval records, dedicated task branches, explicit-file commits, verification records, advisory thinking reports, and repository-local analysis summaries.
