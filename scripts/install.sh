...
# Configuration
REPO_URL_SSH="git@github.com:arcpis/zerme.git"
REPO_URL_HTTPS="https://github.com/arcpis/zerme.git"
...
#   code at /usr/local/lib/zermes, command at /usr/local/bin/hermes,
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "                   default (non-root):  ~/.hermes/zermes"
            echo "                   default (root, Linux): /usr/local/lib/zermes"
            echo "  --hermes-home PATH  Data directory (default: ~/.hermes, or \$HERMES_HOME)"
            echo "  -h, --help     Show this help"
            echo ""
            echo "Notes:"
            echo "  When running as root on Linux, Hermes installs the code under"
            echo "  /usr/local/lib/zermes and links the command into"
            echo "  /usr/local/bin/hermes (FHS layout — matches Claude Code / Codex CLI)."
            echo "  Data, config, sessions, and logs still live in \$HERMES_HOME"
            echo "  (default /root/.hermes).  This keeps Docker bind-mounted volumes"
            echo "  small and ensures the command is on PATH for all shells."
            echo "  Existing installs at \$HERMES_HOME/zermes are preserved in-place."
            exit 0
            ;;
        *)
...
#
# Defaults:
#   - Non-root, any OS:       INSTALL_DIR = $HERMES_HOME/zermes
#                             command link in $HOME/.local/bin
#   - Termux (any uid):       INSTALL_DIR = $HERMES_HOME/zermes
#                             command link in $PREFIX/bin (already on PATH)
#   - Root on Linux (new):    INSTALL_DIR = /usr/local/lib/zermes
#                             command link in /usr/local/bin
#                             (unless a legacy install already exists at
#                              $HERMES_HOME/zermes — then preserve it)
#
...
resolve_install_layout() {
    ...
    if is_termux; then
        INSTALL_DIR="$HERMES_HOME/zermes"
        return 0
    fi
    ...
    if [ "$OS" = "linux" ] && [ "$(id -u)" -eq 0 ]; then
        if [ -d "$HERMES_HOME/zermes/.git" ]; then
            INSTALL_DIR="$HERMES_HOME/zermes"
            log_info "Existing install detected at $INSTALL_DIR — keeping legacy layout"
            log_info "  (new root installs use /usr/local/lib/zermes)"
            return 0
        fi
        INSTALL_DIR="/usr/local/lib/zermes"
...
detect_os() {
    case "$(uname -s)" in
        ...
        CYGWIN*|MINGW*|MSYS*)
            ...
            log_info "  irm https://raw.githubusercontent.com/arcpis/zerme/main/scripts/install.ps1 | iex"
            exit 1
            ;;
        *)
            OS="unknown"
...
install_deps() {
    ...
    if [ "$DISTRO" = "termux" ]; then
        ...
        if ! "$PIP_PYTHON" -m pip install -e '.[termux-all]' -c docs/platforms/termux/constraints.txt; then
            log_warn "Termux broad profile (.[termux-all]) failed, trying baseline Termux profile..."
            if ! "$PIP_PYTHON" -m pip install -e '.[termux]' -c docs/platforms/termux/constraints.txt; then
                log_warn "Termux baseline profile (.[termux]) failed, trying base install..."
                if ! "$PIP_PYTHON" -m pip install -e '.' -c docs/platforms/termux/constraints.txt; then
                    ...
                    log_info "Then re-run: cd $INSTALL_DIR && python -m pip install -e '.[termux-all]' -c docs/platforms/termux/constraints.txt"
                    exit 1
                fi
            fi
        fi
...
setup_path() {
    ...
    if [ ! -x "$HERMES_BIN" ]; then
        ...
        if [ "$DISTRO" = "termux" ]; then
            log_info "Try: cd $INSTALL_DIR && python -m pip install -e '.[termux-all]' -c docs/platforms/termux/constraints.txt"
        else
