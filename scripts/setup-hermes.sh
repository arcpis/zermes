...
if is_termux; then
    ...
    if [ -f "docs/platforms/termux/constraints.txt" ]; then
        "$SETUP_PYTHON" -m pip install -e ".[termux]" -c docs/platforms/termux/constraints.txt || {
            echo -e "${YELLOW}⚠${NC} Termux bundle install failed, falling back to base install..."
            "$SETUP_PYTHON" -m pip install -e "." -c docs/platforms/termux/constraints.txt
        }
    else
...
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    ...
    if [ -d "$SCRIPT_DIR/resources/skills/bundled" ]; then
        cp -rn "$SCRIPT_DIR/resources/skills/bundled/"* "$HERMES_SKILLS_DIR/" 2>/dev/null || true
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m zermes.hermes_cli.main setup
fi
