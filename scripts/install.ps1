...
param(
    ...
    [string]$InstallDir = "$env:LOCALAPPDATA\hermes\zermes"
)
...
$RepoUrlSsh = "git@github.com:arcpis/zerme.git"
$RepoUrlHttps = "https://github.com/arcpis/zerme.git"
$PythonVersion = "3.11"
$NodeVersion = "22"
...
function Install-Repository {
    ...
    if (-not $didUpdate) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                $zipUrl = "https://github.com/arcpis/zerme/archive/refs/heads/$Branch.zip"
                $zipPath = "$env:TEMP\zermes-$Branch.zip"
                $extractPath = "$env:TEMP\zermes-extract"
...
function Install-Dependencies {
    ...
    if (-not $installed) {
        throw "Failed to install zermes-agent package even with no extras. Inspect the uv pip install output above."
    }
...
try {
    ...
    Write-Host "  Invoke-WebRequest -Uri 'https://raw.githubusercontent.com/arcpis/zerme/main/scripts/install.ps1' -OutFile install.ps1" -ForegroundColor Yellow
