#!/usr/bin/env python3
"""Generate per-skill Docusaurus pages from resources/skills/bundled/ and resources/skills/optional/ SKILL.md files.
...
REPO = Path(__file__).resolve().parent.parent.parent.parent  # zermes repo root
DOCS = REPO / "apps" / "website" / "docs"
SKILLS_PAGES = DOCS / "user-guide" / "skills"

SKILL_SOURCES = [
    ("bundled", REPO / "resources" / "skills" / "bundled"),
    ("optional", REPO / "resources" / "skills" / "optional"),
]
...
def rewrite_relative_links(body: str, meta: dict[str, Any]) -> str:
    ...
    """
    source_dir = "resources/skills/bundled" if meta["source_kind"] == "bundled" else "resources/skills/optional"
...
def derive_skill_meta(skill_path: Path, source_dir: Path, source_kind: str) -> dict[str, Any]:
    ...
    resources/skills/bundled/<cat>/<skill>/SKILL.md           -> cat=<cat>, slug=<skill>
    resources/skills/bundled/<cat>/<sub>/<skill>/SKILL.md     -> cat=<cat>, sub=<sub>, slug=<skill>
    resources/skills/optional/<cat>/<skill>/SKILL.md          -> cat=<cat>, slug=<skill>
    """
...
def render_skill_page(
    ...
    return (
        ...
        "---\n"
        "\n"
        "{/* This page is auto-generated from the skill's SKILL.md by apps/website/scripts/generate-skill-docs.py. Edit the source SKILL.md, not this page. */}\n"
        "\n"
        f"# {display_name}\n"
        "\n"
...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        "",
        "If a skill is missing from this list but present in the repo, the catalog is regenerated by `apps/website/scripts/generate-skill-docs.py`.",
        "",
    ]
...
def build_catalog_md_optional(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        "",
        "Optional skills ship with hermes-agent under `resources/skills/optional/` but are **not active by default**. Install them explicitly:",
        "",
        "```bash",
    ...
    lines.extend(
        [
            ...
            "",
            "1. Create a directory under `resources/skills/optional/<category>/<skill-name>/`",
...
def write_sidebar(entries):
    ...
    sidebar_path = REPO / "apps" / "website" / "sidebars.ts"
