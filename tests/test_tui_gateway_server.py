...
def test_load_enabled_toolsets_rejects_disabled_mcp_env(monkeypatch, capsys):
    ...
    # whose tools live in hermes-cli's universe (see zermes.toolsets.py).
...
def test_status_callback_emits_kind_and_text():
    with patch("zermes.tui_gateway.server._emit") as emit:
...
def test_startup_runtime_does_not_treat_inference_provider_as_explicit(monkeypatch):
    ...
    monkeypatch.setattr(
        "zermes.hermes_cli.models.detect_static_provider_for_model",
...
def test_startup_runtime_detects_provider_for_model_env(monkeypatch):
    ...
    monkeypatch.setattr(
        "zermes.hermes_cli.models.detect_static_provider_for_model", fake_detect
    )
...
def test_startup_runtime_resolves_short_alias_without_network(monkeypatch):
    ...
    monkeypatch.setattr(server, "_load_cfg", lambda: {"model": {"provider": "auto"}})
    monkeypatch.setattr(
        "zermes.hermes_cli.models.fetch_openrouter_models",
...
def test_startup_runtime_does_not_call_network_detector(monkeypatch):
    ...
    monkeypatch.setattr(server, "_load_cfg", lambda: {"model": {"provider": "auto"}})
    monkeypatch.setattr(
        "zermes.hermes_cli.models.detect_provider_for_model",
        lambda *_args, **_kwargs: (_ for _ in ()).throw(
...
def test_init_session_fires_reset_hook(monkeypatch):
    ...
    import zermes.tools.approval as _approval
...
def test_config_set_yolo_toggles_session_scope():
    from zermes.tools.approval import clear_session, is_session_yolo_enabled
...
def test_config_set_fast_updates_live_agent_and_config(monkeypatch):
    ...
    monkeypatch.setattr(
        "zermes.hermes_cli.models.resolve_fast_mode_overrides",
    ...
    try:
        ...
        assert ("zermes.agent.service_tier", "fast") in writes
        ...
        assert ("zermes.agent.service_tier", "normal") in writes
    finally:
        server._sessions.pop("sid", None)
...
def test_setup_status_reports_provider_config(monkeypatch):
    monkeypatch.setattr("zermes.hermes_cli.main._has_any_provider_configured", lambda: False)
...
def test_config_set_model_global_persists(monkeypatch):
    ...
    monkeypatch.setattr("zermes.hermes_cli.model_switch.switch_model", _switch_model)
    monkeypatch.setattr(server, "_restart_slash_worker", lambda session: None)
    monkeypatch.setattr(server, "_emit", lambda *args, **kwargs: None)
    monkeypatch.setattr("zermes.hermes_cli.config.save_config", lambda cfg: saved.update(cfg))
...
def test_config_set_model_syncs_inference_provider_env(monkeypatch):
    ...
    monkeypatch.setattr(
        "zermes.hermes_cli.model_switch.switch_model", lambda **_kwargs: result
    )
    monkeypatch.setattr(server, "_restart_slash_worker", lambda session: None)
    monkeypatch.setattr(server, "_emit", lambda *args, **kwargs: None)
...
def test_config_set_model_syncs_tui_provider_env(monkeypatch):
    ...
    monkeypatch.setattr("zermes.hermes_cli.model_switch.switch_model", fake_switch_model)
...
def test_session_compress_syncs_session_key_after_rotation(monkeypatch):
    ...
    try:
        with patch("zermes.tui_gateway.server._emit"):
            server.handle_request(
                {
                    "id": "1",
...
def test_prompt_submit_sets_approval_session_key(monkeypatch):
    from zermes.tools.approval import get_current_session_key
...
def test_prompt_submit_expands_context_refs(monkeypatch):
    ...
    fake_ctx = types.ModuleType("zermes.agent.context_references")
    ...
    fake_meta = types.ModuleType("zermes.agent.model_metadata")
    ...
    monkeypatch.setattr(server, "make_stream_renderer", lambda cols: None)
    monkeypatch.setattr(server, "render_message", lambda raw, cols: None)
    monkeypatch.setitem(sys.modules, "zermes.agent.context_references", fake_ctx)
    monkeypatch.setitem(sys.modules, "zermes.agent.model_metadata", fake_meta)
...
def test_skills_reload_runs_in_gateway_process(monkeypatch):
    import zermes.agent.skill_commands as skill_commands
...
def test_plugins_list_surfaces_loader_error(monkeypatch):
    with patch("zermes.hermes_cli.plugins.get_plugin_manager", side_effect=Exception("boom")):
        resp = server.handle_request(
            {"id": "1", "method": "zermes.plugins.list", "params": {}}
        )
...
def test_complete_slash_surfaces_completer_error(monkeypatch):
    with patch(
        "zermes.hermes_cli.commands.SlashCommandCompleter",
...
def test_session_steer_calls_agent_steer_when_agent_supports_it():
    """The TUI RPC method must call zermes.agent.steer(text) and return a
...
def test_session_info_includes_mcp_servers(monkeypatch):
    ...
    fake_mod = types.ModuleType("zermes.tools.mcp_tool")
    fake_mod.get_mcp_status = lambda: fake_status
    monkeypatch.setitem(sys.modules, "zermes.tools.mcp_tool", fake_mod)
...
# session is running.  zermes.agent.switch_model() mutates self.model, self.provider,
# self.base_url, self.client etc. in place — the worker thread running
# zermes.agent.run_conversation is reading those on every iteration.  Same class of
...
def test_config_set_model_rejects_while_running(monkeypatch):
    ...
    try:
        ...
        assert not seen["called"], (
            ...
            "the worker thread reading zermes.agent.model / zermes.agent.client"
        )
    finally:
        server._sessions.pop("sid", None)
...
def test_model_options_does_not_overwrite_curated_models(monkeypatch):
    ...
    with patch(
        "zermes.hermes_cli.model_switch.list_authenticated_providers",
        ...
        # guarding against.
        with patch("zermes.hermes_cli.models.provider_model_ids") as live_fetch:
...
def test_prompt_submit_auto_titles_session_on_complete(monkeypatch):
    ...
    with patch("zermes.agent.title_generator.maybe_auto_title") as mock_title:
        server.handle_request(
            {
                "id": "1",
                "method": "prompt.submit",
...
def test_browser_manage_status_falls_back_to_config_cdp_url(monkeypatch):
    ...
    with patch.dict(sys.modules, {"zermes.hermes_cli.config": fake_cfg}):
        resp = server.handle_request(
            {"id": "1", "method": "browser.manage", "params": {"action": "status"}}
        )
...
def test_browser_manage_status_does_not_call_get_cdp_override(monkeypatch):
    ...
    with patch.dict(sys.modules, {"zermes.tools.browser_tool": fake}):
        resp = server.handle_request(
            {"id": "1", "method": "browser.manage", "params": {"action": "status"}}
        )
...
def test_browser_manage_connect_default_local_reports_launch_hint(monkeypatch):
    ...
    with patch.dict(sys.modules, {"zermes.tools.browser_tool": fake}):
        ...
        with (
            patch(
                "zermes.hermes_cli.browser_connect.try_launch_chrome_debug", return_value=False
            ),
            patch(
                "zermes.hermes_cli.browser_connect.get_chrome_debug_candidates",
                return_value=[],
            ),
        ):
            resp = server.handle_request(
...
def test_browser_manage_connect_default_local_retries_after_launch(monkeypatch):
    ...
    with patch.dict(sys.modules, {"zermes.tools.browser_tool": fake}):
        with patch(
            "zermes.hermes_cli.browser_connect.try_launch_chrome_debug", return_value=True
        ):
            resp = server.handle_request(
                {"id": "1", "method": "browser.manage", "params": {"action": "connect"}}
            )
...
def test_reload_env_rpc_calls_hermes_cli_reload_env(monkeypatch):
    ...
    with patch.dict(sys.modules, {"zermes.hermes_cli.config": fake}):
...
def test_make_agent_reads_nested_max_turns(monkeypatch):
    ...
    with patch("zermes.run_agent.AIAgent") as mock_agent:
