...
import pytest
from zermes.hermes_cli import model_switch
...
def test_openrouter_models_replaced_with_live_catalog(monkeypatch):
    ...
    monkeypatch.setattr(model_switch, "list_authenticated_providers",
                        lambda **kw: list(base))
    monkeypatch.setattr("zermes.hermes_cli.models.fetch_openrouter_models",
...
def test_openrouter_falls_back_to_base_models_on_fetch_failure(monkeypatch):
    ...
    monkeypatch.setattr(model_switch, "list_authenticated_providers",
                        lambda **kw: list(base))
    monkeypatch.setattr("zermes.hermes_cli.models.fetch_openrouter_models", _raise)
...
def test_current_custom_endpoint_passthrough_marks_current_row(monkeypatch):
    ...
    monkeypatch.setattr("zermes.agent.models_dev.PROVIDER_TO_MODELS_DEV", {})
