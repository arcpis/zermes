...
from zermes.hermes_cli.copilot_auth import _try_gh_cli_token
...
class TestProviderRegistry:
    ...
    def test_base_urls(self):
        ...
        assert PROVIDER_REGISTRY["ai-gateway"].inference_base_url == "https://ai-zermes.gateway.vercel.sh/v1"
        assert PROVIDER_REGISTRY["kilocode"].inference_base_url == "https://api.kilo.ai/api/gateway"
        assert PROVIDER_REGISTRY["gmi"].inference_base_url == "https://api.gmi-serving.com/v1"
        assert PROVIDER_REGISTRY["huggingface"].inference_base_url == "https://router.huggingface.co/v1"

    def test_oauth_providers_unchanged(self):
        """Ensure we didn't break the existing OAuth zermes.providers."""
...
def _clear_provider_env(monkeypatch):
    ...
    monkeypatch.setattr("zermes.hermes_cli.auth._load_auth_store", lambda: {})


class TestResolveProvider:
    """Test resolve_provider() with new zermes.providers."""
...
class TestApiKeyProviderStatus:
    ...
    def test_copilot_status_uses_gh_cli_token(self, monkeypatch):
        monkeypatch.setattr("zermes.hermes_cli.copilot_auth._try_gh_cli_token", lambda: "gho_gh_cli_token")
    ...
    def test_copilot_acp_status_detects_local_cli(self, monkeypatch):
        ...
        monkeypatch.setattr("zermes.hermes_cli.auth.shutil.which", lambda command: f"/usr/local/bin/{command}")
    ...
    def test_get_auth_status_dispatches_to_external_process(self, monkeypatch):
        monkeypatch.setattr("zermes.hermes_cli.auth.shutil.which", lambda command: f"/opt/bin/{command}")
...
class TestResolveApiKeyProviderCredentials:
    ...
    def test_resolve_zai_with_key(self, monkeypatch):
        ...
        monkeypatch.setattr("zermes.hermes_cli.auth.detect_zai_endpoint", lambda *a, **kw: None)
    ...
    def test_resolve_copilot_with_gh_cli_fallback(self, monkeypatch):
        monkeypatch.setattr("zermes.hermes_cli.copilot_auth._try_gh_cli_token", lambda: "gho_cli_secret")
        creds = resolve_api_key_provider_credentials("copilot")
        assert creds["provider"] == "copilot"
    ...
    def test_try_gh_cli_token_uses_homebrew_path_when_not_on_path(self, monkeypatch):
        monkeypatch.setattr("zermes.hermes_cli.copilot_auth.shutil.which", lambda command: None)
        monkeypatch.setattr(
            "zermes.hermes_cli.copilot_auth.os.path.isfile",
            lambda path: path == "/opt/homebrew/bin/gh",
        )
        monkeypatch.setattr(
            "zermes.hermes_cli.copilot_auth.os.access",
        ...
        monkeypatch.setattr("zermes.hermes_cli.copilot_auth.subprocess.run", _fake_run)
    ...
    def test_resolve_ai_gateway_with_key(self, monkeypatch):
        ...
        assert creds["base_url"] == "https://ai-zermes.gateway.vercel.sh/v1"
...
class TestRuntimeProviderResolution:
    ...
    def test_runtime_ai_gateway(self, monkeypatch):
        ...
        assert "ai-zermes.gateway.vercel.sh" in result["base_url"]
    ...
    def test_runtime_copilot_uses_responses_for_gpt_5_4(self, monkeypatch):
        monkeypatch.setattr("zermes.hermes_cli.copilot_auth._try_gh_cli_token", lambda: "gho_cli_secret")
        monkeypatch.setattr(
            "zermes.hermes_cli.runtime_provider._get_model_config",
            lambda: {"provider": "copilot", "default": "gpt-5.4"},
        )
        monkeypatch.setattr(
            "zermes.hermes_cli.models.fetch_github_model_catalog",
...
class TestHasAnyProviderConfigured:
    ...
    def test_glm_key_counts(self, monkeypatch, tmp_path):
        from zermes.hermes_cli import config as config_module
    ...
    def test_claude_code_creds_ignored_on_fresh_install(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(config_module, "get_env_path", lambda: hermes_home / ".env")
        monkeypatch.setattr(config_module, "get_hermes_home", lambda: hermes_home)
        monkeypatch.setattr("zermes.hermes_cli.copilot_auth.resolve_copilot_token", lambda: ("", ""))
        ...
        monkeypatch.setattr("zermes.hermes_cli.auth.get_auth_status", lambda _pid: {})
...
class TestKimiCodeCredentialAutoDetect:
    ...
    def test_non_kimi_providers_unaffected(self, monkeypatch):
        """Ensure the auto-detect logic doesn't leak to other zermes.providers."""
...
class TestZaiEndpointAutoDetect:
    ...
    def test_probe_success_returns_detected_url(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "zermes.hermes_cli.auth.detect_zai_endpoint",
            lambda *a, **kw: {
                "id": "coding-global",
    ...
    def test_env_override_skips_probe(self, monkeypatch):
        ...
        monkeypatch.setattr("zermes.hermes_cli.auth.detect_zai_endpoint", _never_called)
        creds = resolve_api_key_provider_credentials("zai")
...
class TestKimiMoonshotModelListIsolation:
    ...
    def test_moonshot_list_excludes_coding_plan_only_models(self):
        from zermes.hermes_cli.main import _PROVIDER_MODELS
...
class TestHuggingFaceModels:
    ...
    def test_model_lists_match(self):
        ...
        from zermes.hermes_cli.main import _PROVIDER_MODELS as main_models
        from zermes.hermes_cli.models import _PROVIDER_MODELS as models_models
    ...
    def test_model_metadata_has_context_lengths(self):
        ...
        from zermes.hermes_cli.models import _PROVIDER_MODELS
        from zermes.agent.model_metadata import DEFAULT_CONTEXT_LENGTHS
    ...
    def test_provider_aliases_in_models_py(self):
        from zermes.hermes_cli.models import _PROVIDER_ALIASES
        assert _PROVIDER_ALIASES.get("hf") == "huggingface"
        assert _PROVIDER_ALIASES.get("hugging-face") == "huggingface"

    def test_provider_label(self):
        from zermes.hermes_cli.models import _PROVIDER_LABELS
...
class TestMinimaxOAuthProvider:
    ...
    def test_minimax_oauth_listed_in_canonical_providers(self):
        from zermes.hermes_cli.models import CANONICAL_PROVIDERS
    ...
    def test_minimax_oauth_aux_model_registered(self):
        from zermes.agent.auxiliary_client import _API_KEY_PROVIDER_AUX_MODELS
