...
def test_resolve_runtime_provider_anthropic_explicit_override_skips_pool(monkeypatch):
    ...
    monkeypatch.setattr(
        "zermes.agent.anthropic_adapter.resolve_anthropic_token",
...
def test_resolve_runtime_provider_ai_gateway(monkeypatch):
    ...
    assert resolved["api_mode"] == "chat_completions"
    assert resolved["base_url"] == "https://ai-zermes.gateway.vercel.sh/v1"
...
class TestAzureFoundryResolution:
    ...
    def test_azure_foundry_missing_api_key_raises(self, monkeypatch):
        ...
        import zermes.hermes_cli.config as cfg_mod
...
class TestProviderEntryApiKeyEnvAlias:
    """The `zermes.providers.<name>` and `custom_providers[i]` normalizer must accept
    ...
    def test_snake_case_api_key_env_normalizes_to_key_env(self):
        from zermes.hermes_cli.config import _normalize_custom_provider_entry
        entry = {
            "name": "vendor",
    ...
    def test_valid_fields_set_lists_key_env(self):
        ...
        from zermes.hermes_cli.config import _VALID_CUSTOM_PROVIDER_FIELDS
...
def test_minimax_oauth_runtime_returns_anthropic_messages_mode(monkeypatch):
    ...
    from zermes.hermes_cli.auth import MINIMAX_OAUTH_GLOBAL_INFERENCE
...
def test_minimax_oauth_runtime_uses_inference_base_url(monkeypatch):
    ...
    from zermes.hermes_cli.auth import MINIMAX_OAUTH_CN_INFERENCE
