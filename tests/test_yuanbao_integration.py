...
from zermes.gateway.platforms.yuanbao import YuanbaoAdapter
...
class TestGatewayRunnerRegistration:
    ...
    def _make_minimal_runner(self, config):
        ...
        stubs = [
            "dotenv",
            "zermes.hermes_cli.env_loader",
            "zermes.hermes_cli.config",
            "hermes_constants",
        ]
        _orig = {}
        for mod in stubs:
    ...
    def test_runner_creates_yuanbao_adapter(self):
        ...
        try:
            ...
            with patch("zermes.gateway.platforms.yuanbao.WEBSOCKETS_AVAILABLE", True):
...
class TestProtoRoundTrip:
    ...
    def test_conn_msg_roundtrip(self):
        from zermes.gateway.platforms.yuanbao_proto import encode_conn_msg, decode_conn_msg
    ...
    def test_text_elem_encoding(self):
        from zermes.gateway.platforms.yuanbao_proto import encode_send_c2c_message
...
class TestMarkdownChunking:
    def test_chunks_are_sent_separately(self):
        from zermes.gateway.platforms.yuanbao import MarkdownProcessor
        long_text = "paragraph\n\n" * 100
...
class TestSignToken:
    def test_import_ok(self):
        from zermes.gateway.platforms.yuanbao import SignManager
...
class TestManagerImports:
    def test_connection_manager_import(self):
        from zermes.gateway.platforms.yuanbao import ConnectionManager
        assert ConnectionManager is not None

    def test_outbound_manager_import(self):
        from zermes.gateway.platforms.yuanbao import OutboundManager
        assert OutboundManager is not None

    def test_message_sender_import(self):
        from zermes.gateway.platforms.yuanbao import MessageSender
        assert MessageSender is not None

    def test_heartbeat_manager_import(self):
        from zermes.gateway.platforms.yuanbao import HeartbeatManager
        assert HeartbeatManager is not None

    def test_slow_response_notifier_import(self):
        from zermes.gateway.platforms.yuanbao import SlowResponseNotifier
        assert SlowResponseNotifier is not None

    def test_adapter_has_outbound_manager(self):
        adapter = YuanbaoAdapter(make_config())
        from zermes.gateway.platforms.yuanbao import ConnectionManager, OutboundManager
    ...
    def test_outbound_composes_sub_managers(self):
        adapter = YuanbaoAdapter(make_config())
        from zermes.gateway.platforms.yuanbao import MessageSender, HeartbeatManager, SlowResponseNotifier
...
class TestMediaModule:
    def test_import_ok(self):
        from zermes.gateway.platforms.yuanbao_media import upload_to_cos, download_url
...
class TestToolset:
    def test_yuanbao_toolset_registered(self):
        """zermes.toolsets.py 中存在 hermes-yuanbao 键"""
        import importlib
    ...
    def test_tools_import(self):
        from zermes.tools.yuanbao_tools import (
            get_group_info,
            query_group_members,
            send_dm,
        )
...
class TestPlatformInit:
    def test_yuanbao_adapter_exported(self):
        """zermes.gateway.platforms.__init__.py 应导出 YuanbaoAdapter"""
        from zermes.gateway.platforms import YuanbaoAdapter as _YuanbaoAdapter
