...
from zermes.hermes_cli import runtime_provider as rp
...
def test_list_authenticated_providers_openai_built_in_nonzero_total(monkeypatch):
    ...
    monkeypatch.setattr(
        "zermes.agent.models_dev.fetch_models_dev",
...
def test_switch_model_resolves_user_provider_credentials(monkeypatch, tmp_path):
    """/model switch should resolve credentials for providers: dict zermes.providers."""
    import yaml
...
def _run_user_provider_override_case(
    *,
    ...
    with patch("zermes.hermes_cli.model_switch.resolve_alias", return_value=None), \
         patch("zermes.hermes_cli.model_switch.list_provider_models", return_value=[]), \
         patch("zermes.hermes_cli.model_switch.normalize_model_for_provider", side_effect=lambda model, provider: model), \
         patch("zermes.hermes_cli.models.validate_requested_model", return_value=_REJECTED_VALIDATION), \
         ...
         patch("zermes.hermes_cli.runtime_provider.resolve_runtime_provider", return_value={"api_key": "***", "base_url": base_url, "api_mode": "anthropic_messages"}):
        return switch_model(
            raw_input=raw_input,
