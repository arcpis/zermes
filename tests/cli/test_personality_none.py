...
class TestCLIPersonalityNone:
    ...
    def test_none_saves_to_config(self):
        cli = self._make_cli()
        with patch("zermes.cli.save_config_value", return_value=True) as mock_save:
            cli._handle_personality_command("/personality none")
        mock_save.assert_called_once_with("zermes.agent.system_prompt", "")
...
class TestGatewayPersonalityNone:
    ...
    async def test_empty_personality_list_uses_profile_display_path(self, tmp_path):
        ...
        with patch("zermes.gateway.run._hermes_home", tmp_path), \
             patch("zermes.hermes_constants.display_hermes_home", return_value="~/.hermes/profiles/coder"):
            event = self._make_event("")
            result = await runner._handle_personality_command(event)
