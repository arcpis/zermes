...
def test_estimate_tool_tokens_returns_positive_counts():
    ...
    from zermes.hermes_cli.tools_config import _estimate_tool_tokens, _tool_token_cache

    # Clear cache to force fresh computation
    import zermes.hermes_cli.tools_config as tc
...
def test_prompt_toolset_checklist_passes_status_fn(monkeypatch):
    ...
    monkeypatch.setattr("zermes.hermes_cli.curses_ui.curses_checklist", fake_checklist)
...
def test_status_fn_returns_formatted_token_count(monkeypatch):
    ...
    import zermes.hermes_cli.tools_config as tc
    from zermes.hermes_cli.tools_config import CONFIGURABLE_TOOLSETS
...
def test_curses_checklist_numbered_fallback_shows_status(monkeypatch, capsys):
    ...
    from zermes.hermes_cli.curses_ui import _numbered_fallback
...
def test_registry_get_schema_returns_none_for_unknown():
    """registry.get_schema() should return None for unknown zermes.tools."""
    from zermes.tools.registry import registry
