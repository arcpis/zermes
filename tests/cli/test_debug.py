...
def hermes_home(tmp_path, monkeypatch):
    ...
    logs_dir = home / "logs"
    logs_dir.mkdir()
    (logs_dir / "zermes.agent.log").write_text(
        "2026-04-12 17:00:00 INFO agent: session started\n"
        "2026-04-12 17:00:01 INFO zermes.tools.terminal: running ls\n"
        "2026-04-12 17:00:02 WARNING agent: high token usage\n"
    )
    (logs_dir / "errors.log").write_text(
        "2026-04-12 17:00:05 ERROR zermes.gateway.run: connection lost\n"
    )
    (logs_dir / "zermes.gateway.log").write_text(
        "2026-04-12 17:00:10 INFO zermes.gateway.run: started\n"
    )
...
class TestUploadPasteRs:
    ...
    def test_upload_paste_rs_success(self):
        from zermes.hermes_cli.debug import _upload_paste_rs
        ...
        with patch("zermes.hermes_cli.debug.urllib.request.urlopen", return_value=mock_resp):
    ...
    def test_upload_paste_rs_network_error(self):
        ...
        with patch(
            "zermes.hermes_cli.debug.urllib.request.urlopen",
...
class TestUploadDpasteCom:
    ...
    def test_upload_dpaste_com_success(self):
        from zermes.hermes_cli.debug import _upload_dpaste_com
...
class TestUploadToPastebin:
    ...
    def test_tries_paste_rs_first(self):
        from zermes.hermes_cli.debug import upload_to_pastebin

        with patch("zermes.hermes_cli.debug._upload_paste_rs",
    ...
    def test_falls_back_to_dpaste_com(self):
        ...
        with patch("zermes.hermes_cli.debug._upload_paste_rs",
             ...
             patch("zermes.hermes_cli.debug._upload_dpaste_com",
...
class TestCaptureLogSnapshot:
    ...
    def test_reads_small_file(self, hermes_home):
        from zermes.hermes_cli.debug import _capture_log_snapshot
    ...
    def test_empty_primary_reports_file_empty(self, hermes_home):
        ...
        (hermes_home / "logs" / "zermes.agent.log").write_text("")
    ...
    def test_race_truncate_after_resolve_reports_empty(self, hermes_home, monkeypatch):
        ...
        log_path = hermes_home / "logs" / "zermes.agent.log"
        from zermes.hermes_cli import debug
    ...
    def test_truncates_large_file(self, hermes_home):
        ...
        big_content = "x" * 100 + "\n"
        (hermes_home / "logs" / "zermes.agent.log").write_text(big_content * 200)
    ...
    def test_keeps_first_line_when_truncation_on_boundary(self, hermes_home):
        ...
        (hermes_home / "logs" / "zermes.agent.log").write_text(line * num_lines)
    ...
    def test_falls_back_to_rotated_file(self, hermes_home):
        """When zermes.gateway.log doesn't exist, falls back to zermes.gateway.log.1."""
        from zermes.hermes_cli.debug import _capture_log_snapshot

        logs_dir = hermes_home / "logs"
        # Remove the primary (if any) and create a .1 rotation
        (logs_dir / "zermes.gateway.log").unlink(missing_ok=True)
        (logs_dir / "zermes.gateway.log.1").write_text(
            "2026-04-12 10:00:00 INFO zermes.gateway.run: rotated content\n"
        )
    ...
    def test_prefers_primary_over_rotated(self, hermes_home):
        ...
        logs_dir = hermes_home / "logs"
        (logs_dir / "zermes.gateway.log").write_text("primary content\n")
        (logs_dir / "zermes.gateway.log.1").write_text("rotated content\n")
    ...
    def test_falls_back_when_primary_empty(self, hermes_home):
        ...
        logs_dir = hermes_home / "logs"
        (logs_dir / "zermes.agent.log").write_text("")
        (logs_dir / "zermes.agent.log.1").write_text("rotated agent data\n")
...
class TestCaptureLogSnapshotRedaction:
    ...
    def hermes_home_with_secret(self, tmp_path, monkeypatch):
        """Isolated HERMES_HOME whose zermes.agent.log contains a vendor-prefixed token."""
        home = tmp_path / ".hermes"
        home.mkdir()
        monkeypatch.setenv("HERMES_HOME", str(home))
    ...
    def test_capture_default_log_snapshots_threads_redact(
        ...
        from zermes.hermes_cli.debug import _capture_default_log_snapshots
...
class TestCollectDebugReport:
    ...
    def test_report_includes_dump_output(self, hermes_home):
        from zermes.hermes_cli.debug import collect_debug_report

        with patch("zermes.hermes_cli.dump.run_dump") as mock_dump:
    ...
    def test_report_includes_agent_log(self, hermes_home):
        ...
        with patch("zermes.hermes_cli.dump.run_dump"):
            report = collect_debug_report(log_lines=50)

        assert "--- zermes.agent.log" in report
    ...
    def test_report_includes_gateway_log(self, hermes_home):
        ...
        assert "--- zermes.gateway.log" in report
...
class TestRunDebugShare:
    ...
    def test_share_sweeps_expired_pastes(self, hermes_home, capsys):
        ...
        with patch("zermes.hermes_cli.dump.run_dump"), \
             patch("zermes.hermes_cli.debug._sweep_expired_pastes", return_value=(0, 0)) as mock_sweep, \
             patch("zermes.hermes_cli.debug.upload_to_pastebin",
    ...
    def test_share_survives_sweep_failure(self, hermes_home, capsys):
        ...
        with patch("zermes.hermes_cli.dump.run_dump"), \
             patch(
                 "zermes.hermes_cli.debug._sweep_expired_pastes",
    ...
    def test_local_flag_prints_full_logs(self, hermes_home, capsys):
        ...
        assert "--- zermes.agent.log" in out
        assert "FULL zermes.agent.log" in out
        assert "FULL zermes.gateway.log" in out

    def test_share_uploads_three_pastes(self, hermes_home, capsys):
        """Successful share uploads report + zermes.agent.log + zermes.gateway.log."""
        from zermes.hermes_cli.debug import run_debug_share
        ...
        out = capsys.readouterr().out
        # Should have 3 uploads: report, zermes.agent.log, zermes.gateway.log
        assert call_count[0] == 3
        assert "paste.rs/paste1" in out  # Report
        assert "paste.rs/paste2" in out  # zermes.agent.log
        assert "paste.rs/paste3" in out  # zermes.gateway.log
        assert "Report" in out
        assert "zermes.agent.log" in out
        assert "zermes.gateway.log" in out

        # Each log paste should start with the dump header
        agent_paste = uploaded_content[1]
        assert "--- hermes dump ---" in agent_paste
        assert "--- full zermes.agent.log ---" in agent_paste
        gateway_paste = uploaded_content[2]
        assert "--- hermes dump ---" in gateway_paste
        assert "--- full zermes.gateway.log ---" in gateway_paste

    def test_share_keeps_report_and_full_log_on_same_snapshot(self, hermes_home, capsys):
        """A mid-run rotation must not make full zermes.agent.log older than the report."""
        from zermes.hermes_cli.debug import run_debug_share, collect_debug_report as real_collect_debug_report

        logs_dir = hermes_home / "logs"
        (logs_dir / "zermes.agent.log").write_text(
            "2026-04-22 12:00:00 INFO agent: newest line\n"
        )
        (logs_dir / "zermes.agent.log.1").write_text(
        ...
        def _wrapped_collect_debug_report(*, log_lines=200, dump_text="", log_snapshots=None):
            ...
            # before the old implementation would have re-read zermes.agent.log for
            # standalone upload.
            (logs_dir / "zermes.agent.log").write_text("")
            (logs_dir / "zermes.agent.log.1").write_text(
                "2026-04-10 12:00:00 INFO agent: old rotated line\n"
        ...
        with patch("zermes.hermes_cli.dump.run_dump"), \
             patch("zermes.hermes_cli.debug.collect_debug_report", side_effect=_wrapped_collect_debug_report), \
             patch("zermes.hermes_cli.debug.upload_to_pastebin", side_effect=_mock_upload):
            run_debug_share(args)
...
class TestRunDebugShareRedaction:
    ...
    def hermes_home_with_secret(self, tmp_path, monkeypatch):
        ...
        (logs_dir / "errors.log").write_text("")
        (logs_dir / "zermes.gateway.log").write_text(
            f"2026-04-12 17:00:01 INFO zermes.gateway.run: token {_REDACT_FIXTURE_TOKEN}\n"
        )
        return home
    ...
    def test_default_share_redacts_uploaded_content(
        ...
        with patch("zermes.hermes_cli.dump.run_dump"), \
             patch("zermes.hermes_cli.debug._sweep_expired_pastes", return_value=(0, 0)), \
             patch("zermes.hermes_cli.debug.upload_to_pastebin", side_effect=fake_upload):
            run_debug_share(args)
    ...
    def test_no_redact_flag_disables_redaction_and_banner(
        ...
        # The zermes.agent.log paste should now contain the raw token.
...
class TestExtractPasteId:
    def test_paste_rs_url(self):
        from zermes.hermes_cli.debug import _extract_paste_id
...
class TestDeletePaste:
    def test_delete_sends_delete_request(self):
        from zermes.hermes_cli.debug import delete_paste
        ...
        with patch("zermes.hermes_cli.debug.urllib.request.urlopen",
...
class TestScheduleAutoDelete:
    ...
    def test_does_not_spawn_subprocess(self, hermes_home):
        ...
        import ast
        import inspect
        from zermes.hermes_cli.debug import _schedule_auto_delete
    ...
    def test_records_pending_to_json(self, hermes_home):
        ...
        from zermes.hermes_cli.debug import _schedule_auto_delete, _pending_file
        import json
    ...
    def test_merges_with_existing_pending(self, hermes_home):
        ...
        from zermes.hermes_cli.debug import _schedule_auto_delete, _load_pending
...
class TestSweepExpiredPastes:
    ...
    def test_sweep_deletes_expired_entries(self, hermes_home):
        ...
        with patch("zermes.hermes_cli.debug.delete_paste", side_effect=fake_delete):
            deleted, remaining = _sweep_expired_pastes()
    ...
    def test_sweep_leaves_future_entries_alone(self, hermes_home):
        from zermes.hermes_cli.debug import _sweep_expired_pastes, _save_pending
        import time
        ...
        with patch("zermes.hermes_cli.debug.delete_paste") as mock_delete:
            deleted, remaining = _sweep_expired_pastes()
    ...
    def test_sweep_survives_network_failure(self, hermes_home):
        ...
        with patch(
            "zermes.hermes_cli.debug.delete_paste",
...
class TestRunDebugSweepsOnInvocation:
    ...
    def test_run_debug_calls_sweep(self, hermes_home):
        ...
        with patch("zermes.hermes_cli.debug._sweep_expired_pastes") as mock_sweep:
            run_debug(args)
...
class TestRunDebugDelete:
    def test_deletes_valid_url(self, capsys):
        from zermes.hermes_cli.debug import run_debug_delete

        args = MagicMock()
        args.urls = ["https://paste.rs/abc"]

        with patch("zermes.hermes_cli.debug.delete_paste", return_value=True):
    ...
    def test_handles_delete_failure(self, capsys):
        ...
        with patch("zermes.hermes_cli.debug.delete_paste",
...
class TestShareIncludesAutoDelete:
    ...
    def test_share_schedules_auto_delete(self, hermes_home, capsys):
        ...
        with patch("zermes.hermes_cli.dump.run_dump"), \
             ...
             patch("zermes.hermes_cli.debug._schedule_auto_delete") as mock_sched:
            run_debug_share(args)
    ...
    def test_share_shows_privacy_notice(self, hermes_home, capsys):
        ...
        with patch("zermes.hermes_cli.dump.run_dump"), \
             patch("zermes.hermes_cli.debug.upload_to_pastebin",
                    return_value="https://paste.rs/test"), \
             patch("zermes.hermes_cli.debug._schedule_auto_delete"):
            run_debug_share(args)
