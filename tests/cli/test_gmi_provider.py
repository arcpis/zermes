...
class TestGmiAliases:
    ...
    def test_providers_normalize_provider(self):
        from zermes.hermes_cli.providers import normalize_provider as normalize_provider_in_providers
...
class TestGmiModelCatalog:
    ...
    def test_provider_model_ids_prefers_live_api(self, monkeypatch):
        monkeypatch.setattr(
            "zermes.hermes_cli.auth.resolve_api_key_provider_credentials",
            lambda provider_id: {
        ...
        monkeypatch.setattr(
            "zermes.hermes_cli.models.fetch_api_models",
    ...
    def test_provider_model_ids_falls_back_to_static_models(self, monkeypatch):
        ...
        monkeypatch.setattr("zermes.hermes_cli.models.fetch_api_models", lambda api_key, base_url: None)
...
class TestGmiDoctor:
    ...
    def test_run_doctor_checks_gmi_models_endpoint(self, monkeypatch, tmp_path):
        ...
        try:
            from zermes.hermes_cli import auth as _auth_mod
...
class TestGmiModelMetadata:
    def test_url_to_provider(self):
        from zermes.agent.model_metadata import _URL_TO_PROVIDER

        assert _URL_TO_PROVIDER.get("api.gmi-serving.com") == "gmi"

    def test_provider_prefixes(self):
        from zermes.agent.model_metadata import _PROVIDER_PREFIXES
    ...
    def test_known_gmi_endpoint_still_uses_endpoint_metadata(self):
        with patch(
            "zermes.agent.model_metadata.get_cached_context_length",
            return_value=None,
        ), patch(
            "zermes.agent.model_metadata.fetch_endpoint_model_metadata",
            return_value={"anthropic/claude-opus-4.6": {"context_length": 409600}},
        ), patch(
            "zermes.agent.models_dev.lookup_models_dev_context",
            return_value=None,
        ), patch(
            "zermes.agent.model_metadata.fetch_model_metadata",
            return_value={},
        ):
...
class TestGmiAuxiliary:
    def test_aux_default_model(self):
        from zermes.agent.auxiliary_client import _get_aux_model_for_provider
...
class TestGmiMainFlow:
    def test_chat_parser_accepts_gmi_provider(self, monkeypatch):
        ...
        monkeypatch.setattr("zermes.hermes_cli.config.get_container_exec_info", lambda: None)
        monkeypatch.setattr(
            "zermes.hermes_cli.main.cmd_chat",
    ...
    def test_select_provider_and_model_routes_gmi_to_generic_flow(self, monkeypatch):
        ...
        monkeypatch.setattr("zermes.hermes_cli.auth.resolve_provider", lambda *args, **kwargs: None)
        ...
        monkeypatch.setattr("zermes.hermes_cli.main._prompt_provider_choice", fake_prompt_provider_choice)
        monkeypatch.setattr("zermes.hermes_cli.main._model_flow_api_key_provider", fake_model_flow_api_key_provider)
    ...
    def test_model_flow_api_key_provider_persists_gmi_selection(self, monkeypatch):
        ...
        with patch(
            ...
            "zermes.hermes_cli.auth._prompt_model_selection",
            return_value="openai/gpt-5.4-mini",
        ), patch(
            "zermes.hermes_cli.auth.deactivate_provider",
        ), patch(
            "builtins.input",
            return_value="",
        ):
            from zermes.hermes_cli.main import _model_flow_api_key_provider
