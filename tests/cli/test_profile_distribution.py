"""Tests for zermes.hermes_cli.profile_distribution — git-based profile installs.
...
class TestUpdate:
    ...
    def test_update_missing_manifest_errors(self, profile_env):
        ...
        from zermes.hermes_cli.profiles import create_profile
...
class TestInstalledAtStamp:
    ...
    def test_update_refreshes_installed_at(self, profile_env, monkeypatch):
        ...
        from zermes.hermes_cli.profiles import get_profile_dir
        ...
        monkeypatch.setattr(
            "zermes.hermes_cli.profile_distribution.datetime", _FakeDT, raising=True
        )

        from zermes.hermes_cli.profile_distribution import update_distribution
...
class TestProfileInfoDistribution:
    ...
    def test_plain_profile_has_no_distribution_fields(self, profile_env):
        from zermes.hermes_cli.profiles import create_profile, list_profiles
        create_profile(name="plain", no_alias=True)
        rows = {p.name: p for p in list_profiles()}
    ...
    def test_malformed_manifest_does_not_break_list(self, profile_env):
        from zermes.hermes_cli.profiles import create_profile, list_profiles, get_profile_dir
