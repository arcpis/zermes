...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./hermes` should use `zermes.hermes_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "hermes"
    called = []

    fake_main_module = types.ModuleType("zermes.hermes_cli.main")

    def fake_main():
        called.append("zermes.hermes_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "zermes.hermes_cli.main", fake_main_module)
    ...
    assert called == ["zermes.hermes_cli.main"]
