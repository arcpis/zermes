...
import pytest
from zermes.hermes_cli.config import DEFAULT_CONFIG, load_config, save_config
...
class TestNonInteractiveSetup:
    """Verify setup paths exit cleanly in headless/non-interactive zermes.environments."""
    ...
    def test_cmd_setup_allows_noninteractive_flag_without_tty(self):
        ...
        from zermes.hermes_cli.main import cmd_setup

        args = _make_setup_args(non_interactive=True)

        with (
            patch("zermes.hermes_cli.setup.run_setup_wizard") as mock_run_setup,
    ...
    def test_non_interactive_flag_skips_wizard(self, capsys):
        ...
        with (
            patch("zermes.hermes_cli.setup.ensure_hermes_home"),
            patch("zermes.hermes_cli.setup.load_config", return_value={}),
            patch("zermes.hermes_cli.setup.get_hermes_home", return_value="/tmp/.hermes"),
            patch("zermes.hermes_cli.auth.get_active_provider", side_effect=AssertionError("wizard continued")),
    ...
    def test_chat_first_run_headless_skips_setup_prompt(self, capsys):
        ...
        from zermes.hermes_cli.main import cmd_chat

        args = _make_chat_args()

        with (
            patch("zermes.hermes_cli.main._has_any_provider_configured", return_value=False),
            patch("zermes.hermes_cli.main.cmd_setup") as mock_setup,
            patch("sys.stdin") as mock_stdin,
            patch("builtins.input", side_effect=AssertionError("input should not be called")),
        ):
            mock_stdin.isatty.return_value = False
