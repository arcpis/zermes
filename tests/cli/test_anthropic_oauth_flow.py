...
def test_run_anthropic_oauth_flow_prefers_claude_code_credentials(tmp_path, monkeypatch, capsys):
    ...
    monkeypatch.setattr(
        "zermes.agent.anthropic_adapter.run_oauth_setup_token",
    ...
    from zermes.hermes_cli.main import _run_anthropic_oauth_flow
...
def test_run_anthropic_oauth_flow_manual_token_still_persists(tmp_path, monkeypatch, capsys):
    monkeypatch.setenv("HERMES_HOME", str(tmp_path))
    monkeypatch.setattr("zermes.agent.anthropic_adapter.run_oauth_setup_token", lambda: None)
    monkeypatch.setattr("zermes.agent.anthropic_adapter.read_claude_code_credentials", lambda: None)
    monkeypatch.setattr("zermes.agent.anthropic_adapter.is_claude_code_token_valid", lambda creds: False)
