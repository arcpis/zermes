"""Tests for zermes.acp_adapter.events — callback factories for ACP notifications."""
...
class TestToolProgressCallback:
    def test_emits_tool_call_start(self, mock_conn, event_loop_fixture):
        ...
        with patch("zermes.acp_adapter.events.asyncio.run_coroutine_threadsafe") as mock_rcts:
...
class TestStepCallback:
    ...
    def test_result_passed_to_build_tool_complete(self, mock_conn, event_loop_fixture):
        ...
        with patch("zermes.acp_adapter.events.asyncio.run_coroutine_threadsafe") as mock_rcts, \
             patch("zermes.acp_adapter.events.build_tool_complete") as mock_btc:
            future = MagicMock(spec=Future)
            future.result.return_value = None
            mock_rcts.return_value = future
    ...
    def test_tool_progress_captures_snapshot_metadata(self, mock_conn, event_loop_fixture):
        ...
        with patch("zermes.acp_adapter.events.make_tool_call_id", return_value="tc-meta"), \
             patch("zermes.acp_adapter.events._send_update") as mock_send, \
             patch("zermes.agent.display.capture_local_edit_snapshot", return_value="snapshot"):
            cb = make_tool_progress_cb(mock_conn, "session-1", loop, tool_call_ids, tool_call_meta)
