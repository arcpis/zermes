"""Tests for zermes.acp_adapter.server — HermesACPAgent ACP server."""
...
from zermes.acp_adapter.server import HermesACPAgent, HERMES_VERSION
...
class TestAuthenticate:
    ...
    async def test_authenticate_with_matching_method_id(self, agent, monkeypatch):
        monkeypatch.setattr(
            "zermes.acp_adapter.server.detect_provider",
            lambda: "openrouter",
        )
...
class TestSessionOps:
    ...
    async def test_new_session_returns_model_state(self):
        ...
        with patch(
            "zermes.hermes_cli.models.curated_models_for_provider",
    ...
    def test_build_usage_update_for_zed_context_indicator(self, agent, mock_manager):
        ...
        with patch(
            "zermes.agent.model_metadata.estimate_request_tokens_rough",
            return_value=25_000,
        ):
    ...
    async def test_load_session_replays_persisted_history_to_client(self, agent):
        ...
        state.history = [
            ...
            },
            {
                ...
                "content": '{"total_count":1,"matches":[{"path":"zermes.cli.py","line":42,"content":"slash commands"}]}',
            },
        ]
        ...
        assert "zermes.cli.py:42" in tool_updates[1].content[0].content.text
...
class TestListAndFork:
    ...
    async def test_list_sessions_pagination_first_page(self, agent):
        from zermes.acp_adapter import server as acp_server
...
class TestSessionConfiguration:
    ...
    async def test_set_session_model_accepts_provider_prefixed_choice(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("zermes.hermes_cli.config.load_config", lambda: {
        ...
        with patch("zermes.run_agent.AIAgent", side_effect=fake_agent):
            acp_agent = HermesACPAgent(session_manager=manager)
...
class TestPrompt:
    ...
    async def test_prompt_runs_agent(self, agent):
        """The prompt method should call run_conversation on the zermes.agent."""
        new_resp = await agent.new_session(cwd=".")
        state = agent.session_manager.get_session(new_resp.session_id)
...
class TestRegisterSessionMcpServers:
    ...
    async def test_registers_stdio_servers(self, agent, mock_manager):
        ...
        with patch("zermes.tools.mcp_tool.register_mcp_servers", side_effect=capture_register),\
             patch("zermes.model_tools.get_tool_definitions", return_value=[]):
    ...
    async def test_refreshes_agent_tool_surface(self, agent, mock_manager):
        """After MCP registration, zermes.agent.tools and valid_tool_names are refreshed."""
        ...
        with patch("zermes.tools.mcp_tool.register_mcp_servers", return_value=["mcp_srv_search"]),\
             patch("zermes.model_tools.get_tool_definitions", return_value=fake_tools) as mock_defs:
            await agent._register_session_mcp_servers(state, [server])
    ...
    async def test_register_failure_logs_warning(self, agent, mock_manager):
        ...
        with patch("zermes.tools.mcp_tool.register_mcp_servers", side_effect=RuntimeError("boom")):
            # Should not raise
            await agent._register_session_mcp_servers(state, [server])
