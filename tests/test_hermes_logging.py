...
import zermes.hermes_logging as hermes_logging
...
def _reset_logging_state():
    ...
    """
    zermes.hermes_logging._logging_initialized = False
    ...
    zermes.hermes_logging._install_session_record_factory()
    yield
    ...
    for h in list(root.handlers):
        if h not in pre_existing:
            ...
            h.close()
    zermes.hermes_logging._logging_initialized = False
    zermes.hermes_logging.clear_session_context()
...
class TestSetupLogging:
    """setup_logging() creates zermes.agent.log + errors.log with RotatingFileHandler."""

    def test_creates_log_directory(self, hermes_home):
        log_dir = zermes.hermes_logging.setup_logging(hermes_home=hermes_home)
        assert log_dir == hermes_home / "logs"
        assert log_dir.is_dir()

    def test_creates_agent_log_handler(self, hermes_home):
        zermes.hermes_logging.setup_logging(hermes_home=hermes_home)
        root = logging.getLogger()

        agent_handlers = [
            h for h in root.handlers
            if isinstance(h, RotatingFileHandler)
            and "zermes.agent.log" in getattr(h, "baseFilename", "")
        ]
    ...
    def test_idempotent_no_duplicate_handlers(self, hermes_home):
        zermes.hermes_logging.setup_logging(hermes_home=hermes_home)
        zermes.hermes_logging.setup_logging(hermes_home=hermes_home)  # second call — should be no-op
    ...
    def test_force_reinitializes(self, hermes_home):
        ...
        zermes.hermes_logging.setup_logging(hermes_home=hermes_home, force=True)
    ...
    def test_custom_log_level(self, hermes_home):
        zermes.hermes_logging.setup_logging(hermes_home=hermes_home, log_level="DEBUG")
    ...
    def test_custom_max_size_and_backup(self, hermes_home):
        zermes.hermes_logging.setup_logging(
    ...
    def test_writes_to_agent_log(self, hermes_home):
        ...
        test_logger.info("test message for zermes.agent.log")

        # Flush handlers
        for h in logging.getLogger().handlers:
            h.flush()

        agent_log = hermes_home / "logs" / "zermes.agent.log"
        assert agent_log.exists()
        content = agent_log.read_text()
        assert "test message for zermes.agent.log" in content
    ...
    def test_explicit_params_override_config(self, hermes_home):
        ...
        zermes.hermes_logging.setup_logging(hermes_home=hermes_home, log_level="WARNING")
...
class TestGatewayMode:
    """setup_logging(mode='gateway') creates a filtered zermes.gateway.log."""

    def test_gateway_log_created(self, hermes_home):
        zermes.hermes_logging.setup_logging(hermes_home=hermes_home, mode="gateway")
        root = logging.getLogger()

        gw_handlers = [
            h for h in root.handlers
            if isinstance(h, RotatingFileHandler)
            and "zermes.gateway.log" in getattr(h, "baseFilename", "")
        ]
        assert len(gw_handlers) == 1

    def test_gateway_log_not_created_in_cli_mode(self, hermes_home):
        zermes.hermes_logging.setup_logging(hermes_home=hermes_home, mode="cli")
        root = logging.getLogger()
    ...
    def test_gateway_log_created_after_cli_init(self, hermes_home):
        """Gateway mode attaches zermes.gateway.log even after earlier CLI init."""
        zermes.hermes_logging.setup_logging(hermes_home=hermes_home, mode="cli")
        zermes.hermes_logging.setup_logging(hermes_home=hermes_home, mode="gateway")
        ...
        logging.getLogger("zermes.gateway.run").info("gateway connected after cli init")

        for h in root.handlers:
            h.flush()

        gw_log = hermes_home / "logs" / "zermes.gateway.log"
    ...
    def test_gateway_log_receives_gateway_records(self, hermes_home):
        """zermes.gateway.log captures records from zermes.gateway.* loggers."""
        zermes.hermes_logging.setup_logging(hermes_home=hermes_home, mode="gateway")

        gw_logger = logging.getLogger("zermes.gateway.platforms.telegram")
    ...
    def test_gateway_log_rejects_non_gateway_records(self, hermes_home):
        """zermes.gateway.log does NOT capture records from zermes.tools.*, zermes.agent.*, etc."""
        zermes.hermes_logging.setup_logging(hermes_home=hermes_home, mode="gateway")

        tool_logger = logging.getLogger("zermes.tools.terminal_tool")
        tool_logger.info("running command")

        agent_logger = logging.getLogger("zermes.agent.context_compressor")
    ...
    def test_agent_log_still_receives_all(self, hermes_home):
        """zermes.agent.log (catch-all) still receives gateway AND tool records."""
        zermes.hermes_logging.setup_logging(hermes_home=hermes_home, mode="gateway")

        gw_logger = logging.getLogger("zermes.gateway.run")
        file_logger = logging.getLogger("zermes.tools.file_tools")
...
class TestSessionContext:
    ...
    def test_session_tag_in_log_output(self, hermes_home):
        ...
        zermes.hermes_logging.setup_logging(hermes_home=hermes_home)
        zermes.hermes_logging.set_session_context("abc123")
    ...
    def test_clear_session_context(self, hermes_home):
        ...
        zermes.hermes_logging.setup_logging(hermes_home=hermes_home)
        zermes.hermes_logging.set_session_context("xyz789")
        zermes.hermes_logging.clear_session_context()
    ...
    def test_session_context_thread_isolated(self, hermes_home):
        ...
        def thread_a():
            zermes.hermes_logging.set_session_context("thread_a_session")
            logging.getLogger("test.thread_a").info("from thread A")
            for h in logging.getLogger().handlers:
                h.flush()

        def thread_b():
            zermes.hermes_logging.set_session_context("thread_b_session")
...
class TestRecordFactory:
    ...
    def test_tag_with_context(self):
        zermes.hermes_logging.set_session_context("sess_42")
        factory = logging.getLogRecordFactory()
        record = factory("test", logging.INFO, "", 0, "msg", (), None)
    ...
    def test_works_with_any_handler(self):
        ...
        zermes.hermes_logging.set_session_context("any_handler_test")
...
class TestComponentFilter:
    ...
    def test_passes_matching_prefix(self):
        f = zermes.hermes_logging._ComponentFilter(("gateway",))
        record = logging.LogRecord(
            "zermes.gateway.run", logging.INFO, "", 0, "msg", (), None
        )
    ...
    def test_passes_nested_matching_prefix(self):
        f = zermes.hermes_logging._ComponentFilter(("gateway",))
        record = logging.LogRecord(
            "zermes.gateway.platforms.telegram", logging.INFO, "", 0, "msg", (), None
        )
        assert f.filter(record) is True
    ...
    def test_blocks_non_matching(self):
        f = zermes.hermes_logging._ComponentFilter(("gateway",))
        record = logging.LogRecord(
            "zermes.tools.terminal_tool", logging.INFO, "", 0, "msg", (), None
        )
        assert f.filter(record) is False

    def test_multiple_prefixes(self):
        f = zermes.hermes_logging._ComponentFilter(("agent", "run_agent", "model_tools"))
        assert f.filter(logging.LogRecord(
            "zermes.agent.compressor", logging.INFO, "", 0, "", (), None
        ))
        assert f.filter(logging.LogRecord(
        ...
        assert not f.filter(logging.LogRecord(
            "zermes.tools.browser", logging.INFO, "", 0, "", (), None
        ))
...
class TestComponentPrefixes:
    ...
    def test_gateway_prefix(self):
        assert "gateway" in zermes.hermes_logging.COMPONENT_PREFIXES
        assert ("gateway",) == zermes.hermes_logging.COMPONENT_PREFIXES["gateway"]

    def test_agent_prefix(self):
        prefixes = zermes.hermes_logging.COMPONENT_PREFIXES["agent"]
        assert "agent" in prefixes
        assert "run_agent" in prefixes
        assert "model_tools" in prefixes

    def test_tools_prefix(self):
        assert ("tools",) == zermes.hermes_logging.COMPONENT_PREFIXES["tools"]

    def test_cli_prefix(self):
        prefixes = zermes.hermes_logging.COMPONENT_PREFIXES["cli"]
        assert "hermes_cli" in prefixes
        assert "cli" in prefixes

    def test_cron_prefix(self):
        assert ("cron",) == zermes.hermes_logging.COMPONENT_PREFIXES["cron"]
...
class TestSetupVerboseLogging:
    ...
    def test_adds_stream_handler(self, hermes_home):
        zermes.hermes_logging.setup_logging(hermes_home=hermes_home)
        zermes.hermes_logging.setup_verbose_logging()
    ...
    def test_idempotent(self, hermes_home):
        zermes.hermes_logging.setup_logging(hermes_home=hermes_home)
        zermes.hermes_logging.setup_verbose_logging()
        zermes.hermes_logging.setup_verbose_logging()  # second call
...
class TestAddRotatingHandler:
    ...
    def test_creates_directory(self, tmp_path):
        ...
        zermes.hermes_logging._add_rotating_handler(
            logger, log_path,
    ...
    def test_log_filter_attached(self, tmp_path):
        ...
        formatter = logging.Formatter("%(message)s")
        component_filter = zermes.hermes_logging._ComponentFilter(("test",))
    ...
    def test_no_session_filter_on_handler(self, tmp_path):
        ...
        zermes.hermes_logging.set_session_context("factory_test")
    ...
    def test_managed_mode_initial_open_sets_group_writable(self, tmp_path):
        ...
        try:
            with patch("zermes.hermes_cli.config.is_managed", return_value=True):
                zermes.hermes_logging._add_rotating_handler(
                    logger, log_path,
                    level=logging.INFO, max_bytes=1024, backup_count=1,
                    formatter=formatter,
...
class TestReadLoggingConfig:
    ...
    def test_returns_none_when_no_config(self, hermes_home):
        level, max_size, backup = zermes.hermes_logging._read_logging_config()
