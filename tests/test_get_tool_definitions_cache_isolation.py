...
The ``quiet_mode=True`` fast path in :func:`zermes.model_tools.get_tool_definitions`
...
def _clear_cache():
    ...
    zermes.model_tools._tool_defs_cache.clear()
    yield
    zermes.model_tools._tool_defs_cache.clear()
...
class TestQuietModeCacheIsolation:
    ...
    def test_first_uncached_call_returns_fresh_list(self):
        ...
        first = zermes.model_tools.get_tool_definitions(quiet_mode=True)
        assert isinstance(first, list)
        # Find the cached value to compare identity.
        assert len(zermes.model_tools._tool_defs_cache) == 1
        cached = next(iter(zermes.model_tools._tool_defs_cache.values()))
    ...
    def test_cache_hit_returns_fresh_list(self):
        ...
        first = zermes.model_tools.get_tool_definitions(quiet_mode=True)
        second = zermes.model_tools.get_tool_definitions(quiet_mode=True)
    ...
    def test_repeated_caller_mutation_does_not_accumulate(self):
        ...
        baseline = len(zermes.model_tools.get_tool_definitions(quiet_mode=True))
        for _ in range(5):
            tools = zermes.model_tools.get_tool_definitions(quiet_mode=True)
            zermes.tools.append({"type": "function", "function": {"name": "lcm_grep"}})
        final = zermes.model_tools.get_tool_definitions(quiet_mode=True)
    ...
    def test_non_quiet_mode_does_not_use_cache(self):
        ...
        zermes.model_tools.get_tool_definitions(quiet_mode=False)
        assert len(zermes.model_tools._tool_defs_cache) == 0
