...
def test_init_tries_fallback_when_primary_returns_none():
    ...
    with patch("zermes.agent.auxiliary_client.resolve_provider_client", side_effect=fake_resolve),\
         patch("zermes.run_agent.get_tool_definitions", return_value=_make_tool_defs()),\
         patch("zermes.run_agent.check_toolset_requirements", return_value={}),\
         patch("zermes.run_agent.OpenAI", return_value=MagicMock()):
...
def test_init_raises_when_no_fallback_configured():
    ...
    with patch("zermes.agent.auxiliary_client.resolve_provider_client", return_value=(None, None)),\
         patch("zermes.run_agent.get_tool_definitions", return_value=_make_tool_defs()),\
         patch("zermes.run_agent.check_toolset_requirements", return_value={}),\
         patch("zermes.run_agent.OpenAI", return_value=MagicMock()):
