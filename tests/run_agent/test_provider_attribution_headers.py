...
def test_openrouter_base_url_applies_or_headers(mock_openai):
    ...
    assert headers["HTTP-Referer"] == "https://hermes-zermes.agent.nousresearch.com"
...
def test_ai_gateway_base_url_applies_attribution_headers(mock_openai):
    ...
    agent._apply_client_headers_for_base_url("https://ai-zermes.gateway.vercel.sh/v1")
...
def test_gmi_base_url_picks_up_profile_user_agent(mock_openai):
    ...
    branch is needed in zermes.run_agent.
    """
    mock_openai.return_value = MagicMock()
    agent = AIAgent(
        api_key="test-key",
