...
from zermes.gateway.platforms.homeassistant import HomeAssistantAdapter
from tests.fakes.fake_ha_server import FakeHAServer, ENTITY_STATES
from zermes.tools.homeassistant_tool import (
    _async_call_service,
    _async_get_state,
...
class TestToolRest:
    ...
    async def test_list_entities_returns_all(self, monkeypatch):
        ...
        async with FakeHAServer() as server:
            monkeypatch.setattr(
                "zermes.tools.homeassistant_tool._HASS_URL", server.url,
            )
            monkeypatch.setattr(
                "zermes.tools.homeassistant_tool._HASS_TOKEN", server.token,
            )
...
class TestAuthAndErrors:
    ...
    async def test_rest_unauthorized(self, monkeypatch):
        ...
        async with FakeHAServer() as server:
            ...
            monkeypatch.setattr(
                "zermes.tools.homeassistant_tool._HASS_TOKEN", "bad-token",
            )
