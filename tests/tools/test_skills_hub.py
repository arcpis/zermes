...
class TestTrustLevelFor:
    ...
    def test_trusted_repo(self):
        ...
        from zermes.tools.skills_guard import TRUSTED_REPOS
        if TRUSTED_REPOS:
...
class TestSkillsShSource:
    ...
    @patch("zermes.tools.skills_hub._write_index_cache")
    @patch("zermes.tools.skills_hub._read_index_cache", return_value=None)
    @patch("zermes.tools.skills_hub.httpx.get")
...
class TestCheckForSkillUpdates:
    def test_bundle_content_hash_matches_installed_content_hash(self, tmp_path):
        from zermes.tools.skills_guard import content_hash
...
class TestConvertToSkillMd:
    def test_basic_conversion(self):
        agent_data = {
            ...
            "meta": {
                "title": "Test Agent",
                "description": "A test zermes.agent.",
                "tags": ["testing", "demo"],
            },
            "config": {
                "systemRole": "You are a helpful test zermes.agent.",
            },
        }
        result = LobeHubSource._convert_to_skill_md(agent_data)
        assert "---" in result
        assert "name: test-agent" in result
        assert "description: A test zermes.agent." in result
        assert "tags: [testing, demo]" in result
        assert "# Test Agent" in result
        assert "You are a helpful test zermes.agent." in result
...
class TestAppendAuditLog:
    def test_creates_log_entry(self, tmp_path):
        ...
        with patch("zermes.tools.skills_hub.AUDIT_LOG", log_file):
...
class TestOptionalSkillSourceBinaryAssets:
    def test_fetch_preserves_binary_assets(self, tmp_path):
        ...
        (pycache_dir / "zermes.cli.cpython-312.pyc").write_bytes(b"junk")
