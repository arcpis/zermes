"""Tests for TTS speed configuration across zermes.providers."""
...
class TestEdgeTtsSpeed:
    def _run(self, tts_config, tmp_path):
        ...
        with patch("zermes.tools.tts_tool._import_edge_tts", return_value=mock_edge):
            from zermes.tools.tts_tool import _generate_edge_tts
...
class TestOpenaiTtsSpeed:
    def _run(self, tts_config, tmp_path, monkeypatch):
        ...
        with patch("zermes.tools.tts_tool._import_openai_client", return_value=mock_cls), \
             patch("zermes.tools.tts_tool._resolve_openai_audio_client_config",
                   return_value=("test-key", None)):
            from zermes.tools.tts_tool import _generate_openai_tts
...
class TestMinimaxTtsSpeed:
    def _run(self, tts_config, tmp_path, monkeypatch):
        ...
        with patch("requests.post", return_value=mock_response) as mock_post:
            from zermes.tools.tts_tool import _generate_minimax_tts
