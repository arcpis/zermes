...
from zermes.tools.checkpoint_manager import (
    CheckpointManager,
    _shadow_repo_path,
    _init_shadow_repo,
    _init_store,
...
def mgr(work_dir, checkpoint_base, monkeypatch):
    monkeypatch.setattr("zermes.tools.checkpoint_manager.CHECKPOINT_BASE", checkpoint_base)
...
class TestTakeCheckpoint:
    ...
    def test_multiple_projects_share_store(self, mgr, tmp_path):
        ...
        from zermes.tools.checkpoint_manager import CHECKPOINT_BASE as BASE
...
class TestErrorResilience:
    ...
    def test_run_git_allows_expected_nonzero_without_error_log(
        ...
        with patch("zermes.tools.checkpoint_manager.subprocess.run", return_value=completed):
            with caplog.at_level(logging.ERROR, logger="zermes.tools.checkpoint_manager"):
    ...
    def test_run_git_missing_git_reports_git_not_found(
        ...
        monkeypatch.setattr("zermes.tools.checkpoint_manager.subprocess.run", raise_missing_git)
        with caplog.at_level(logging.ERROR, logger="zermes.tools.checkpoint_manager"):
            ok, _, stderr = _run_git(
    ...
    def test_checkpoint_failure_does_not_raise(self, mgr, work_dir, monkeypatch):
        ...
        monkeypatch.setattr("zermes.tools.checkpoint_manager._run_git", broken_run_git)
...
class TestPruneCheckpointsV2:
    ...
    def test_deletes_orphan_project_entry(self, tmp_path, monkeypatch):
        base = tmp_path / "checkpoints"
        monkeypatch.setattr("zermes.tools.checkpoint_manager.CHECKPOINT_BASE", base)
