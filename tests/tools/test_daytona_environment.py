...
def make_env(daytona_sdk, monkeypatch):
    ...
    monkeypatch.setattr("zermes.tools.credential_files.get_skills_directory_mount", lambda **kw: None)
    monkeypatch.setattr("zermes.tools.credential_files.iter_skills_files", lambda **kw: [])
    ...
    def _factory(
        ...
        **kwargs,
        ...
        from zermes.tools.environments.daytona import DaytonaEnvironment
...
class TestInterrupt:
    def test_interrupt_stops_sandbox_and_returns_130(self, make_env, monkeypatch):
        ...
        monkeypatch.setattr(
            "zermes.tools.environments.base.is_interrupted", lambda: True
        )
        try:
