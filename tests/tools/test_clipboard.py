...
from zermes.hermes_cli.clipboard import (
...
from zermes.cli import _should_auto_attach_clipboard_image_on_paste
...
class TestSaveClipboardImage:
    def test_dispatches_to_macos_on_darwin(self, tmp_path):
        ...
        with patch("zermes.hermes_cli.clipboard.sys") as mock_sys:
            mock_sys.platform = "darwin"
            with patch("zermes.hermes_cli.clipboard._macos_save", return_value=False) as m:
    ...
    def test_dispatches_to_windows_on_win32(self, tmp_path):
        ...
        with patch("zermes.hermes_cli.clipboard.sys") as mock_sys:
            ...
            with patch("zermes.hermes_cli.clipboard._windows_save", return_value=False) as m:
                save_clipboard_image(dest)
                m.assert_called_once_with(dest)
    ...
    def test_dispatches_to_linux_on_linux(self, tmp_path):
        ...
        with patch("zermes.hermes_cli.clipboard.sys") as mock_sys:
            ...
            with patch("zermes.hermes_cli.clipboard._linux_save", return_value=False) as m:
                save_clipboard_image(dest)
                m.assert_called_once_with(dest)
    ...
    def test_creates_parent_dirs(self, tmp_path):
        ...
        with patch("zermes.hermes_cli.clipboard.sys") as mock_sys:
            mock_sys.platform = "linux"
            with patch("zermes.hermes_cli.clipboard._linux_save", return_value=False):
                save_clipboard_image(dest)
...
class TestMacosPngpaste:
    def test_success_writes_file(self, tmp_path):
        ...
        with patch("zermes.hermes_cli.clipboard.subprocess.run", side_effect=fake_run):
            assert _macos_pngpaste(dest) is True
        assert dest.stat().st_size == len(FAKE_PNG)

    def test_not_installed(self, tmp_path):
        with patch("zermes.hermes_cli.clipboard.subprocess.run", side_effect=FileNotFoundError):
            assert _macos_pngpaste(tmp_path / "out.png") is False

    def test_no_image_in_clipboard(self, tmp_path):
        dest = tmp_path / "out.png"
        with patch("zermes.hermes_cli.clipboard.subprocess.run") as mock_run:
    ...
    def test_timeout_returns_false(self, tmp_path):
        dest = tmp_path / "out.png"
        with patch("zermes.hermes_cli.clipboard.subprocess.run",
...
class TestMacosOsascript:
    ...
    def test_clipboard_info_fails(self, tmp_path):
        with patch("zermes.hermes_cli.clipboard.subprocess.run", side_effect=Exception("fail")):
            assert _macos_osascript(tmp_path / "out.png") is False
...
class TestIsWsl:
    def setup_method(self):
        # _is_wsl is zermes.hermes_constants.is_wsl; reset the function's own module
    ...
    def test_regular_linux(self):
        ...
        # supposed to intercept zermes.hermes_constants.is_wsl's `open` call,
...
class TestWaylandSave:
    ...
    def test_bmp_extraction_with_pillow_convert(self, tmp_path):
        ...
        def fake_run(cmd, **kw):
            ...
            return MagicMock(returncode=0)
        with patch("zermes.hermes_cli.clipboard.subprocess.run", side_effect=fake_run):
            with patch("zermes.hermes_cli.clipboard._convert_to_png", return_value=True):
                assert _wayland_save(dest) is True
...
class TestLinuxSave:
    ...
    def setup_method(self):
        import zermes.hermes_cli.clipboard as cb
        cb._wsl_detected = None

    def test_wsl_tried_first(self, tmp_path):
        dest = tmp_path / "out.png"
        with patch("zermes.hermes_cli.clipboard._is_wsl", return_value=True):
            with patch("zermes.hermes_cli.clipboard._wsl_save", return_value=True) as m:
    ...
    def test_wsl_fails_falls_through_to_xclip(self, tmp_path):
        dest = tmp_path / "out.png"
        with patch("zermes.hermes_cli.clipboard._is_wsl", return_value=True):
            with patch("zermes.hermes_cli.clipboard._wsl_save", return_value=False):
                with patch.dict(os.environ, {}, clear=True):
                    with patch("zermes.hermes_cli.clipboard._xclip_save", return_value=True) as m:
                        assert _linux_save(dest) is True
                        m.assert_called_once_with(dest)

    def test_wayland_tried_when_display_set(self, tmp_path):
        dest = tmp_path / "out.png"
        with patch("zermes.hermes_cli.clipboard._is_wsl", return_value=False):
            with patch.dict(os.environ, {"WAYLAND_DISPLAY": "wayland-0"}):
                with patch("zermes.hermes_cli.clipboard._wayland_save", return_value=True) as m:
                    assert _linux_save(dest) is True
                    m.assert_called_once_with(dest)
    ...
    def test_wayland_fails_falls_through_to_xclip(self, tmp_path):
        ...
        with patch("zermes.hermes_cli.clipboard._is_wsl", return_value=False):
            with patch.dict(os.environ, {"WAYLAND_DISPLAY": "wayland-0"}):
                with patch("zermes.hermes_cli.clipboard._wayland_save", return_value=False):
                    with patch("zermes.hermes_cli.clipboard._xclip_save", return_value=True) as m:
                        assert _linux_save(dest) is True
                        m.assert_called_once_with(dest)
...
class TestWindowsHasImage:
    ...
    def test_clipboard_has_image(self):
        with patch("zermes.hermes_cli.clipboard._get_ps_exe", return_value="powershell"):
            with patch("zermes.hermes_cli.clipboard.subprocess.run") as mock_run:
                mock_run.return_value = MagicMock(stdout="True\n", returncode=0)
    ...
    def test_no_powershell_available(self):
        with patch("zermes.hermes_cli.clipboard._get_ps_exe", return_value=None):
            assert _windows_has_image() is False
...
class TestHasClipboardImageWin32:
    ...
    def test_dispatches_on_win32(self):
        with patch("zermes.hermes_cli.clipboard.sys") as mock_sys:
            mock_sys.platform = "win32"
            with patch("zermes.hermes_cli.clipboard._windows_has_image", return_value=True) as m:
...
class TestConvertToPng:
    ...
    def test_imagemagick_failure_preserves_original(self, tmp_path):
        ...
        with patch.dict(sys.modules, {"PIL": None, "PIL.Image": None}):
            with patch("zermes.hermes_cli.clipboard.subprocess.run", side_effect=fake_run_fail):
    ...
    def test_imagemagick_timeout_preserves_original(self, tmp_path):
        ...
        with patch.dict(sys.modules, {"PIL": None, "PIL.Image": None}):
            with patch("zermes.hermes_cli.clipboard.subprocess.run", side_effect=subprocess.TimeoutExpired("convert", 5)):
                _convert_to_png(dest)
...
class TestHasClipboardImage:
    ...
    def test_macos_dispatch(self):
        with patch("zermes.hermes_cli.clipboard.sys") as mock_sys:
            mock_sys.platform = "darwin"
            with patch("zermes.hermes_cli.clipboard._macos_has_image", return_value=True) as m:
                assert has_clipboard_image() is True
                m.assert_called_once()
    ...
    def test_linux_wsl_dispatch(self):
        with patch("zermes.hermes_cli.clipboard.sys") as mock_sys:
            mock_sys.platform = "linux"
            with patch("zermes.hermes_cli.clipboard._is_wsl", return_value=True):
                with patch("zermes.hermes_cli.clipboard._wsl_has_image", return_value=True) as m:
                    assert has_clipboard_image() is True
                    m.assert_called_once()
    ...
    def test_wsl_falls_through_to_wayland_when_windows_path_empty(self):
        ...
        with patch("zermes.hermes_cli.clipboard.sys") as mock_sys:
            mock_sys.platform = "linux"
            with patch("zermes.hermes_cli.clipboard._is_wsl", return_value=True):
                with patch("zermes.hermes_cli.clipboard._wsl_has_image", return_value=False) as wsl:
                    with patch.dict(os.environ, {"WAYLAND_DISPLAY": "wayland-0"}):
                        with patch("zermes.hermes_cli.clipboard._wayland_has_image", return_value=True) as wl:
                            assert has_clipboard_image() is True
    ...
    def test_linux_wayland_dispatch(self):
        with patch("zermes.hermes_cli.clipboard.sys") as mock_sys:
            ...
            with patch("zermes.hermes_cli.clipboard._is_wsl", return_value=False):
                with patch.dict(os.environ, {"WAYLAND_DISPLAY": "wayland-0"}):
                    with patch("zermes.hermes_cli.clipboard._wayland_has_image", return_value=True) as m:
                        assert has_clipboard_image() is True
                        m.assert_called_once()
    ...
    def test_linux_x11_dispatch(self):
        with patch("zermes.hermes_cli.clipboard.sys") as mock_sys:
            ...
            with patch("zermes.hermes_cli.clipboard._is_wsl", return_value=False):
                with patch.dict(os.environ, {}, clear=True):
                    with patch("zermes.hermes_cli.clipboard._xclip_has_image", return_value=True) as m:
                        assert has_clipboard_image() is True
                        m.assert_called_once()
...
class TestPreprocessImagesWithVision:
    ...
    def cli(self):
        ...
        with patch("zermes.cli.load_cli_config") as mock_cfg:
            ...
            with patch.dict("os.environ", {"OPENROUTER_API_KEY": "test-key"}):
                with patch("zermes.cli.CLI_CONFIG", mock_cfg.return_value):
    ...
    def test_single_image_with_text(self, cli, tmp_path):
        ...
        with patch("zermes.tools.vision_tools.vision_analyze_tool", side_effect=self._mock_vision_success()):
    ...
    def test_vision_failure_includes_path(self, cli, tmp_path):
        img = self._make_image(tmp_path)
        with patch("zermes.tools.vision_tools.vision_analyze_tool", side_effect=self._mock_vision_failure()):
    ...
    def test_vision_exception_includes_path(self, cli, tmp_path):
        ...
        with patch("zermes.tools.vision_tools.vision_analyze_tool", side_effect=_explode):
            result = cli._preprocess_images_with_vision("check this", [img])
        assert isinstance(result, str)
        assert str(img) in result  # path still included for retry
...
class TestTryAttachClipboardImage:
    ...
    def test_image_found_attaches(self, cli):
        with patch("zermes.hermes_cli.clipboard.save_clipboard_image", return_value=True):
    ...
    def test_no_image_doesnt_attach(self, cli):
        with patch("zermes.hermes_cli.clipboard.save_clipboard_image", return_value=False):
            result = cli._try_attach_clipboard_image()
    ...
    def test_mixed_success_and_failure(self, cli):
        ...
        with patch("zermes.hermes_cli.clipboard.save_clipboard_image", side_effect=results):
            cli._try_attach_clipboard_image()
            cli._try_attach_clipboard_image()
            cli._try_attach_clipboard_image()
...
class TestVoiceSubmission:
    ...
    def test_voice_transcript_clears_stale_attached_images(self, cli):
        with patch("zermes.tools.voice_mode.play_beep"):
            with patch("zermes.tools.voice_mode.transcribe_recording", return_value={"success": True, "transcript": "hello"}):
                with patch("os.path.isfile", return_value=False):
                    with patch("zermes.cli._cprint"):
