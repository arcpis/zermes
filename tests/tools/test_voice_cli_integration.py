...
from zermes.tools.tts_tool import _strip_markdown_for_tts
...
class TestStreamingTTSActivation:
    ...
    def test_activates_when_elevenlabs_and_sounddevice_available(self):
        ...
        try:
            ...
            pytest.skip("zermes.tools.tts_tool not available")

        with patch("zermes.tools.tts_tool._load_tts_config") as mock_cfg,\
             patch("zermes.tools.tts_tool._get_provider", return_value="elevenlabs"),\
             patch("zermes.tools.tts_tool._import_elevenlabs") as mock_el,\
             patch("zermes.tools.tts_tool._import_sounddevice") as mock_sd:
    ...
    def test_does_not_activate_when_elevenlabs_missing(self):
        ...
        use_streaming_tts = False
        with patch("zermes.tools.tts_tool._load_tts_config", return_value={"provider": "elevenlabs"}),\
             patch("zermes.tools.tts_tool._get_provider", return_value="elevenlabs"),\
             patch("zermes.tools.tts_tool._import_elevenlabs", side_effect=ImportError("no elevenlabs")):
            try:
                from zermes.tools.tts_tool import (
                    _load_tts_config as load_cfg,
                    _get_provider as get_prov,
    ...
    def test_does_not_activate_when_sounddevice_missing(self):
        ...
        with patch("zermes.tools.tts_tool._load_tts_config", return_value={"provider": "elevenlabs"}),\
             patch("zermes.tools.tts_tool._get_provider", return_value="elevenlabs"),\
             patch("zermes.tools.tts_tool._import_elevenlabs", return_value=MagicMock()),\
             patch("zermes.tools.tts_tool._import_sounddevice", side_effect=OSError("no PortAudio")):
            try:
                from zermes.tools.tts_tool import (
                    _load_tts_config as load_cfg,
                    _get_provider as get_prov,
    ...
    def test_does_not_activate_for_non_elevenlabs_provider(self):
        ...
        use_streaming_tts = False
        with patch("zermes.tools.tts_tool._load_tts_config", return_value={"provider": "edge"}),\
             patch("zermes.tools.tts_tool._get_provider", return_value="edge"):
            try:
                from zermes.tools.tts_tool import (
                    _load_tts_config as load_cfg,
                    _get_provider as get_prov,
    ...
    def test_stale_boolean_imports_no_longer_exist(self):
        ...
        import zermes.tools.tts_tool as tts_mod
...
class TestVoiceMessagePrefix:
    ...
    def test_enable_voice_mode_does_not_modify_system_prompt(self):
        ...
        zermes.agent.ephemeral_system_prompt -- the system prompt must stay
...
class TestVprintForceParameter:
    ...
    def test_error_messages_use_force_in_run_agent(self):
        """Verify that critical error _vprint calls in zermes.run_agent.py
        include force=True."""
        with open("zermes.run_agent.py", "r") as f:
            source = f.read()
...
class TestEnableVoiceModeReal:
    ...
    @patch("zermes.cli._cprint")
    @patch("zermes.hermes_cli.config.load_config", return_value={"voice": {}})
    @patch("zermes.tools.voice_mode.check_voice_requirements",
           return_value={"available": True, "details": "OK"})
    @patch("zermes.tools.voice_mode.detect_audio_environment",
    ...
    @patch("zermes.cli._cprint")
    @patch("zermes.hermes_cli.config.load_config", return_value={"voice": {"auto_tts": True}})
    @patch("zermes.tools.voice_mode.check_voice_requirements",
           return_value={"available": True, "details": "OK"})
    @patch("zermes.tools.voice_mode.detect_audio_environment",
           return_value={"available": True, "warnings": []})
    ...
    @patch("zermes.cli._cprint")
    @patch("zermes.hermes_cli.config.load_config", side_effect=Exception("broken config"))
    @patch("zermes.tools.voice_mode.check_voice_requirements",
           return_value={"available": True, "details": "OK"})
    @patch("zermes.tools.voice_mode.detect_audio_environment",
           return_value={"available": True, "warnings": []})
...
class TestVoiceBeepConfigReal:
    ...
    @patch("zermes.hermes_cli.config.load_config", return_value={"voice": {"beep_enabled": False}})
    def test_beeps_can_be_disabled(self, _cfg):
        cli = _make_voice_cli()
        assert cli._voice_beeps_enabled() is False

    @patch("zermes.cli._cprint")
    @patch("zermes.cli.threading.Thread")
    @patch("zermes.tools.voice_mode.play_beep")
    @patch("zermes.tools.voice_mode.create_audio_recorder")
    @patch(
        "zermes.tools.voice_mode.check_voice_requirements",
        return_value={
            "available": True,
...
class TestDisableVoiceModeReal:
    ...
    @patch("zermes.cli._cprint")
    @patch("zermes.tools.voice_mode.stop_playback")
    ...
    @patch("zermes.cli._cprint")
    @patch("zermes.tools.voice_mode.stop_playback", side_effect=RuntimeError("boom"))
...
class TestVoiceSpeakResponseReal:
    ...
    def test_async_scheduling_clears_done_before_thread_start(self):
        ...
        with patch("zermes.cli.threading.Thread", FakeThread):
    ...
    def test_early_return_when_tts_off(self, _cp):
        ...
        with patch("zermes.tools.tts_tool.text_to_speech_tool") as mock_tts:
            cli._voice_speak_response("Hello")
            mock_tts.assert_not_called()

    @patch("zermes.cli._cprint")
    @patch("zermes.cli.os.unlink")
    @patch("zermes.cli.os.path.getsize", return_value=1000)
    @patch("zermes.cli.os.path.isfile", return_value=True)
    @patch("zermes.cli.os.makedirs")
    @patch("zermes.tools.voice_mode.play_audio_file")
    @patch("zermes.tools.tts_tool.text_to_speech_tool", return_value='{"success": true}')
    ...
    @patch("zermes.cli._cprint")
    @patch("zermes.cli.os.makedirs")
    @patch("zermes.tools.tts_tool.text_to_speech_tool", side_effect=RuntimeError("tts fail"))
...
class TestVoiceStopAndTranscribeReal:
    ...
    def test_guard_not_recording(self, _cp):
        ...
        with patch("zermes.tools.voice_mode.transcribe_recording") as mock_tr:
    ...
    @patch("zermes.cli.os.unlink")
    @patch("zermes.cli.os.path.isfile", return_value=True)
    @patch("zermes.hermes_cli.config.load_config", return_value={"stt": {}})
    @patch("zermes.tools.voice_mode.transcribe_recording",
    ...
    @patch("zermes.cli.os.unlink")
    @patch("zermes.cli.os.path.isfile", return_value=True)
    @patch("zermes.hermes_cli.config.load_config", return_value={"stt": {"model": "whisper-large-v3"}})
    @patch("zermes.tools.voice_mode.transcribe_recording",
