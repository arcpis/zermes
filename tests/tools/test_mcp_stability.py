...
class TestMCPLoopExceptionHandler:
    ...
    def test_suppresses_event_loop_closed(self):
        from zermes.tools.mcp_tool import _mcp_loop_exception_handler
        loop = MagicMock()
...
class TestStdioPidTracking:
    ...
    def test_snapshot_returns_set(self):
        from zermes.tools.mcp_tool import _snapshot_child_pids
    ...
    def test_stdio_pids_starts_empty(self):
        from zermes.tools.mcp_tool import _stdio_pids, _lock
        with _lock:
    ...
    def test_kill_orphaned_uses_sigkill_when_available(self, monkeypatch):
        ...
        # ``zermes.gateway.status._pid_exists`` (so it's safe on Windows — see
        # bpo-14484). Return True so the SIGKILL escalation fires.
        with patch("zermes.tools.mcp_tool.os.kill") as mock_kill, \
             patch("zermes.gateway.status._pid_exists", return_value=True), \
...
class TestMCPInitialConnectionRetry:
    ...
    def test_initial_connect_retries_constant_exists(self):
        ...
        from zermes.tools.mcp_tool import _MAX_INITIAL_CONNECT_RETRIES
