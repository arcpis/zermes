"""Tests for probe_mcp_server_tools() in zermes.tools.mcp_tool."""
...
def _reset_mcp_state():
    ...
    import zermes.tools.mcp_tool as mcp
...
class TestProbeMcpServerTools:
    ...
    def test_returns_empty_when_no_config(self):
        with patch("zermes.tools.mcp_tool._load_mcp_config", return_value={}):
            from zermes.tools.mcp_tool import probe_mcp_server_tools
            result = probe_mcp_server_tools()
        assert result == {}
    ...
    def test_returns_empty_when_all_servers_disabled(self):
        ...
        with patch("zermes.tools.mcp_tool._load_mcp_config", return_value=config):
            from zermes.tools.mcp_tool import probe_mcp_server_tools
            result = probe_mcp_server_tools()
        assert result == {}
    ...
    def test_returns_tools_from_successful_server(self):
        ...
        with patch("zermes.tools.mcp_tool._MCP_AVAILABLE", True), \
             patch("zermes.tools.mcp_tool._load_mcp_config", return_value=config), \
             patch("zermes.tools.mcp_tool._connect_server", side_effect=fake_connect), \
             patch("zermes.tools.mcp_tool._ensure_mcp_loop"), \
             patch("zermes.tools.mcp_tool._run_on_mcp_loop") as mock_run, \
             patch("zermes.tools.mcp_tool._stop_mcp_loop"):
    ...
    def test_cleanup_called_even_on_failure(self):
        ...
        with patch("zermes.tools.mcp_tool._MCP_AVAILABLE", True), \
             patch("zermes.tools.mcp_tool._load_mcp_config", return_value=config), \
             patch("zermes.tools.mcp_tool._ensure_mcp_loop"), \
             patch("zermes.tools.mcp_tool._run_on_mcp_loop", side_effect=RuntimeError("boom")), \
             patch("zermes.tools.mcp_tool._stop_mcp_loop") as mock_stop:
