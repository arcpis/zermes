...
def _reset_signal_scheduler():
    ...
    fresh token bucket."""
    from zermes.gateway.platforms.signal_rate_limit import _reset_scheduler
    _reset_scheduler()
    yield
    _reset_scheduler()

from zermes.gateway.config import Platform
from zermes.tools.send_message_tool import (
...
class TestSendMessageTool:
    def test_cron_duplicate_target_is_skipped_and_explained(self):
        ...
        with patch.dict(
            ...
            clear=False,
             ...
             patch("zermes.gateway.config.load_gateway_config", return_value=config), \
             patch("zermes.tools.interrupt.is_interrupted", return_value=False), \
             patch("zermes.model_tools._run_async", side_effect=_run_async_immediately), \
             patch("zermes.tools.send_message_tool._send_to_platform", new=AsyncMock(return_value={"success": True})) as send_mock, \
             patch("zermes.gateway.mirror.mirror_to_session", return_value=True) as mirror_mock:
            result = json.loads(
                send_message_tool(
                    {
                        "action": "send",
    ...
    def test_resolved_telegram_topic_name_preserves_thread_id(self):
        ...
        with patch("zermes.gateway.config.load_gateway_config", return_value=config), \
             patch("zermes.tools.interrupt.is_interrupted", return_value=False), \
             patch("zermes.gateway.channel_directory.resolve_channel_name", return_value="-1001:17585"), \
             patch("zermes.model_tools._run_async", side_effect=_run_async_immediately), \
             patch("zermes.tools.send_message_tool._send_to_platform", new=AsyncMock(return_value={"success": True})) as send_mock, \
             patch("zermes.gateway.mirror.mirror_to_session", return_value=True):
            result = json.loads(
                send_message_tool(
                    {
                        "action": "send",
    ...
    def test_display_label_target_resolves_via_channel_directory(self, tmp_path):
        ...
        with patch("zermes.gateway.channel_directory.DIRECTORY_PATH", cache_file), \
             patch("zermes.gateway.config.load_gateway_config", return_value=config), \
             patch("zermes.tools.interrupt.is_interrupted", return_value=False), \
             patch("zermes.model_tools._run_async", side_effect=_run_async_immediately), \
             patch("zermes.tools.send_message_tool._send_to_platform", new=AsyncMock(return_value={"success": True})) as send_mock, \
    ...
    def test_mirror_receives_current_session_user_id(self):
        ...
        with patch("zermes.gateway.config.load_gateway_config", return_value=config), \
             ...
             patch("zermes.gateway.session_context.get_session_env") as get_session_env_mock, \
             patch("zermes.gateway.mirror.mirror_to_session", return_value=True) as mirror_mock:
    ...
    def test_top_level_send_failure_redacts_query_token(self):
        ...
        with patch("zermes.gateway.config.load_gateway_config", return_value=config), \
             patch("zermes.tools.interrupt.is_interrupted", return_value=False), \
             patch("zermes.model_tools._run_async", side_effect=_raise_and_close):
            result = json.loads(
                send_message_tool(
                    {
                        "action": "send",
...
class TestSendToPlatformChunking:
    def test_long_message_is_chunked(self):
        ...
        with patch("zermes.tools.send_message_tool._send_discord", send):
            result = asyncio.run(
                _send_to_platform(
                    Platform.DISCORD,
    ...
    def test_slack_messages_are_formatted_before_send(self, monkeypatch):
        ...
        import zermes.gateway.platforms.slack as slack_mod

        monkeypatch.setattr(slack_mod, "SLACK_AVAILABLE", True)
        send = AsyncMock(return_value={"success": True, "message_id": "1"})

        with patch("zermes.tools.send_message_tool._send_slack", send):
            result = asyncio.run(
                _send_to_platform(
                    Platform.SLACK,
                    SimpleNamespace(enabled=True, token="***", extra={}),
    ...
    def test_telegram_media_attaches_to_last_chunk(self):
        ...
        with patch("zermes.tools.send_message_tool._send_telegram", fake_send):
            asyncio.run(
                _send_to_platform(
                    Platform.TELEGRAM,
    ...
    def test_matrix_media_uses_native_adapter_helper(self):
        ...
        try:
            ...
            with patch("zermes.tools.send_message_tool._send_matrix_via_adapter", helper):
                result = asyncio.run(
                    _send_to_platform(
                        Platform.MATRIX,
    ...
    def test_matrix_text_only_uses_lightweight_path(self):
        ...
        with patch("zermes.tools.send_message_tool._send_matrix_via_adapter", helper), \
             patch("zermes.tools.send_message_tool._send_matrix", lightweight):
            result = asyncio.run(
                _send_to_platform(
                    Platform.MATRIX,
                    SimpleNamespace(enabled=True, token="tok", extra={"homeserver": "https://matrix.example.com"}),
    ...
    def test_send_matrix_via_adapter_sends_document(self, tmp_path):
        ...
        with patch.dict(sys.modules, {"zermes.gateway.platforms.matrix": fake_module}):
            result = asyncio.run(
                _send_matrix_via_adapter(
                    SimpleNamespace(enabled=True, token="tok", extra={"homeserver": "https://matrix.example.com"}),
                    "!room:example.com",
...
class TestSendToPlatformWhatsapp:
    def test_whatsapp_routes_via_local_bridge_sender(self):
        ...
        with patch("zermes.tools.send_message_tool._send_whatsapp", async_mock):
            result = asyncio.run(
                _send_to_platform(
                    Platform.WHATSAPP,
...
class TestSendToPlatformDiscordThread:
    ...
    def test_discord_thread_id_passed_to_send_discord(self):
        ...
        with patch("zermes.tools.send_message_tool._send_discord", send_mock):
            result = asyncio.run(
                _send_to_platform(
                    Platform.DISCORD,
                    SimpleNamespace(enabled=True, token="tok", extra={}),
...
class TestSendToPlatformDiscordMedia:
    ...
    def test_media_files_passed_on_last_chunk_only(self):
        ...
        with patch("zermes.tools.send_message_tool._send_discord", side_effect=mock_send_discord):
            result = asyncio.run(
                _send_to_platform(
                    Platform.DISCORD,
                    SimpleNamespace(enabled=True, token="tok", extra={}),
...
class TestSendMatrixUrlEncoding:
    ...
    def test_room_id_is_percent_encoded_in_url(self):
        ...
        with patch("aiohttp.ClientSession", return_value=mock_session):
            from zermes.tools.send_message_tool import _send_matrix
...
class TestSendDiscordForum:
    ...
    def test_directory_forum_creates_thread(self):
        ...
        with patch("aiohttp.ClientSession", return_value=mock_session), \
             patch("zermes.gateway.channel_directory.lookup_channel_type", return_value="forum"):
            result = asyncio.run(
    ...
    def test_directory_channel_skips_forum(self):
        ...
        with patch("aiohttp.ClientSession", return_value=mock_session), \
             patch("zermes.gateway.channel_directory.lookup_channel_type", return_value="channel"):
            result = asyncio.run(
    ...
    def test_directory_none_probes_and_detects_forum(self):
        ...
        with patch("aiohttp.ClientSession", side_effect=lambda **kw: next(session_iter)), \
             patch("zermes.gateway.channel_directory.lookup_channel_type", return_value=None):
            result = asyncio.run(
    ...
    def test_directory_lookup_exception_falls_through_to_probe(self):
        ...
        with patch("aiohttp.ClientSession", return_value=mock_session), \
             patch("zermes.gateway.channel_directory.lookup_channel_type", side_effect=Exception("io error")):
            result = asyncio.run(
                _send_discord("tok", "ch1", "Hello")
            )
...
class TestSendDiscordForumMedia:
    ...
    def test_forum_with_media_uses_multipart(self, tmp_path, monkeypatch):
        ...
        from zermes.tools import send_message_tool as smt
        ...
        monkeypatch.setattr(
            "zermes.gateway.channel_directory.lookup_channel_type", lambda p, cid: "forum"
        )
...
class TestForumProbeCache:
    ...
    def test_probe_result_is_memoized(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "zermes.gateway.channel_directory.lookup_channel_type", lambda p, cid: None
        )
...
def _patch_sendmsg_sleep_and_time(monkeypatch, capture: list):
    ...
    monkeypatch.setattr(
        "zermes.gateway.platforms.signal_rate_limit.asyncio.sleep", fake_sleep
    )
    monkeypatch.setattr(
        "zermes.gateway.platforms.signal_rate_limit.time.monotonic", lambda: offset[0]
    )
...
class TestSendSignalChunking:
    ...
    def test_chunks_attachments_above_max(self, tmp_path, monkeypatch):
        ...
        from zermes.gateway.platforms.signal_rate_limit import (
    ...
    def test_429_with_retry_after_drives_exact_backoff(self, tmp_path, monkeypatch):
        ...
        from zermes.gateway.platforms.signal_rate_limit import SIGNAL_RPC_ERROR_RATELIMIT
    ...
    def test_429_without_retry_after_falls_back_to_default(self, tmp_path, monkeypatch):
        ...
        from zermes.gateway.platforms.signal_rate_limit import SIGNAL_RATE_LIMIT_DEFAULT_RETRY_AFTER
