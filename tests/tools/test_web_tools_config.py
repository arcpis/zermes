...
class TestFirecrawlClientConfig:
    ...
    def setup_method(self):
        ...
        import zermes.tools.web_tools
        zermes.tools.web_tools._firecrawl_client = None
        zermes.tools.web_tools._firecrawl_client_config = None
        for key in (
            "FIRECRAWL_API_KEY",
            "FIRECRAWL_API_URL",
        ...
        self._managed_patchers = [
            patch("zermes.tools.web_tools.managed_nous_tools_enabled", return_value=True),
            patch("zermes.tools.managed_tool_gateway.managed_nous_tools_enabled", return_value=True),
        ]
    ...
    def test_no_config_raises_with_helpful_message(self):
        ...
        with patch("zermes.tools.web_tools.Firecrawl"):
            with patch("zermes.tools.web_tools._read_nous_access_token", return_value=None):
                from zermes.tools.web_tools import _get_firecrawl_client
    ...
    def test_tool_gateway_domain_builds_firecrawl_gateway_origin(self):
        ...
        with patch.dict(os.environ, {"TOOL_GATEWAY_DOMAIN": "nousresearch.com"}):
            with patch("zermes.tools.web_tools._read_nous_access_token", return_value="nous-token"):
                with patch("zermes.tools.web_tools.Firecrawl") as mock_fc:
                    from zermes.tools.web_tools import _get_firecrawl_client
                    result = _get_firecrawl_client()
                    mock_fc.assert_called_once_with(
                        api_key="nous-token",
                        api_url="https://firecrawl-zermes.gateway.nousresearch.com",
                    )
        ...
        with patch.dict(os.environ, {
            ...
            with patch("zermes.tools.web_tools._read_nous_access_token", return_value="nous-token"):
                with patch("zermes.tools.web_tools.Firecrawl") as mock_fc:
                    ...
                    mock_fc.assert_called_once_with(
                        api_key="nous-token",
                        api_url="http://firecrawl-zermes.gateway.nousresearch.com",
                    )
                    assert result is mock_fc.return_value
    ...
    def test_explicit_firecrawl_gateway_url_takes_precedence(self):
        ...
        with patch.dict(os.environ, {
            "FIRECRAWL_GATEWAY_URL": "https://firecrawl-zermes.gateway.localhost:3009/",
            "TOOL_GATEWAY_DOMAIN": "nousresearch.com",
        }):
            with patch("zermes.tools.web_tools._read_nous_access_token", return_value="nous-token"):
                with patch("zermes.tools.web_tools.Firecrawl") as mock_fc:
                    ...
                    mock_fc.assert_called_once_with(
                        api_key="nous-token",
                        api_url="https://firecrawl-zermes.gateway.localhost:3009",
                    )
    ...
    def test_nous_auth_token_respects_hermes_home_override(self, tmp_path):
        ...
        with patch.dict(os.environ, {
            ...
            import zermes.tools.web_tools
            importlib.reload(zermes.tools.web_tools)
            assert zermes.tools.web_tools._read_nous_access_token() == "nous-token"
    ...
    def test_check_auxiliary_model_re_resolves_backend_each_call(self):
        ...
        zermes.tools.web_tools.__dict__["_aux_async_client"] = None

        with patch(
            "zermes.tools.web_tools.get_async_text_auxiliary_client",
            side_effect=[(None, None), (MagicMock(base_url="https://api.openrouter.ai/v1"), "test-model")],
        ):
            assert zermes.tools.web_tools.check_auxiliary_model() is False
            assert zermes.tools.web_tools.check_auxiliary_model() is True
    ...
    async def test_summarizer_re_resolves_backend_after_initial_unavailable_state(self):
        ...
        with patch(
            "zermes.tools.web_tools._resolve_web_extract_auxiliary",
            side_effect=[(None, None, {}), (MagicMock(base_url="https://api.openrouter.ai/v1"), "test-model", {})],
        ), patch(
            "zermes.tools.web_tools.async_call_llm",
            new=AsyncMock(return_value=response),
        ) as mock_async_call:
            assert zermes.tools.web_tools.check_auxiliary_model() is False
            result = await zermes.tools.web_tools._call_summarizer_llm(
    ...
    def test_constructor_failure_allows_retry(self):
        ...
        with patch.dict(os.environ, {"FIRECRAWL_API_KEY": "fc-test"}):
            with patch("zermes.tools.web_tools.Firecrawl") as mock_fc:
                ...
                assert zermes.tools.web_tools._firecrawl_client is None
                result = _get_firecrawl_client()
...
class TestBackendSelection:
    ...
    def test_config_parallel(self):
        ...
        from zermes.tools.web_tools import _get_backend
        with patch("zermes.tools.web_tools._load_web_config", return_value={"backend": "parallel"}):
            assert _get_backend() == "parallel"

    def test_config_exa(self):
        """web.backend=exa in config → 'exa' regardless of other keys."""
        from zermes.tools.web_tools import _get_backend
        with patch("zermes.tools.web_tools._load_web_config", return_value={"backend": "exa"}), \
    ...
    def test_config_firecrawl(self):
        ...
        from zermes.tools.web_tools import _get_backend
        with patch("zermes.tools.web_tools._load_web_config", return_value={"backend": "firecrawl"}), \
             patch.dict(os.environ, {"PARALLEL_API_KEY": "test-key"}):
    ...
    def test_config_tavily(self):
        ...
        from zermes.tools.web_tools import _get_backend
        with patch("zermes.tools.web_tools._load_web_config", return_value={"backend": "tavily"}):
    ...
    def test_fallback_parallel_only_key(self):
        ...
        from zermes.tools.web_tools import _get_backend
        with patch("zermes.tools.web_tools._load_web_config", return_value={}), \
             patch.dict(os.environ, {"PARALLEL_API_KEY": "test-key"}):
            assert _get_backend() == "parallel"
    ...
    def test_invalid_config_falls_through_to_fallback(self):
        ...
        from zermes.tools.web_tools import _get_backend
        with patch("zermes.tools.web_tools._load_web_config", return_value={"backend": "nonexistent"}), \
             patch.dict(os.environ, {"PARALLEL_API_KEY": "test-key"}):
            assert _get_backend() == "parallel"
...
class TestParallelClientConfig:
    ...
    def setup_method(self):
        import zermes.tools.web_tools
        zermes.tools.web_tools._parallel_client = None
    ...
    def test_creates_client_with_key(self):
        ...
        with patch.dict(os.environ, {"PARALLEL_API_KEY": "test-key"}):
            from zermes.tools.web_tools import _get_parallel_client
...
class TestWebSearchSchema:
    ...
    def test_schema_exposes_optional_limit(self):
        ...
        limit_schema = zermes.tools.web_tools.WEB_SEARCH_SCHEMA["parameters"]["properties"]["limit"]

        assert limit_schema["type"] == "integer"
        assert limit_schema["minimum"] == 1
        assert limit_schema["maximum"] == 100
        assert limit_schema["default"] == 5
        assert "limit" not in zermes.tools.web_tools.WEB_SEARCH_SCHEMA["parameters"]["required"]

    def test_registered_handler_passes_limit(self):
        import zermes.tools.web_tools

        entry = zermes.tools.web_tools.registry.get_entry("web_search")
        with patch("zermes.tools.web_tools.web_search_tool", return_value='{"success": true}') as mock_search:
    ...
    def test_web_search_clamps_limit_before_backend_call(self):
        ...
        with patch("zermes.tools.web_tools._get_backend", return_value="parallel"), \
             patch("zermes.tools.web_tools._parallel_search", return_value={"success": True, "data": {"web": []}}) as mock_search, \
             patch("zermes.tools.interrupt.is_interrupted", return_value=False), \
             patch.object(zermes.tools.web_tools._debug, "log_call"), \
             patch.object(zermes.tools.web_tools._debug, "save"):
            result = json.loads(zermes.tools.web_tools.web_search_tool("docs", limit=500))
...
class TestWebSearchErrorHandling:
    ...
    def test_search_error_response_does_not_expose_diagnostics(self):
        ...
        with patch("zermes.tools.web_tools._get_backend", return_value="firecrawl"), \
             patch("zermes.tools.web_tools._get_firecrawl_client", return_value=firecrawl_client), \
             patch("zermes.tools.interrupt.is_interrupted", return_value=False), \
             patch.object(zermes.tools.web_tools._debug, "log_call") as mock_log_call, \
             patch.object(zermes.tools.web_tools._debug, "save"):
            result = json.loads(zermes.tools.web_tools.web_search_tool("test query", limit=3))
...
class TestCheckWebApiKey:
    ...
    def test_parallel_key_only(self):
        with patch.dict(os.environ, {"PARALLEL_API_KEY": "test-key"}):
            from zermes.tools.web_tools import check_web_api_key
...
def test_web_requires_env_includes_exa_key():
    from zermes.tools.web_tools import _web_requires_env
