...
_TIRITH_PATCH = "zermes.tools.tirith_security.check_command_security"
...
class TestTirithImportError:
    def test_import_error_allows(self):
        """When zermes.tools.tirith_security can't be imported, treated as allow."""
        import sys
        # Temporarily remove the module and replace with something that raises
        original = sys.modules.get("zermes.tools.tirith_security")
        sys.modules["zermes.tools.tirith_security"] = None  # causes ImportError on from-import
        try:
            result = check_all_command_guards("echo hello", "local")
            assert result["approved"] is True
        finally:
            if original is not None:
                sys.modules["zermes.tools.tirith_security"] = original
            else:
                sys.modules.pop("zermes.tools.tirith_security", None)
