...
def hermes_env(tmp_path, monkeypatch):
    ...
    import zermes.cron.jobs
    importlib.reload(zermes.cron.jobs)
    import zermes.cron.scheduler
    importlib.reload(zermes.cron.scheduler)
...
def test_update_job_roundtrips_no_agent_flag(hermes_env):
    from zermes.cron.jobs import create_job, update_job, get_job
...
def test_run_job_no_agent_success_returns_script_stdout(hermes_env):
    ...
    from zermes.cron.jobs import create_job
    from zermes.cron.scheduler import run_job
...
def test_run_job_no_agent_empty_output_is_silent(hermes_env):
    ...
    from zermes.cron.jobs import create_job
    from zermes.cron.scheduler import run_job, SILENT_MARKER
...
def test_run_job_no_agent_never_invokes_aiagent(hermes_env):
    ...
    with patch("zermes.run_agent.AIAgent") as ai_mock:
        from zermes.cron.scheduler import run_job
...
def test_run_job_script_shell_script_runs_via_bash(hermes_env):
    ...
    from zermes.cron.scheduler import _run_job_script
