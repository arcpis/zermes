...
class TestMem0UserIdScoping:
    ...
    def test_gateway_user_id_overrides_default(self):
        ...
        from zermes.plugins.memory.mem0 import Mem0MemoryProvider

        provider = Mem0MemoryProvider()
        # Mock _load_config to return a config with default user_id
        with patch("zermes.plugins.memory.mem0._load_config", return_value={
            "api_key": "test-key",
            "user_id": "hermes-user",
            "agent_id": "hermes",
            "rerank": True,
...
class TestHonchoUserIdScoping:
    ...
    def test_gateway_user_id_is_passed_as_runtime_peer(self):
        ...
        with patch(
            ...
            "zermes.plugins.memory.honcho.session.HonchoSessionManager",
        ) as mock_manager_cls:
