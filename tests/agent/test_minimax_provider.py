...
class TestMinimaxBetaHeaders:
    ...
    def _build_and_get_betas(self, api_key, base_url=None):
        ...
        with patch("zermes.agent.anthropic_adapter._anthropic_sdk") as mock_sdk:
    ...
    def test_common_betas_none_url(self):
        from zermes.agent.anthropic_adapter import _common_betas_for_base_url, _COMMON_BETAS
    ...
    def test_common_betas_minimax_url(self):
        from zermes.agent.anthropic_adapter import _common_betas_for_base_url, _TOOL_STREAMING_BETA
...
class TestMinimaxApiMode:
    """Verify determine_api_mode returns anthropic_messages for MiniMax zermes.providers.
...
class TestMinimaxSwitchModelCredentialGuard:
    ...
    def test_switch_to_minimax_does_not_resolve_anthropic_token(self):
        ...
        with patch("zermes.run_agent.AIAgent.__init__", return_value=None):
            from zermes.run_agent import AIAgent
        ...
        with patch("zermes.agent.anthropic_adapter.build_anthropic_client") as mock_build,\
             patch("zermes.agent.anthropic_adapter.resolve_anthropic_token", return_value="sk-ant-leaked") as mock_resolve,\
             patch("zermes.agent.anthropic_adapter._is_oauth_token", return_value=False):
