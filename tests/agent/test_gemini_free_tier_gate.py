...
from zermes.agent.gemini_native_adapter import (
    gemini_http_error,
...
def _run_probe(resp: MagicMock) -> str:
    with patch("zermes.agent.gemini_native_adapter.httpx.Client") as MC:
        inst = MagicMock()
...
class TestProbeGeminiTier:
    ...
    def test_network_error_returns_unknown(self):
        with patch(
            "zermes.agent.gemini_native_adapter.httpx.Client",
