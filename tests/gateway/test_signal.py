...
def _make_signal_adapter(monkeypatch, account="+15551234567", **extra):
    ...
    from zermes.gateway.platforms.signal import SignalAdapter
...
class TestSignalConnectCleanup:
    ...
    async def test_releases_lock_and_closes_client_on_healthcheck_failure(self, monkeypatch):
        ...
        with patch("zermes.gateway.platforms.signal.httpx.AsyncClient", return_value=mock_client), \
...
class TestSignalHelpers:
    def test_redact_phone_long(self):
        from zermes.gateway.platforms.helpers import redact_phone
    ...
    def test_parse_comma_list(self):
        from zermes.gateway.platforms.signal import _parse_comma_list
        assert _parse_comma_list("+1234, +5678 , +9012") == ["+1234", "+5678", "+9012"]
        assert _parse_comma_list("") == []
        assert _parse_comma_list("  ,  ,  ") == []

    def test_guess_extension_png(self):
        from zermes.gateway.platforms.signal import _guess_extension
    ...
    def test_is_image_ext(self):
        from zermes.gateway.platforms.signal import _is_image_ext
    ...
    def test_is_audio_ext(self):
        from zermes.gateway.platforms.signal import _is_audio_ext
        assert _is_audio_ext(".mp3") is True
        assert _is_audio_ext(".ogg") is True
        assert _is_audio_ext(".png") is False

    def test_check_requirements(self, monkeypatch):
        from zermes.gateway.platforms.signal import check_signal_requirements
    ...
    def test_render_mentions(self):
        from zermes.gateway.platforms.signal import _render_mentions
...
class TestSignalAttachmentFetch:
    ...
    async def test_fetch_attachment_uses_id_parameter(self, monkeypatch):
        ...
        with patch("zermes.gateway.platforms.signal.cache_image_from_bytes", return_value="/tmp/test.png"):
    ...
    async def test_fetch_attachment_handles_dict_response(self, monkeypatch):
        ...
        with patch("zermes.gateway.platforms.signal.cache_document_from_bytes", return_value="/tmp/test.pdf"):
...
class TestSignalPhoneRedaction:
    ...
    def _ensure_redaction_enabled(self, monkeypatch):
        # zermes.agent.redact snapshots _REDACT_ENABLED at import time from the
...
class TestSignalMediaExtraction:
    ...
    def test_signal_has_all_media_methods(self, monkeypatch):
        """SignalAdapter must override all media send methods used by zermes.gateway."""
        adapter = _make_signal_adapter(monkeypatch)
        from zermes.gateway.platforms.base import BasePlatformAdapter
...
class TestSignalRpcRateLimit:
    ...
    async def test_raises_on_429_when_opted_in(self, monkeypatch):
        from zermes.gateway.platforms.signal import SignalRateLimitError
...
class TestSignalSendMultipleImages:
    ...
    async def test_full_batch_emits_pacing_notice_for_followup(
        ...
        from zermes.gateway.platforms.signal import SIGNAL_MAX_ATTACHMENTS_PER_MSG
        from zermes.gateway.platforms.signal_rate_limit import (
...
class TestSignalRateLimitDetection:
    ...
    def test_detect_substring_fallback(self):
        from zermes.gateway.platforms.signal import _is_signal_rate_limit_error
    ...
    def test_extract_retry_after_from_results(self):
        from zermes.gateway.platforms.signal import _extract_retry_after_seconds
        err = {
            "code": -5,
            "message": "Failed to send message due to rate limiting",
            "data": {
...
class TestSignalSendTimeout:
    ...
    def test_zero_attachments_uses_default(self):
        from zermes.gateway.platforms.signal import _signal_send_timeout
...
class TestSignalContentlessEnvelope:
    ...
    async def test_allows_message_with_attachment_no_text(self, monkeypatch):
        ...
        with patch("zermes.gateway.platforms.signal.cache_image_from_bytes", return_value="/tmp/img.png"):
            await adapter._handle_envelope({
                "envelope": {
                    "sourceNumber": "+155****9999",
                    "sourceUuid": "05668cf3-8ffa-467e-9b24-f5eefa5cf475",
