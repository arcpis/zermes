...
class TestCheckRequirements:
    ...
    @patch("zermes.gateway.platforms.api_server.AIOHTTP_AVAILABLE", False)
...
class TestAdapterInit:
    ...
    def test_create_agent_forwards_config_reasoning_effort(self, monkeypatch):
        ...
        monkeypatch.setattr("zermes.run_agent.AIAgent", FakeAgent)
        monkeypatch.setattr(
            "zermes.gateway.run._resolve_runtime_agent_kwargs",
            lambda: {
        ...
        monkeypatch.setattr("zermes.gateway.run._resolve_gateway_model", lambda: "gpt-5.5")
        monkeypatch.setattr(
            "zermes.gateway.run._load_gateway_config",
            lambda: {"agent": {"reasoning_effort": "xhigh"}},
        )
        monkeypatch.setattr(
            "zermes.gateway.run.GatewayRunner._load_reasoning_config",
            staticmethod(lambda: {"enabled": True, "effort": "xhigh"}),
        )
        monkeypatch.setattr("zermes.gateway.run.GatewayRunner._load_fallback_model", staticmethod(lambda: None))
        monkeypatch.setattr("zermes.hermes_cli.tools_config._get_platform_tools", lambda *_: set())
...
class TestAgentExecution:
    ...
    async def test_run_agent_uses_session_id_as_task_id(self, adapter):
        ...
        # _run_agent annotates result with the effective zermes.agent.session_id
...
class TestHealthDetailedEndpoint:
    ...
    async def test_health_detailed_returns_ok(self, adapter):
        ...
        app = _create_app(adapter)
        with patch("zermes.gateway.status.read_runtime_status", return_value={
    ...
    async def test_health_detailed_no_runtime_status(self, adapter):
        ...
        app = _create_app(adapter)
        with patch("zermes.gateway.status.read_runtime_status", return_value=None):
            async with TestClient(TestServer(app)) as cli:
                resp = await cli.get("/health/detailed")
                assert resp.status == 200
                data = await resp.json()
...
class TestModelsEndpoint:
    ...
    async def test_models_returns_profile_name(self):
        ...
        with patch("zermes.gateway.platforms.api_server.APIServerAdapter._resolve_model_name", return_value="lucas"):
    ...
    def test_resolve_model_name_default_profile(self):
        ...
        with patch("zermes.hermes_cli.profiles.get_active_profile_name", return_value="default"):
    ...
    def test_resolve_model_name_named_profile(self):
        ...
        with patch("zermes.hermes_cli.profiles.get_active_profile_name", return_value="lucas"):
...
class TestChatCompletionsEndpoint:
    ...
    async def test_stream_sends_keepalive_during_quiet_tool_gap(self, adapter):
        ...
        import asyncio
        import zermes.gateway.platforms.api_server as api_server_mod
    ...
    async def test_successful_completion(self, adapter):
        """Test a successful chat completion with mocked zermes.agent."""
        mock_result = {
...
class TestResponsesStreaming:
    ...
    async def test_stream_cancelled_persists_incomplete_snapshot(self, adapter):
        ...
        import zermes.gateway.platforms.api_server as api_mod
        import queue as _q
...
class TestSessionIdHeader:
    ...
    async def test_db_failure_falls_back_to_empty_history(self, auth_adapter):
        ...
        async with TestClient(TestServer(app)) as cli:
            with patch.object(auth_adapter, "_run_agent", new_callable=AsyncMock) as mock_run, \
                 patch("zermes.hermes_state.SessionDB", side_effect=Exception("DB unavailable")):
                mock_run.return_value = (mock_result, {"input_tokens": 0, "output_tokens": 0, "total_tokens": 0})
