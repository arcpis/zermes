...
class TestUsageCachedAgent:
    ...
    async def test_cached_agent_shows_detailed_usage(self):
        ...
        with patch("zermes.agent.rate_limit_tracker.format_rate_limit_compact", return_value="RPM: 50/60"),\
             patch("zermes.agent.usage_pricing.estimate_usage_cost") as mock_cost:
    ...
    async def test_no_agent_anywhere_falls_to_history(self):
        ...
        with patch("zermes.agent.model_metadata.estimate_messages_tokens_rough", return_value=500):
            result = await runner._handle_usage_command(event)
...
class TestUsageAccountSection:
    ...
    async def test_usage_command_includes_account_section(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "zermes.gateway.run.fetch_account_usage",
            lambda provider, base_url=None, api_key=None: object(),
        )
        monkeypatch.setattr(
            "zermes.gateway.run.render_account_usage_lines",
    ...
    async def test_usage_command_uses_persisted_provider_when_agent_not_running(self, monkeypatch):
        ...
        monkeypatch.setattr("zermes.gateway.run.asyncio.to_thread", _fake_to_thread)
        monkeypatch.setattr(
            "zermes.gateway.run.fetch_account_usage",
            lambda provider, base_url=None, api_key=None: object(),
        )
