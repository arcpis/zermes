"""Tests for zermes.gateway.run._check_unavailable_skill.
...
* Slug is derived from the frontmatter ``name:`` (exactly matching
  :func:`zermes.agent.skill_commands.scan_skill_commands`), so the slug differs
...
* ``disabled`` membership is checked by the declared name, because that
  is what :func:`zermes.hermes_cli.skills_config.save_disabled_skills` stores.
"""
...
def test_frontmatter_slug_matched_even_when_dir_name_differs(
    ...
    monkeypatch.setattr(
        "zermes.gateway.run._get_disabled_skill_names",
    ...
    with patch(
        ...
        "zermes.agent.skill_utils.get_all_skills_dirs",
...
def test_unknown_command_still_returns_none(
    ...
    with patch(
        "zermes.tools.skills_tool._get_disabled_skill_names", return_value=set()
    ), patch(
        "zermes.agent.skill_utils.get_all_skills_dirs", return_value=[tmp_skills]
    ):
...
def test_optional_skill_uses_frontmatter_slug(
    ...
    optional skill at ``resources/skills/optional/mlops/stable-diffusion/SKILL.md``
    ...
    # ``get_optional_skills_dir(repo_root / "resources/skills/optional")`` — we
    # can't easily retarget ``repo_root``, so patch the resolver.
    monkeypatch.setattr(
        "zermes.hermes_constants.get_optional_skills_dir",
    ...
    with patch(
        ...
        "zermes.agent.skill_utils.get_all_skills_dirs", return_value=[empty_skills]
    ):
        msg = gateway_run._check_unavailable_skill("stable-diffusion-image-generation")
