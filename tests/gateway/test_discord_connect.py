...
import zermes.gateway.platforms.discord as discord_platform  # noqa: E402
...
async def test_connect_only_requests_members_intent_when_needed(monkeypatch, allowed_users, expected_members_intent):
    ...
    monkeypatch.setattr("zermes.gateway.status.acquire_scoped_lock", lambda scope, identity, metadata=None: (True, None))
    monkeypatch.setattr("zermes.gateway.status.release_scoped_lock", lambda scope, identity: None)
...
async def test_connect_releases_token_lock_on_timeout(monkeypatch):
    ...
    monkeypatch.setattr("zermes.gateway.status.acquire_scoped_lock", lambda scope, identity, metadata=None: (True, None))
    released = []
    monkeypatch.setattr("zermes.gateway.status.release_scoped_lock", lambda scope, identity: released.append((scope, identity)))
...
async def test_post_connect_initialization_skips_same_fingerprint_after_success(tmp_path, monkeypatch):
    adapter = DiscordAdapter(PlatformConfig(enabled=True, token="test-token"))
    monkeypatch.setattr("zermes.hermes_constants.get_hermes_home", lambda: tmp_path)
