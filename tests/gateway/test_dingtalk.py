...
class TestDingTalkRequirements:
    ...
    def test_returns_false_when_sdk_missing(self, monkeypatch):
        with patch.dict("sys.modules", {"dingtalk_stream": None}):
            monkeypatch.setattr(
                "zermes.gateway.platforms.dingtalk.DINGTALK_STREAM_AVAILABLE", False
            )
            from zermes.gateway.platforms.dingtalk import check_dingtalk_requirements
            assert check_dingtalk_requirements() is False

    def test_returns_false_when_env_vars_missing(self, monkeypatch):
        monkeypatch.setattr(
            "zermes.gateway.platforms.dingtalk.DINGTALK_STREAM_AVAILABLE", True
        )
        monkeypatch.setattr("zermes.gateway.platforms.dingtalk.HTTPX_AVAILABLE", True)
...
class TestDingTalkAdapterInit:
    ...
    def test_reads_config_from_extra(self):
        from zermes.gateway.platforms.dingtalk import DingTalkAdapter
...
class TestWebhookDomainAllowlist:
    ...
    def test_api_domain_accepted(self):
        from zermes.gateway.platforms.dingtalk import _DINGTALK_WEBHOOK_RE
...
class TestHandlerProcessIsAsync:
    ...
    def test_process_is_coroutine_function(self):
        from zermes.gateway.platforms.dingtalk import _IncomingHandler
...
class TestIncomingHandlerProcess:
    ...
    async def test_process_extracts_session_webhook(self):
        ...
        from zermes.gateway.platforms.dingtalk import _IncomingHandler, DingTalkAdapter
