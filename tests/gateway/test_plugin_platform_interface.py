...
def _discover_platform_plugins() -> list[str]:
    """Return names of all bundled platform zermes.plugins."""
...
class _MockPluginContext:
    """Minimal mock of zermes.hermes_cli.plugins.PluginContext.
...
def _import_platform_module(name: str) -> ModuleType:
    """Import zermes.plugins.platforms.<name> in a test-safe way."""
    # Make sure the project root is on sys.path so relative imports work
    if str(PROJECT_ROOT) not in sys.path:
        sys.path.insert(0, str(PROJECT_ROOT))
    module = importlib.import_module(f"zermes.plugins.platforms.{name}")
    return module
