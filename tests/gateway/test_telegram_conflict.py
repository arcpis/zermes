...
def _no_auto_discovery(monkeypatch):
    ...
    async def _noop():
        return []
    monkeypatch.setattr("zermes.gateway.platforms.telegram.discover_fallback_ips", _noop)
    # Mock HTTPXRequest so the builder chain doesn't fail
    monkeypatch.setattr("zermes.gateway.platforms.telegram.HTTPXRequest", lambda **kwargs: MagicMock())
...
async def test_connect_rejects_same_host_token_lock(monkeypatch):
    ...
    monkeypatch.setattr(
        "zermes.gateway.status.acquire_scoped_lock",
...
async def test_polling_conflict_retries_before_fatal(monkeypatch):
    ...
    monkeypatch.setattr(
        "zermes.gateway.status.release_scoped_lock",
    ...
    monkeypatch.setattr("zermes.gateway.platforms.telegram.Application", SimpleNamespace(builder=MagicMock(return_value=builder)))
...
async def test_connect_clears_webhook_before_polling(monkeypatch):
    ...
    builder.build.return_value = app
    monkeypatch.setattr(
        "zermes.gateway.platforms.telegram.Application",
...
async def test_disconnect_skips_inactive_updater_and_app(monkeypatch):
    ...
    warning = MagicMock()
    monkeypatch.setattr("zermes.gateway.platforms.telegram.logger.warning", warning)
