...
async def test_notify_on_complete_sets_internal_flag(monkeypatch, tmp_path):
    ...
    import zermes.tools.process_registry as pr_module
...
async def test_non_internal_event_without_user_triggers_pairing(monkeypatch, tmp_path):
    ...
    import zermes.gateway.run as gateway_run
    import zermes.gateway.pairing as pairing_mod

    monkeypatch.setattr(gateway_run, "_hermes_home", tmp_path)
    # zermes.gateway.pairing.PAIRING_DIR is a module-level constant captured at
