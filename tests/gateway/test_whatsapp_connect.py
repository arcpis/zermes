...
def _make_adapter():
    ...
    from zermes.gateway.platforms.whatsapp import WhatsAppAdapter
...
def _connect_patches(mock_proc, mock_fh, mock_client_cls=None):
    ...
    patches = {
        "zermes.gateway.platforms.whatsapp.check_whatsapp_requirements": True,
        "zermes.gateway.platforms.whatsapp.asyncio.create_task": MagicMock(),
    }
    base = [
        patch("zermes.gateway.platforms.whatsapp.check_whatsapp_requirements", return_value=True),
        patch.object(Path, "exists", return_value=True),
        patch.object(Path, "mkdir", return_value=None),
        patch("subprocess.run", return_value=MagicMock(returncode=0)),
        patch("subprocess.Popen", return_value=mock_proc),
        patch("builtins.open", return_value=mock_fh),
        patch("zermes.gateway.platforms.whatsapp.asyncio.sleep", new_callable=AsyncMock),
        patch("zermes.gateway.platforms.whatsapp.asyncio.create_task"),
    ]
...
class TestConnectCleanup:
    ...
    async def test_releases_lock_when_npm_install_fails(self):
        ...
        with patch("zermes.gateway.platforms.whatsapp.check_whatsapp_requirements", return_value=True), \
...
class TestKillPortProcess:
    ...
    def test_uses_netstat_and_taskkill_on_windows(self):
        from zermes.gateway.platforms.whatsapp import _kill_port_process
        ...
        with patch("zermes.gateway.platforms.whatsapp._IS_WINDOWS", True), \
             patch("zermes.gateway.platforms.whatsapp.subprocess.run", side_effect=run_side_effect) as mock_run:
    ...
    def test_does_not_kill_wrong_port_on_windows(self):
        ...
        with patch("zermes.gateway.platforms.whatsapp._IS_WINDOWS", True), \
             patch("zermes.gateway.platforms.whatsapp.subprocess.run", return_value=mock_netstat) as mock_run:
            _kill_port_process(3000)
    ...
    def test_uses_fuser_on_linux(self):
        ...
        with patch("zermes.gateway.platforms.whatsapp._IS_WINDOWS", False), \
             patch("zermes.gateway.platforms.whatsapp.subprocess.run", return_value=mock_check) as mock_run:
            _kill_port_process(3000)
    ...
    def test_suppresses_exceptions(self):
        ...
        with patch("zermes.gateway.platforms.whatsapp._IS_WINDOWS", True), \
             patch("zermes.gateway.platforms.whatsapp.subprocess.run", side_effect=OSError("no netstat")):
...
class TestHttpSessionLifecycle:
    ...
    async def test_disconnect_uses_taskkill_tree_on_windows(self):
        ...
        with patch("zermes.gateway.platforms.whatsapp._IS_WINDOWS", True), \
             patch("zermes.gateway.platforms.whatsapp.subprocess.run", return_value=MagicMock(returncode=0)) as mock_run, \
             patch("zermes.gateway.platforms.whatsapp.asyncio.sleep", new_callable=AsyncMock):
