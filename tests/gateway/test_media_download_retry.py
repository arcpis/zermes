...
class TestCacheImageFromBytes:
    """Tests for zermes.gateway.platforms.base.cache_image_from_bytes"""

    def test_caches_valid_jpeg(self, tmp_path, monkeypatch):
        monkeypatch.setattr("zermes.gateway.platforms.base.IMAGE_CACHE_DIR", tmp_path / "img")
...
class TestCacheImageFromUrl:
    """Tests for zermes.gateway.platforms.base.cache_image_from_url"""
    ...
    def test_success_on_first_attempt(self, _mock_safe, tmp_path, monkeypatch):
        ...
        async def run():
            with patch("httpx.AsyncClient", return_value=mock_client):
                from zermes.gateway.platforms.base import cache_image_from_url
...
class TestCacheAudioFromUrl:
    """Tests for zermes.gateway.platforms.base.cache_audio_from_url"""

    def test_success_on_first_attempt(self, _mock_safe, tmp_path, monkeypatch):
        """A clean 200 response caches the audio and returns a path."""
        monkeypatch.setattr("zermes.gateway.platforms.base.AUDIO_CACHE_DIR", tmp_path / "audio")
        ...
        async def run():
            with patch("httpx.AsyncClient", return_value=mock_client):
                from zermes.gateway.platforms.base import cache_audio_from_url
...
class TestSSRFRedirectGuard:
    ...
    def test_image_blocks_private_redirect(self, tmp_path, monkeypatch):
        ...
        async def run():
            with patch("zermes.tools.url_safety.is_safe_url", side_effect=fake_safe), \
...
import zermes.gateway.platforms.slack as _slack_mod  # noqa: E402
_slack_mod.SLACK_AVAILABLE = True

from zermes.gateway.platforms.slack import SlackAdapter  # noqa: E402
from zermes.gateway.config import Platform, PlatformConfig  # noqa: E402
...
def _make_mm_adapter():
    ...
    from zermes.gateway.platforms.mattermost import MattermostAdapter
