...
class TestWeComRequirements:
    def test_returns_false_without_aiohttp(self, monkeypatch):
        monkeypatch.setattr("zermes.gateway.platforms.wecom.AIOHTTP_AVAILABLE", False)
        monkeypatch.setattr("zermes.gateway.platforms.wecom.HTTPX_AVAILABLE", True)
        from zermes.gateway.platforms.wecom import check_wecom_requirements

        assert check_wecom_requirements() is False

    def test_returns_false_without_httpx(self, monkeypatch):
        monkeypatch.setattr("zermes.gateway.platforms.wecom.AIOHTTP_AVAILABLE", True)
        monkeypatch.setattr("zermes.gateway.platforms.wecom.HTTPX_AVAILABLE", False)
        from zermes.gateway.platforms.wecom import check_wecom_requirements
...
class TestWeComAdapterInit:
    def test_declares_non_editable_message_capability(self):
        from zermes.gateway.platforms.wecom import WeComAdapter
...
class TestWeComConnect:
    ...
    async def test_connect_records_missing_credentials(self, monkeypatch):
        import zermes.gateway.platforms.wecom as wecom_module
        from zermes.gateway.platforms.wecom import WeComAdapter
...
class TestWeComQrScan:
    @patch("zermes.gateway.platforms.wecom.time")
    @patch("zermes.gateway.platforms.wecom.json.loads")
    @patch("zermes.gateway.platforms.wecom.logger")
...
class TestMediaHelpers:
    ...
    def test_oversized_file_is_rejected(self):
        from zermes.gateway.platforms.wecom import ABSOLUTE_MAX_BYTES, WeComAdapter
...
class TestMediaUpload:
    ...
    async def test_upload_media_bytes_uses_sdk_sequence(self, monkeypatch):
        import zermes.gateway.platforms.wecom as wecom_module
        from zermes.gateway.platforms.wecom import (
    ...
    @pytest.mark.asyncio
    @patch("zermes.tools.url_safety.is_safe_url", return_value=True)
...
class TestSend:
    ...
    async def test_send_uses_proactive_payload(self):
        from zermes.gateway.platforms.wecom import APP_CMD_SEND, WeComAdapter
...
class TestWeComZombieSessionFix:
    ...
    async def test_open_connection_includes_device_id_in_subscribe(self):
        from zermes.gateway.platforms.wecom import APP_CMD_SUBSCRIBE, WeComAdapter
        ...
        with patch("zermes.gateway.platforms.wecom.aiohttp.ClientSession", _FakeSession):
    ...
    def test_remember_chat_req_id_is_bounded(self):
        from zermes.gateway.platforms.wecom import DEDUP_MAX_SIZE, WeComAdapter
